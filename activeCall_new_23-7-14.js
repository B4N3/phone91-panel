...
function getDataFromDb(socket,pool)
{
    ...
    var i;
    var str="";
     fs.appendFile('/backup/nodeError', ' datafrom db ', function (err) {
//                    console.log(err);
                    });
    pool.getConnection(function(err, connection) {
         fs.appendFile('/backup/nodeError', ' pool connection '+err, function (err) {
//                    console.log(err);
                    });
//        var chainIdEsc = sanitizer.sanitize(globalID.chainId); 
       ...
       // console.log("chain Id::"+JSON.stringify(chainIdEsc));
        ...
        connection.query(sql,function(error, rows,fields){
            ...
            fs.appendFile('/backup/nodeError', ' pool connection '+error+'Q'+sql, function (err) {
//                    console.log(err);
                    });
...
//            return false;
            console.log("type "+ typeof rows);
             fs.appendFile('/backup/nodeError', ' type or rows '+ typeof rows, function (err) {
  //                  console.log(err);
                    });
    ...
    //        console.log("scket length"+sqlResult[socket.id].length);
             fs.appendFile('/backup/nodeError', ' sqlresult length '+ sqlResult[socket.id].length, function (err) {
    //                console.log(err);
                    });
            if(sqlResult[socket.id].length < 1 || typeof sqlResult[socket.id] === undefined){
                 fs.appendFile('/backup/nodeError', ' length undefined ', function (err) {
//                    console.log(err);
                    });
                return false;
            }
...
//            pool.getConnection(function(err, connection) {
               console.log("chain"+chainIdStr);               
                fs.appendFile('/backup/nodeError', ' chainid list '+chainIdStr, function (err) {
//                    console.log(err);
                    });
               if(chainIdStr != ""){
                var chainIdStrEsc = sanitizer.sanitize(chainIdStr[socket.id]);
                var manageSql = "select name,chainId,userName from 91_manageClient where chainId IN ("+chainIdStr+")";
                connection.query("select name,chainId,userName from 91_manageClient where chainId IN ("+chainIdStr+")",function(error, result,fields){
                 //   console.log(error);
                  fs.appendFile('/backup/nodeError', ' manageClientError '+error + 'sqlM '+ manageSql , function (err) {
//                    console.log(err);
                    });
                    var j;
                    nameArr = [];
                    fs.appendFile('/backup/nodeError', ' resultmanageclinet length '+ result.length , function (err) {
  //                  console.log(err);
                    });
...
//            });
      ...
      fs.appendFile('/backup/nodeError', ' sqlresultsocket ', function (err) {
                    console.log('pool = ',pool);
                    console.log('err = ',err);
//                    console.log(connection);
                    
                    fs.appendFile('/backup/nodeError', ' pooldata'+ pool , function (err) {
    //                console.log(err);
                    });
//                    fs.appendFile('/backup/nodeError', ' connection '+ connection , function (err) {
//                    console.log(err);
//                    });
                    });
...
function getCounter(socket,pool)
{
    var chainIdEsc = sanitizer.sanitize(connectChainArrTwo[socket.id]); 
        ...
        fs.appendFile('/backup/nodeError', ' getcounter chain id '+chainIdEsc, function (err) {
                    console.log(err);
                    });
  ...
  pool.getConnection(function(err, connection) { 
      //console.log(err);
       fs.appendFile('/backup/nodeError', ' currentcall sql error '+err, function (err) {
      //              console.log(err);
                    });
        connection.query(sql,function(error, rows,fields){
           fs.appendFile('/backup/nodeError', ' currentcall sql errorQ '+sql, function (err) {
        //            console.log(err);
                    });
            //res.writeHead(200,{'Content-Type':'text/plain'});
