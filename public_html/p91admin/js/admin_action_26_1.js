...
function add_client_submit()
	{
			...
			else
			{
				...
				$.ajax({
		   		...
		   		data: "update_detail="+update_detail+"&fname="+fname+"&lname="+lname+"&user_name="+user_name+"&mob_no="+mob_no+"&expiry="+expiry+"&balance="+balance+"&utype="+utype+"&user_email="+email,
...
function edit_funds_submit(form_id)
{
   ...
   var amount=$("#amount_"+form_id).val();
       $.ajax({
              type: "POST",
              url: "../action_layer.php?action=41",
              data: "id="+form_id+"&update_detail=3&expiry="+expiry+"&sms="+sms+"&amount="+amount+"&type="+type,
             success: function(msg){
              if(msg=='Update Successful')
				{
				 load_next('manage_admin.php');
...
function edit_voice_bal(id)
{
	...
	$.ajax({
             type: "POST",
             url: "../action_layer.php?action=41",
			 data: "id="+id+"&update_detail=5&vsms="+vsms+"&vbal="+vbal+"&type="+type,
	         success: function(msg){
			 if(msg=='Update Successful')
			  {
				load_next('manage_admin.php');
