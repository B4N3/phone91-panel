...
function edit_funds_submit(form_id)
{
   ...
   var description=$("#description_"+form_id).val();
      $.ajax({
              type: "POST",
              url: "../action_layer.php?action=41",
              data: "id="+form_id+"&update_detail=3&expiry="+expiry+"&sms="+sms+"&amount="+amount+"&type="+type+"&description="+description+"&plan="+plan,
             success: function(msg){
              if(msg=='Update Successful')
				{
				// load_next('manage_admin.php');
...
function edit_voice_bal(id)
{
	...
	$.ajax({
             type: "POST",
             url: "../action_layer.php?action=41",
			 data: "id="+id+"&update_detail=5&vsms="+vsms+"&vbal="+vbal+"&type="+type+"&description="+description,
	         success: function(msg){
			 if(msg=='Update Successful')
			  {
				//load_next('manage_admin.php');
