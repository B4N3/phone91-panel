        ...
        if (strlen($userName) > 3) 
        {
           ...
           $resultSel = $this->selectData('userId,userName', '91_userLogin','userName="'.$userName.'"');
            ...
            else 
            {
                ...
                $resultVerSel = $this->selectData('verifiedNumber,countryCode,confirmCode', '91_verifiedNumbers',"userId='" . $uid . "'  and isDefault=1 ");
...
//                    $result = mysql_query("select tempNumber,countryCode from 91_tempNumbers where userId=" . $uid) or die("Error");
    ...
    public function forgetPassword($userName,$smsCall)
    {
        ...
        {
            $resultVerSel = $this->selectData('verifiedNumber,countryCode,confirmCode', '91_verifiedNumbers',"varifiedNumber='" . $userName . "' and domainResellerId = '".$resellerIdServer."' and isDefault=1 ");
            //echo $this->querry;
           // die();
	...
	if($userId != NULL && preg_match(NOTNUM_REGX, $userId))
        ...
        if(is_numeric($userName) || ctype_digit($userName))
        {
            ...
            if(isset($userId) && !empty($userId))
                $condition =  "userId = '".$userId."' and domainResellerId = '".$resellerIdServer."' and isDefault=1 ";
            else
                $condition =  "domainResellerId = '".$resellerIdServer."' and isDefault=1";
...
#find country name 
    ...
    function countryAllDetail() 
    {
       $country = array();
       $url = CALLSERVERURL."isoData.php";  
	   //$url = 'http://127.0.0.1/isoData.php'; 
       $ch = curl_init(); 
...
//                            ob_end_clean(); 
    ...
    */
	...
	if(!isset($param['userId']) || preg_match(NOTNUM_REGX,$param['userId']) )
	    return false;
        #condition for find username and pin detail 
        $condition = "userId = '" .trim($param['userId'])."' ";
