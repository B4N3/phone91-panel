...
var isReseller = <?php if(isset($_REQUEST['isReseller'])){ echo $_REQUEST['isReseller']; }else echo 0;?>;
var number = <?php if(isset($_REQUEST['number'])){ echo $_REQUEST['number'];} else echo 0;?>;
...
if(isReseller == 2){
    
    {
            ...
            $("#forgot_div").empty();
            //append confirm box below forget div
            $('<p> verification code send.</p><p>Enter  confirmation code:</p><input tabindex="1" name="textCon" type="text" id="textConfirm" /><input name="" tabindex="3" type="button" value="confirm" id="idconfirm" class="btn"/><span id="codeError"></span>').appendTo("#forgot_div");
                        ...
                        success:function(flag)
                            {//flag contain userid or 0
                                ...
                                if(flag.key != "" && flag.key != undefined)
                                {
                                    //empty forgat_pass div and append fields for resend
                                    $("#forgot_div").empty();
                                    $('<p>Successfully confirmed.</p><div id="changePass"><h3>Reset Password</h3><div class=""><form name="f" method="post" class="form1" action="resetpass.php"><label>New Password</label><input type="hidden" name="mobNum" id="mobNum" value="'+number+'"><input type="password" name="new_pass" id="new_pwd" /><div></div><label>Confirm New Password</label><input type="password" name="confirm" id="confirm_pwd" /><div></div><input type="button" id="change_pwd_btn" name="submit" class="search_btn medium green awesome" value="Set Password" onClick="reset_pass(\''+code+'\',\''+flag.key+'\',\''+number+'\');" /></form></div></div><span id="passError"></span>').appendTo("#forgot_div");
                                    show_message("Successfully confirmed","success");
                                }
                                else //if wrong confirmation code
                                {
