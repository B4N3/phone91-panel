...
mail('sudhirp29@gmail.com', 'cron run', "successfully cron run with nzd currency");
...
$urlQAR = "http://www.exchangerate-api.com/USD/QAR/1?k=mhXK9-WknIS-he8My";
$QARamt = get_data($urlQAR);

$urlRUB = "http://www.exchangerate-api.com/USD/RUB/1?k=mhXK9-WknIS-he8My";
$RUBamt = get_data($urlRUB);
...
$currency = array("USD_to_INR"=>(float)$USDamt,"USD_to_AED"=>(float)$AEDamt,"USD_to_EURO"=>(float)$Euroamt,"USD_to_NZD"=>(float)$NZDamt,"USD_to_QAR"=>(float)$QARamt,"USD_to_RUB"=>(float)$RUBamt);//,"USD_to_INR"=>$usd_inr,"AED_to_INR"=>$aed_inr,"EURO_to_INR"=>$euro_inr
$data=array("date"=>new MongoDate(time()),"currency"=>$currency);
#insert current date currency record in database 
$status = $conn->mongo_insert($currencyRate,$data);
if(!$status){
    mail('sameer@hostnsoft.com', 'cron run', "successfully cron run");
    mail('sudhirp29@gmail.com', 'cron run', "currency database not update.");
}
