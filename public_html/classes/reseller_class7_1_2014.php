...
//              $allUserCondition ="and lower(userName) like '%".$q."%'";
                ...
                $result = $this->loadUsers($q, $userid, $first_limit, $limit, 0, $filterCond);
            ...
            if ($allclient != 0) {
                ...
                $result = $this->loadUsers(0, $userid, $first_limit, $limit, 3, $filterCond);
            }
        }
...
//            #find user status block or unblock
    ...
    function getCurrCondition($amt) {
        ...
        foreach ($tariffAmtArr as $key => $value) {
            $condStr.=' tariffId=' . $key . ' and balance <' . $value . ' or ';
        }
...
//		 die('dfd');
        ...
        if (isset($request['reNotUsingCall']) && $request['reNotUsingCall'] == 'on') {
            ...
            $rechargedButNotUsing.=' and userId NOT IN (SELECT id_client from 91_calls WHERE status = "ANSWERED") ';
        ...
        if (!empty($platFormCondition)) {
            ...
            $conditionArr[] = ' userName IN (SELECT userName from 91_pushNotification WHERE appId IN (' . implode(',', $platFormCondition) . '))';
        }
        ...
        if (isset($request['selectCountry']) && $request['selectCountry'] == 'on') {
            ...
            $code = $request['code'];
            $countryCond = "userId IN (SELECT userId from 91_verifiedNumbers WHERE countryCode=$code)";
