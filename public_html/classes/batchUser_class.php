...
class batchUser_class extends fun //validation_class.php
{
    ...
    function addNewClientBatch($request,$resellerId){
        ...
        if (!preg_match("/^[0-9]+(\.[0-9]{1,3})?$/", $request['balance'])) {
...
function batchTrans_sub($fromUser,$batchId,$talktime,$paymentMode,$debit,$credit,$closingBalance,$description,$currency)
    ...
    function getBatchInternalDetail($batchId){
        ...
        if($result->num_rows > 0)  #- log error
        {
            ...
            isset($row['chainId'])? $chainId = $row['chainId'] : $chainId ='';
            
            return array( "name" => $name, "userName" => $userName , "currencyId" => $currencyId,"resellerId" =>$resellerId,"deleteFlag" =>$deleteFlag,"chainId"=>$chainId);
        }
        else
        {
...
//      $transactionData['totalCount'] = ceil($count/$limit);
     ...
     function checkTransactionValidation($parm)
    {
        ...
        {
           return json_encode(array("status"=>"error","msg"=>"please enter a valid amount (4 digit after decimal.) !"));
        }
    ...
    function editBatchDetail($param,$userId,$type){
            ...
            if (!preg_match("/^[a-zA-Z_@-]{2,30}$/",$param['batchName']))
            {  
...
//                $listenTime = 0;
            ...
            if(!$result || $this->db->affected_rows == 0){
                  trigger_error('problem while edit batch detail,query:'.$qur);
                     return json_encode(array("status"=>"success","msg"=>"nothing to  update !"));
            }
     ...
     * @author sudhir pandey <sudhir@hostnsoft.com>
     * @since 22-05-14
     * @desc function use to show batch chain detail (batch creator chain). 
     */
    function showBatchChainDetail($param,$session){
        
       
        # check for admin or not 
        if($session['client_type'] != 1){
             return json_encode(array("status" => "error", "msg" => "you have no permission to get batch reseller name!"));
        }
        
        #check batch id is valid or not 
        if(preg_match("/[^0-9]+/",$param['batchId']) || $param['batchId'] == "")
            return json_encode (array("msg"=>"Invalid batch id","status"=>"error"));
        
        #get batch chain id from user id 
        $userBatch = $this->getbatchinternaldetail($param['batchId']);
        if($userBatch['chainId'] == 0 || $userBatch['chainId'] == '' || $userBatch['chainId'] == NULL){
            return json_encode(array("msg"=>"chain id not found","status"=>"error"));
        }
        $chainId = $userBatch['chainId'];
