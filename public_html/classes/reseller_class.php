...
//       if(preg_match("/[^a-zA-Z0-9\@\$\%\^\&\*\(\)\:\<\>\?\\\/\,\.\_\-\|]+/",$param['fundDescription']))
    ...
    function getBatchDetail($batchId, $pageNo = 1, $isadmin = 0, $export = 0) {
        ...
        if ($result->num_rows > 0) {
            ...
            $userBatchData['listenTime'] = $row['listenTime'];
        }
        ...
        if ($result->num_rows > 0) {
            while ($data = $result->fetch_array(MYSQL_ASSOC)) {
                ...
                $this->db->select('callingStatus,userPin,sipFlag,beforeLoginFlag,type,deleteFlag,isBlocked,AES_DECRYPT(password,"' . ENCRYPT_KEY . '") as password,userName,userId')->from($table)->where($condition);
                $qur = $this->db->getQuery();
...
//        echo "verifiedNumber = '" . $parm['contactNumber'] . "' and countryCode = '" . $parm['contactNo_code'] . "'";
        $this->db->select('*')->from($table)->where("verifiedNumber = '" . $parm['contactNumber'] . "' and countryCode = '" . $parm['contactNo_code'] . "' and 	resellerId='".$resellerId."'");
        $qur = $this->db->getQuery();
        $result = $this->db->execute();
        ...
        $data = array("name" => $name, "date" => date('Y-m-d H:i:s'),"menuIndex"=>'1,3,4,5,6,8,12');
        ...
        $result = $this->insertWithEncryption($loginTable, $data);
//        $qur = $this->db->getQuery();
//        $result = $this->db->execute();
        //var_dump($result);
        #check data inserted or not 
...
//        }
        ...
        if ($result->num_rows > 0) {
            while ($row = $result->fetch_array(MYSQL_ASSOC)) {
                ...
                if ($row['acmId'] == 0) {
                    ...
                    $acmNames = $acmObj->getAcmName($row['acmId']);
                }


                $data['acmName'] = $acmNames['fullName'];
    ...
    function searchBulkClient($parm) {
        ...
        $this->db->select('SQL_CALC_FOUND_ROWS * , AES_DECRYPT(password,"' . ENCRYPT_KEY . '") as passwordDec')->from($table)->where($condition)->limit($limit)->offset($skip);
        $qur = $this->db->getQuery();
        $result = $this->db->execute();

        //$this->sendErrorMail("nidhi@walkover.in", "VerifyOldEmail :: ".$qur); 
    ...
    function getBatchClientsInArry($result) {
        ...
        if ($result->num_rows > 0) {
            while ($row = $result->fetch_array(MYSQLI_ASSOC)) {
                ...
                $userdata['password'] = $row['passwordDec'];
    ...
    function resetClientPassword($request, $session) {
        ...
        $table = '91_userLogin';
        $this->db->select('AES_DECRYPT(password,"' . ENCRYPT_KEY . '") as password')->from($table)->where("userId = " . $request['clientId'] . "");
        ...
        //$this->db->update($table, $data)->where($condition);
        $result = $this->updateWithEncryption($table, $data, $condition);
        // $sql = $this->db->getQuery();
        // $result = $this->db->execute();
        #add password log 

        if ($oldPassword != $newPass) {
            $this->accountManagerLog($request['clientId'], 8, '-', '-', $session['id'], "change password");
        }
...
/**
 * @author ANkit Patidar <ankitpatidar@hostnsoft.com>
 * @uses function used for advance search
 * @param type $condition
 * @param type $start_limit
 * @param type $limit
 * @param type $sort
 * @return int
 */
    function allUserDetailNew($condition = null, $start_limit = 0, $limit = 10, $sort = 0) {

        if ($sort == 1)
            $order = "lower(u.userName)";
        else
            $order = "u.userId desc";
...
//        $this->db->select('SQL_CALC_FOUND_ROWS *')->from($table)->where('type != 4 and type !=7 and type !=1 ' . $allcondition . ' order by ' . $order . ' limit ' . $start_limit . ',' . $limit);
	...
	if($this->limit)
	{
	    $limitQur.="limit $start_limit,$limit";
	}
	
	$sql="SELECT SQL_CALC_FOUND_ROWS DISTINCT u.userId,u.userName, u.isBlocked, u.deleteFlag, u.type, u.beforeLoginFlag, u.sipFlag, u.userPin, u.callingStatus, u.password,ub.tariffId,ub.balance,ub.resellerId,pi.name,pln.planName,ub.currencyId $this->columns FROM `91_userLogin` u JOIN `91_userBalance` ub ON u.userId=ub.userId JOIN `91_personalInfo` pi ON ub.userId = pi.userId JOIN `91_plan` pln ON ub.tariffId=pln.tariffId $this->joinQur WHERE  u.type NOT IN (4,7,1) $condition order by u.userId desc $limitQur";
//        $qur = $this->db->getQuery();
//        $result = $this->db->execute();
	trigger_error('RENDERQUERY FOR SHOW All USERS:'.$sql);
        $result = $this->db->query($sql);

        //log error
        if (!$result)
            trigger_error('problem while get client details,query:' . $qur);
     ...
     * @param type $limit
     * @param type $sort
     * @param type $checkCondition
     * @uses added for advance search
     * @return type
     */
    function loadUsersNew($q, $user_id, $start_limit, $limit, $sort, $checkCondition = null)
    {
	$condition = '';
        $qryPart = '';
        if ($sort == 0)
        ...
        if ($checkCondition == null) {
            if (isset($q) && strlen($q) > 1)
                $condition = " and lower(u.userName) like '" . $q . "%' ";
            $qryPart.='SQL_CALC_FOUND_ROWS';
        }
        else {
	    $qryPart.='SQL_CALC_FOUND_ROWS';
	...
	//JOIN 91_calls cl ON pi.userId=cl.id_client and cl.status != 'ANSWERED'
        //$sql = "Select $qryPart * from 91_manageClient where resellerId='" . $user_id . "' and type != 4 and type !=7 and type!= 1 $condition order by " . $order . " limit " . $start_limit . "," . $limit;
	//$sql="SELECT DISTINCT u.userId,u.userName, u.isBlocked, u.deleteFlag, u.type, u.beforeLoginFlag, u.sipFlag, u.userPin, u.callingStatus, u.password,ub.tariffId,ub.balance,ub.resellerId,pi.name FROM `91_userLogin` u JOIN `91_userBalance` ub ON u.userId=ub.userId and u.type NOT IN (4,7,1) JOIN `91_personalInfo` pi ON ub.userId = pi.userId ".$condition;
	//(ub.tariffId=9 and ub.balance =0.73) or (ub.tariffId=7 and ub.balance =12) or (ub.tariffId=84 and ub.balance =0.2) or (ub.tariffId=8 and ub.balance =0.1)
	$sql="SELECT $qryPart DISTINCT u.userId,u.userName, u.isBlocked, u.deleteFlag, u.type, u.beforeLoginFlag, u.sipFlag, u.userPin, u.callingStatus, u.password,ub.tariffId,ub.balance,ub.resellerId,pi.name,pln.planName,ub.currencyId $this->columns FROM `91_userLogin` u JOIN `91_userBalance` ub ON u.userId=ub.userId JOIN `91_personalInfo` pi ON ub.userId = pi.userId JOIN `91_plan` pln ON ub.tariffId=pln.tariffId $this->joinQur WHERE ub.resellerId='" . $user_id . "' and u.type NOT IN (4,7,1) $condition order by u.userId desc $limitQur";
...
//        print_r($emailArray['email']);
     ...
     * @return json
     */
    function getFilteredClients($request, $session, $allclient = 0) {

        $this->status = 'error';
        $q = $request['q'];
        if (preg_match('/[^a-zA-Z0-9\@\_\-\$\.\s]+/', $q)) {
            $this->msg = 'Error Invalid data please provide a valid data';
            return 0;
        }
        #user id 
        $userid = $session["userid"];
        ...
        if (isset($q) and trim($q) != '') {
            ...
            if (is_numeric($q)) {
                $useridArray = $this->searchByNumber($q);
                $condition = implode(",", $useridArray);
                $newcondition = " and u.userId IN (" . $condition . ")";
            } elseif (strpos($q, '@')) {
                $useridArray = $this->searchByEmail($q);
                $condition = implode(",", $useridArray);
                $newcondition = " and u.userId IN (" . $condition . ")";
            } else {
                $newcondition = "and lower(u.userName) like '" . $q . "%'";
            }
            ...
            $conditionStatus = $this->getRenderedCondition($request);
            if (!$conditionStatus)
                $filterCond = $newcondition;
            else
                $filterCond = $newcondition . ' and ' . $this->data;
...
//              $allUserCondition ="and lower(userName) like '%".$q."%'";
                ...
                $result = $this->allUserDetailNew($filterCond, $first_limit, $limit, 1);
            } else
                $result = $this->loadUsersNew($q, $userid, $first_limit, $limit, 0, $filterCond);
            ...
            if (!$conditionStatus)
                ...
                $filterCond = ' and ' . $this->data;


            ############################################
            #condition to fetch all client detail 
            if ($allclient != 0) {
                $result = $this->allUserDetailNew($filterCond, $first_limit, $limit);
                if (isset($q)) {
                    $jade["isSearchResult"] = "true";
                }
            } else {
                ...
                $result = $this->loadUsersNew(0, $userid, $first_limit, $limit, 3, $filterCond);
            }
        }
...
//            #find user status block or unblock
     ...
     * @author Ankit Patidar <ankitpatidar@hostnsoft.com>
     * @since 25/12/2014
     * @abstract from currenct class
     * @uses to get active currencies in array
     * @return array
     */
    function getActiveCurrencies() {
        $result = $this->selectData('currencyId,currency', '91_currencyDesc', 'status=1');

        if (!$result || $result->num_rows == 0) {
            $this->msg = 'currency not found.';
            return 0;
        }

        $currArr = array();
        while ($row = $result->fetch_array(MYSQLI_ASSOC)) {
            $currArr[$row['currencyId']] = $row['currency'];
        }
     ...
     * @author Ankit Patidar <ankitpatidar@hostnsoft.com>
     * @param type $amt
     * @abstract called in current class
     * @since 25/12/2014
     * @return int
     */
    function getCurrCondition($amt) {
        error_reporting(-1);
        $currArrStatus = $this->getActiveCurrencies();
        ...
        $activePlanStatus = $this->getActivePlans();

        if (!$activePlanStatus)
            return 0;

        $activePlans = $this->data;

        //$tariffCurrArr = array();
        $tariffAmtArr = array();

        if (!empty($currArr))
            foreach ($currArr as $currId => $currency) {
                //$tariffCurrArr[$activePlans[$currId]]= $currency;
                if (isset($activePlans[$currId]) && $currency != 'USD')
                    $tariffAmtArr[$activePlans[$currId]] = $this->currencyConvert("USD", $currency, $amt);
                else if (isset($activePlans[$currId]) && $currency == 'USD') {
                    $tariffAmtArr[$activePlans[$currId]] = $amt;
                }
            }


        if (empty($tariffAmtArr)) {
            $this->msg = 'problem while get tariff.';
            return 0;
        }

        $condStr = '(';
        foreach ($tariffAmtArr as $key => $value) {
            $condStr.=' (ub.tariffId=' . $key . ' and ub.balance <' . $value . ') or ';
        }

        if ($condStr == '') {
            $this->msg = 'Info not found for the condition';
            return 0;
        }



        $condStr = substr($condStr, 0, -3);
	$condStr.=')';
        $this->msg = 'condition made successfully.';
        $this->data = $condStr;
    ...
    function getActivePlans() {
        $result = $this->selectData('tariffId,outputCurrency', '91_plan', "status='1'");
        ...
        $tariffArr = array();
        while ($row = $result->fetch_array(MYSQLI_ASSOC)) {
            $tariffArr[$row['outputCurrency']] = (int) $row['tariffId'];
        }
     ...
     * @author ANkit Patidar <ankitpatidar@hostnsoft.com>
     * @since 27/12/2014
     * @abstract called from same place
     * @uses function use to generate dynamic query
     * @return boolean
     */
    function getRenderedCondition($request) {
        $conditionArr = array();
        $lessThanCondRes = '';
        // for less then flag
        if (isset($request['lessThan']) && $request['lessThan'] == 'on') {

            if (isset($request['amtToCheck']) && !preg_match(DECIMAL_REGX, $request['amtToCheck'])) {
                $this->msg = 'Invalid amount Please enter valid amount.';
                return 0;
            }

            //amount in USD
            $amtToCheck = trim($request['amtToCheck']);

            //call function to get 
            $lessThanCondResStatus = $this->getCurrCondition($amtToCheck);

            $conditionArr[] = $this->data;
//		echo $this->data;
//		echo '<br>';
        }
        ...
        $signupBalanceQur = '';
        //signupBalance flag
        if (isset($request['signupBal']) && $request['signupBal'] == 'on') {
	    
	    //get signup amount
	    $signUpBalStatus= $this->getSignUpAmountForReseller();
	    if(!$signUpBalStatus)
		return 0;
	    $signBalArr = $this->data;
	    
	    $condStr = '(';
	    foreach ($signBalArr as $key => $value) {
		$condStr.=' (ub.tariffId=' . $key . ' and ub.balance =' . $value . ') or ';
	    }
	    ...
	    $signupBalanceQur = substr($condStr, 0, -3);

	    $signupBalanceQur.=')';
            //$signupBalanceQur = ' userId NOT IN (SELECT id_client from 91_calls WHERE status = "ANSWERED") ';
            $conditionArr[] = $signupBalanceQur;
//		 die('dfd');
        }

        if (isset($request['reNotUsingCall']) && $request['reNotUsingCall'] == 'on') {
            //get minimum recharge amount and currency Id
            $rechargeAmtStatus = $this->getMinimumRechargeAmt();

            if (!$rechargeAmtStatus) {
                return 0;
            }

            $rechargeAmtArr = $this->data;

            $rechargedButNotUsing = '(';
            if (is_array($rechargeAmtArr)) {
                foreach ($rechargeAmtArr as $currId => $minBalance) {
                    $rechargedButNotUsing.=' (currencyId =' . $currId . ' and balance >' . $minBalance . ') or ';
                }
            }

            $rechargedButNotUsing = substr($rechargedButNotUsing, 0, -3);
	    $rechargedButNotUsing.=')';
            $this->joinQur.=" JOIN 91_calls cl ON pi.userId=cl.id_client ";

	    $rechargedButNotUsing.="and cl.status != 'ANSWERED'";
            $conditionArr[] = $rechargedButNotUsing;
        }

        $notUseSince = '';
        if (isset($request['notUseSince']) && $request['notUseSince'] == 'on') {
            //get minimum recharge amount and currency Id
            if (isset($request['dayToCheck']) && preg_match(NOTNUM_REGX, $request['dayToCheck'])) {
                $this->msg = 'Invalid day entered.';
                return 0;
            }

            $days = $request['dayToCheck'];

            $oldDate = date('Y-m-d H:i:s', strtotime('- ' . $days . ' days'));

	    //"JOIN 91_calls cl ON pi.userId=cl.id_client and call_start > '" . $oldDate . "' and cl.status != 'ANSWERED'"
            //$notUseSince = " userId NOT IN (SELECT id_client from 91_calls WHERE call_start > '" . $oldDate . "' and status='ANSWERED') ";
	     $this->joinQur.= " JOIN 91_calls cl ON pi.userId=cl.id_client";
	     $notUseSince = "cl.call_start > '" . $oldDate . "' and cl.status != 'ANSWERED'";
            $conditionArr[] = $notUseSince;
        }
        ...
        if (isset($request['windowsU']) && $request['windowsU'] == 'on') {

            $platFormCondition[] = 2;
            $platFormCondition[] = 21;
        }

        if (isset($request['isoU']) && $request['isoU'] == 'on') {

            $platFormCondition[] = 1;
            $platFormCondition[] = 11;
        }

        if (isset($request['androidU']) && $request['androidU'] == 'on') {

            $platFormCondition[] = 3;
            $platFormCondition[] = 31;
        }

        if (!empty($platFormCondition)) {

            $conditionArr[] = ' u.userName IN (SELECT userName from 91_pushNotification WHERE appId IN (' . implode(',', $platFormCondition) . '))';
        }
         ...
         * 
         */
        if (isset($request['selectCountry']) && $request['selectCountry'] == 'on') {
            if (isset($request['code']) && preg_match(NOTNUM_REGX, $request['code'])) {
                $this->msg = 'Invalid country selected.';
                return 0;
            }

            $code = $request['code'];
            $countryCond = "u.userId IN (SELECT userId from 91_verifiedNumbers WHERE countryCode=$code)";
            $conditionArr[] = $countryCond;
        }
         ...
         * 
         */
        if (isset($request['selectLimitedPlan']) && $request['selectLimitedPlan'] == 'on') {
            if (isset($request['selectPlan']) && preg_match(NOTNUM_REGX, $request['selectPlan'])) {
                $this->msg = 'Invalid plan selected.';
                return 0;
            }

            $plan = $request['selectPlan'];
            $planCond = "u.userId IN (SELECT userId from 91_limitedUserBalance WHERE planId=$plan)";
            $conditionArr[] = $planCond;
        }
        ...
        $queryCond = substr($queryCond, 0, -4);

        if ($queryCond == '') {
            $this->msg = 'No criteria selected to filter.';
            return 0;
        }
     ...
     * @return int
     */
    function getMinimumRechargeAmt() {
        $this->status = 'error';
        //get range
        $result = $this->selectData('currencyId,minimum', '91_rechargeAmt');

        if (!$result || $result->num_rows == 0) {
            $this->msg = "problem while get record.";
            return 0;
        }

        $amtArr = array();
        while ($row = $result->fetch_array(MYSQLI_ASSOC)) {
            $amtArr[$row['currencyId']] = (int) $row['minimum'];
        }

        if (empty($amtArr)) {
            $this->msg = 'Recharged amount not found.';
            return 0;
        }
    ...
    function getSignUpAmountForReseller()
    {
	 $this->status = 'error';
	  $result = $this->selectData('tariffId,balance', '91_resellerDefaultCurrency','resellerId=2');
        ...
        $amtArr = array();
        while ($row = $result->fetch_array(MYSQLI_ASSOC)) {
	    $amtArr[$row['tariffId']] = (float) $row['balance'];
        }
     ...
     * @param array $request
     * @param array $session
     * @abstract called from adminManageClientCnt 
     * @return boolean
     */
    function sendPushOrSMS($request, $session) {

        $notCount = 0;
        if (isset($request['sendType']) && $request['sendType'] == 0) {

            $numberArr91 = array();

            //send msg
            //print_r($request['numbers']);
            if (isset($request['numbers']) && is_array($request['numbers']))
                foreach ($request['numbers'] as $number) {
                    $codeNum = explode('-', $number);
                    $code = $codeNum[0];
                    $num = $codeNum[1];

                    if ($code == "91") {
                        $numberArr91[] = $code . $num;
                    } else {

                        $tempparam['to'] = $code . $num;
                        //$tempparam["password"] = "AWIKSeLIcPFHBS";
                        //$tempparam["apiId"] = "3468158";

                        $tempparam['text'] = $request['textCon'];
                        ...
                        $smsObj->sendMessagesGlobal($tempparam);
                        
                        //$statusRes = $this->SendSMSUSDnew($tempparam);
			$notCount++;
                    }
                }

            if (!empty($numberArr91)) {
                $nine['sender'] = "Utteru";
                $nine['mobiles'] = implode(',', $numberArr91); // mobile number without 91
                $nine['message'] = $request['textCon'];
                $sendRes = $this->SendSMS91($nine);
		$notCount++;
            }


            $this->msg = 'Message has been sent succeefully.';
        } else if (isset($request['sendType']) && $request['sendType'] == 1) {

            if (isset($request['numbers']) && is_array($request['numbers']))
                foreach ($request['numbers'] as $value) {
                
                if($request['contentType'] == 2){
                    $postArr = array('username' => $request['numbers'][0],
                        'data' => "change Access Number",
                        // 'dataType' => 'hgjjhg',
                        'datamsg' => '{"contentType":"2"}');
                }else if($request['contentType'] == 3){
                    
                    
                    if (preg_match('/[^0-9\.]+/', $request['ipdata'])) {
                         $this->msg = 'Ip detail not valid.';
                         $this->status = 'error';
                         return 0;
                    }
                    
                    if (preg_match('/[^0-9]+/', $request['port'])) {
                         $this->msg = 'Port detail not valid.';
                         $this->status = 'error';
                         return 0;
                    }
                    ...
                    $postArr = array('username' => $request['numbers'][0],
                        'data' => "change Ip",
                        // 'dataType' => 'hgjjhg',
                        'datamsg' => '{"contentType":"3","content":{"'.$request['parseUdp'].'":["'.$request['ipdata'].'","'.$request['port'].'"]}}');
                }else{
                    $postArr = array('username' => $request['numbers'][0],
                        'data' => $request['textCon'],
                        // 'dataType' => 'hgjjhg',
                        'datamsg' => '{"contentType":"0"}');
                }    
                    
                    //send push notification
		    $url = 'https://voice.phone91.com/newapi/sendnotification.php?username=' . $postArr['username'] . '&data=' . urlencode($postArr['data']) . '&datamsg=' . $postArr['datamsg'];
		    trigger_error($url);
                    $response = $this->getResponseByCurl($url);
            ...
            $this->msg = 'Push notification has been sent successfully.';
        }
     ...
     * @param type $request
     * @param type $session
     * @param type $allclient
     * @return int
     */
    function getDetailsForExport($request, $session, $allclient = 0)
    {
	$this->status = 'error';
	$this->limit = false;
        $q = $request['q'];
	...
	$this->columns = ',vn.CountryCode,vn.VerifiedNumber,ve.email';
	$this->joinQur.=' LEFT JOIN `91_verifiedNumbers` vn ON u.userId=vn.userId LEFT JOIN `91_verifiedEmails` ve ON u.userId=ve.userId';
	# search by email and contact no if $q value is given from request 
        if (isset($q) and trim($q) != '') {
            $q = strtolower($q);
...
//              $allUserCondition ="and lower(userName) like '%".$q."%'";
	    ...
	    if($row['isBlocked'] == 1 && $row['VerifiedNumber'] != '' && $row['VerifiedNumber'] != null)
	    {
		...
		$data['name'] = $row['name'];
		$data["planName"] = $planName;
		$data["contact_no"] = $row['CountryCode'].'-'.$row['VerifiedNumber'];
...
//		$data["id_currency_name"] = $currency;
	...
	$fieldType = array('A' => 'id' , 
	 ...
	 $fileName = $this->exportRecords($jade["client"] ,'xlsx' ,'Client List' ,$fieldType);
	...
	{
	    $this->data = '';
	    $this->msg= 'Problem while export client List.';
	    return 0; 
	}
