...
//            }
	...
	$sql="SELECT DISTINCT u.userId,u.userName, u.isBlocked, u.deleteFlag, u.type, u.beforeLoginFlag, u.sipFlag, u.userPin, u.callingStatus, u.password,ub.tariffId,ub.balance,ub.resellerId,pi.name FROM `91_userLogin` u JOIN `91_userBalance` ub ON u.userId=ub.userId JOIN `91_personalInfo` pi ON ub.userId = pi.userId $this->joinQur WHERE  u.type NOT IN (4,7,1) $condition order by ub.balance desc limit $start_limit,$limit";
...
//              $allUserCondition ="and lower(userName) like '%".$q."%'";
                ...
                $result = $this->allUserDetail($filterCond, $first_limit, $limit, 1);
            } else
            ...
            #condition to fetch all client detail 
            if ($allclient != 0) {
                $result = $this->allUserDetail($filterCond, $first_limit, $limit);
                if (isset($q)) {
                    $jade["isSearchResult"] = "true";
                }
            } else {
...
//		 die('dfd');
	    ...
	    $rechargedButNotUsing.="cl.status != 'ANSWERED'";
	    ...
	    //"JOIN 91_calls cl ON pi.userId=cl.id_client and call_start > '" . $oldDate . "' and cl.status != 'ANSWERED'"
	     ...
	     $notUseSince = "JOIN 91_calls cl ON pi.userId=cl.id_client and cl.call_start > '" . $oldDate . "' and cl.status != 'ANSWERED'";
        ...
        if (!empty($platFormCondition)) {
            ...
            $conditionArr[] = ' pi.userName IN (SELECT userName from 91_pushNotification WHERE appId IN (' . implode(',', $platFormCondition) . '))';
        }
        ...
        if (isset($request['selectCountry']) && $request['selectCountry'] == 'on') {
            ...
            $code = $request['code'];
            $countryCond = "pi.userId IN (SELECT userId from 91_verifiedNumbers WHERE countryCode=$code)";
