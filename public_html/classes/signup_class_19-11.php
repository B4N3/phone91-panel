...
class signup_class extends fun //validation_class.php
{
   ...
   ++$last;
    ...
    function createUser($parm,$tariff_id,$balance,$currency_id)
    {
      ...
      $data=array("name"=>$parm['username'],"date"=>date('Y-m-d H:i:s')); 
...
//        $parm['email'] = trim($parm['email']);
        ...
        #check country code is numeric or not 
        if (!preg_match("/^[0-9]+$/", $parm['code'])) {
            return json_encode(array("status" => "error", "msg" => "country code not valid ! "));
        }

        #check currency id is numeric or not 
        if (!preg_match("/^[0-9]+$/",$parm['currency'])) {
            return json_encode(array("status" => "error", "msg" => "currency are not valid ! "));
        }
      
        #check user type  1 for admin ,2 for reseller , 3 for user 
        if (!preg_match("/^[0-9]+$/",$parm['client_type'])) {
            return json_encode(array("status" => "error", "msg" => "client type are not valid ! "));
        }
