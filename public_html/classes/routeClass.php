...
class routeClass extends fun {
    ...
    public $routeId;
    public $userId;
    var $defaultCodecs = array('ulaw', 'alaw', 'G.722', 'G.723.1', 'G.726', 'G.729', 'gsm', 'iLBC', 'LPC10', 'Speex', 'g729');
    ...
    function validateRouteParam($request) {
        ...
        if (preg_match('/[^0-9]+/', $request['routePort']) || strlen($request['routePort']) > 5) {
            $this->msg = "Error Invalid Route Port,please enter a port less than 6 digits!";
            return 0;
        }
...
//        }
        ...
        $counts = count($request['codexNames']);


        if ($counts > 0)
            for ($i = 0; $i < $counts; $i++) {

                if (!in_array($request['codexNames'][$i], $this->defaultCodecs)) {
                    $this->msg = "Error Invalid codec found!";
                    return 0;
                }
            }
        ...
        if (preg_match('/[^0-9]+/', $request['routeCallLimit']) || $request['routeCallLimit'] < 1 || $request['routeCallLimit'] > 50) {
    ...
    function addRoute($request) {
        ...
        if (!empty($request['routePrefix']))
            $routePrefix = $this->db->real_escape_string(trim($request['routePrefix']));
        else
            $routePrefix = '';

        $routeTariff = $this->db->real_escape_string(trim($request['tariff']));
        $routeIp = $this->db->real_escape_string(trim($request['routeIps']));
        ...
        $callLimit = $this->db->real_escape_string(trim($request['routeCallLimit']));
        $port = $this->db->real_escape_string(trim($request['routePort']));

        $sipData = array("name" => $routeName,
            'type' => 'peer',
            '`call-limit`' => (int) $callLimit,
            'context' => 'internatinal',
            'port' => $port,
            'host' => $routeIp,
            'allow' => implode(';', $request['codexNames'])
        );


        $sipTable = 'sip_buddies';
        #insert query (insert data into 91_sip_buddies table )
        $sipResult = $this->insertData($sipData, $sipTable);

        if (!$sipResult) {
            trigger_error('problem while route entery in sip buddies,qur:' . $this->querry);
            $this->deleteData($routeTable, 'routeId=' . $routeId);
            return json_encode(array("status" => "error", "msg" => "Problem while route add!"));
        }
...
//      }
    ...
    function editRouteInfo($request) {
        ...
        if (empty($request['sipBuddyId']) || !is_numeric($request['sipBuddyId']))
            return json_encode(array("msg" => 'Invalid Parameter passed Please try again!', "status" => "error"));
...
//        }
        ...
        $routePort = $this->db->real_escape_string(trim($request['routePort']));
        $routeCallLimit = $this->db->real_escape_string(trim($request['routeCallLimit']));
        $updateSipData['port'] = $routePort;
        $updateSipData['host'] = $routeIp;
        $updateSipData['name'] = $routeName;

        $updateSipData['`call-limit`'] = $routeCallLimit;
        if (count($request['codexNames']))
            $updateSipData['allow'] = implode(';', $request['codexNames']);

        $sipBuddyId = trim($request['sipBuddyId']);

        $updateRes = $this->updateData($updateSipData, 'sip_buddies', 'id=' . $sipBuddyId);
    ...
    function addRouteTransactional_sub($amount, $currentBalance, $paymentType, $debit, $credit, $closingBalance, $description, $currency = 1) {
        ...
        $data = array("fromUser" => $this->userId,
            ...
            "debitConvert" => (float) $debitCntAmt,
            "creditConvert" => (float) $creditCntAmt,
            "currency" => $currency
        );
    ...
    function routeClosingBalCurrencyCnvt($routeId, $currency, $amount) {
        ...
        if ($currency != $routeDetail['currencyId']) {
            ...
            $debitCnt = $this->currencyConvert($paymentCurrency, $userCurrency, $amount);
            $debitCntAmt = (float) $debitCnt;
        } else {
            $debitCntAmt = $amount;
        }
    ...
    function getRoute($request, $userId) {
        ...
        if (preg_match(NOTALPHANUM_REGX, $request['q']))
            return false;
        ...
        //join query to get diverted route id with result
        $query = "SELECT r1.routeId, r1.route, r1.tariffId, r1.routeCredits, r2.divertedRouteId
                    FROM 91_route r1
                    INNER JOIN 91_divertedRoute r2 ON r1.routeId = r2.routeId
                    WHERE " . $condition;

        trigger_error("getRoute" . $query);

        $selRes = $this->db->query($query);
        ...
        if (empty($routeList)) {
            trigger_error('problem while get route list');
            return json_encode($detailArr);
        }



        //get route closing balance
        $routeClosing = $this->getRouteElement('closingAmt', '91_routeClosingAmt', '1');


//        if(empty($routeClosing))
//        {
//            trigger_error('problem while get route closing detail!');
//            return json_encode($detailArr);
//        }
        ...
        while ($row = $selRes->fetch_array(MYSQLI_ASSOC)) {
            ...
            if (isset($routeClosing[$row['routeId']]))
                $data['routeClosingAmt'] = (float) $routeClosing[$row['routeId']];
            else
                $data['routeClosingAmt'] = 0;
...
//                return false;
            ...
            //get diverted route
            $data['divertedRoute'] = $routeList[$row['divertedRouteId']];
            $detailArr[] = $data;
...
//            unset($row);
    ...
    function getRouteProfit($request, $userId) {
        if (empty($userId) || preg_match(NOTNUM_REGX, $userId))
            return false;
        ...
        if ($request['fromDate'] == '' || $request['toDate'] == '') {
            ...
            $preDate = date('Y-m-d H:i:s', strtotime('-2 day', $now));
            $startDate = $preDate;
            $toDate = $nowDate;
        } else {
        ...
        //join query to get diverted route id with result
        $query = "SELECT r.routeId, sum(r.profit) as profit, p.route, r.currencyDesc from 91_routeProfit r, 91_route p where p.routeId = r.routeId and r.date between '$startDate' and '$toDate' group by r.routeId ";
...
//echo $query;
        ...
        while ($row = $selRes->fetch_array(MYSQLI_ASSOC)) {
            ...
            $data['currency'] = $row["currencyDesc"];

            if ("USD" != $data['currency']) {
                $data['profit'] = $this->currencyConvert($data['currency'], "USD", $data['profit']);
                $data['currency'] = "USD";
            }
            $detailArr[] = $data;
        }
    ...
    function getRouteDetail($request, $userId) {
        ...
        //get details from sip_buddies
        $sipCondition = "name='" . $row['route'] . "'";

        $sipRes = $this->selectData("*", "sip_buddies", $sipCondition);

        if (!$sipRes || $sipRes->num_rows < 1) {
            return json_encode(array());
        }

        $sipRow = $sipRes->fetch_array(MYSQLI_ASSOC);

        $data['port'] = $sipRow['port'];
        $data['routeCallLimit'] = $sipRow['call-limit'];
        $data['allowCodecs'] = explode(';', $sipRow['allow']);
        $data['sipBuddyId'] = $sipRow['id'];
...
//      #add transaction in case of voip91(payment type advance).
    ...
    function getRouteTransaction($routeId, $userId, $pageNo = 1) {
        if (empty($routeId) || preg_match(NOTNUM_REGX, $routeId) || empty($userId) || preg_match(NOTNUM_REGX, $userId) || preg_match(NOTNUM_REGX, $pageNo))
            return json_encode(array());
...
//                  case '0' : 
                ...
                $totalClosingAmt = $row['closingBalance'];
                ...
                if ($currencyName == $data['currencyName']) {
                    ...
                    $totalCredit+=$data['creditActualCurrency'];
                    $totalDebit+=$data['debitActualCurrency'];
                }
        ...
        $transactionData['totalDebit'] = $totalDebit;

        $transactionData['totalCredit'] = $totalCredit;

        $transactionData['totalClosingAmt'] = $totalClosingAmt;


        return json_encode($transactionData);
    }

    function getAllRouteTransaction($userId, $pageNo = 1) {
        if (empty($userId) || preg_match(NOTNUM_REGX, $userId))
            return json_encode(array());
        ...
        $condition = "fromUser=$this->userId order by date desc";
        ...
        if (empty($routeList)) {
            return json_encode(array('status' => 'error', 'msg' => 'Route not found!', 'detail' => array()));
        }
...
//                  case '0' : 
                ...
                $data['routeName'] = $routeList[$row['routeId']];
...
//              else
        ...
        if (empty($transactionData))
            return json_encode(array('status' => 'error', 'msg' => 'problem while get transactional details!', 'detail' => $transactionData));
        else
            return json_encode(array('status' => 'success', 'msg' => 'Transactions successfully found!', 'detail' => $transactionData));
    }
...
//            
     ...
     * 
     * @author Ankit Patidar <ankitpatidar@hostnsoft.com>
     * @uses function to get route element with array 
     * @since 3/7/2014
     * @abstract calling places from current class functions
     * @return array
     */
    function getRouteElement($field = '', $table = '91_route', $condition = '1') {
        $selRes = $this->selectData('routeId,' . $field, $table, $condition);
        ...
        $data = array();
        while ($row = $selRes->fetch_array(MYSQLI_ASSOC)) {
            $data[$row['routeId']] = $row[$field];
        }

        return $data;
    }

    function getlimitedPlanList($param, $userId, $userType) {

        $limitedPlanData = array();
        if ($userType != 1) {
            return json_encode(array("msg" => "you have no permission to get limited plan list", "status" => "error"));
        }
        if (preg_match('/[^0-9]+/', $userId) || $userId == "")
            return json_encode(array("msg" => "Error Invalid user please login again", "status" => "error"));
        ...
        $selRes = $this->selectData("*", "91_limitedPlan");
        if ($selRes) {
            while ($row = $selRes->fetch_array(MYSQLI_ASSOC)) {
                $limitedPlanData[$row['planId']] = $row['planName'];
            }
        }

        $userInfo = $this->getUserLimitedPlan($param['clientId']);


        $islimitedPlan = $userInfo['planId'];

        return json_encode(array("status" => "success", "limitedPlanData" => $limitedPlanData, "userPlan" => $islimitedPlan));
    }

    function setUserlimitedPlan($param, $userId, $userType) {
        ...
        if (preg_match('/[^0-9]+/', $param['clientId']) || $param['clientId'] == "")
            return json_encode(array("msg" => "you have no permission to update limited plan id ", "status" => "error"));
        ...
        if ($resellerId != $userId) {
            return json_encode(array("status" => "error", "msg" => "you have no permission to change limited plan."));
        }
        ...
        $planId = $param['limitedPlanList'];

        $planDetail = $this->getlimitedPlanDetail($planId);

        $remainingMinute = $planDetail['remainingMinute'];

        $tariffRate = $planDetail['tariffRate'];

        #convert tariff rate to user currency 
        $userDetail = $this->getNameAndUserName($param['clientId']);
        $userCurrency = $userDetail['currencyId'];
        $Currency = $this->getCurrencyViaApc($userCurrency, 1);
        if ("USD" != $Currency) {
            $remainingCredit = $this->currencyConvert("USD", $Currency, $tariffRate);
        } else {
            $remainingCredit = $tariffRate;
        }

        $limitedUserTable = '91_limitedUserBalance';
        #update limited user balance table 
        $limitedtable = $this->selectData("*", $limitedUserTable, "userId='" . $param['clientId'] . "'");

        $data = array("userId" => $param['clientId'],
            "chainId" => $chainId,
            "planId" => $planId,
            "startDate" => date('Y-m-d H:i:s'),
            "remainingMinutes" => $remainingMinute,
            "remainingRealCredits" => $remainingCredit
        );

        if (!$limitedtable || $limitedtable->num_rows < 1) {
            #insert new record
            $result = $this->insertData($data, $limitedUserTable);
        } else {
            #update user record
            $result = $this->updateData($data, $limitedUserTable, "userId='" . $param['clientId'] . "'");
        }
    ...
    function getlimitedPlanDetail($planId) {

        $Condition = "planId='" . $planId . "'";

        $data = array();
        $result = $this->selectData("*", "91_limitedPlan", $Condition);
        ...
        $data['remainingMinute'] = $res['minutes'];
        $data['tariffRate'] = $res['tariffRate'];
        $data['planName'] = $res['planName'];

        return $data;
    }

    function getUserLimitedPlan($userId) {

        $Condition = "userId='" . $userId . "'";

        $data = array();
        $result = $this->selectData("*", "91_limitedUserBalance", $Condition);
        ...
        $data['remainingMinutes'] = $res["remainingMinutes"];
        $data['remainingRealCredits'] = $res["remainingRealCredits"];
        return $data;
    }

    function getUserLimitedPlanDetail($param, $userId, $userType) {

        if ($userType != 1) {
            return json_encode(array("msg" => "you have no permission to get user limited plan list", "status" => "error"));
        }
        ...
        if (preg_match('/[^0-9]+/', $param['userId']) || $param['userId'] == "")
            return json_encode(array("msg" => "you have no permission to get user limited plan id ", "status" => "error"));


        $data = $this->getUserLimitedPlan($param['userId']);

        $planDetail = $this->getlimitedPlanDetail($data['planId']);

        if (count($planDetail) < 1 || count($data) < 1) {
            return json_encode(array("msg" => "Plan detail not found.", "status" => "error"));
        }

        $userDetail = $this->getNameAndUserName($param['userId']);
        $userCurrency = $userDetail['currencyId'];
        $Currency = $this->getCurrencyViaApc($userCurrency, 1);

        $planName = $planDetail['planName'];
        $remainingMinute = $data['remainingMinutes'];
        $remainingRealCredit = $data['remainingRealCredits'];


        return json_encode(array("msg" => "successfully record found.", "status" => "success", "planName" => $planName, "remainingMinute" => $remainingMinute, "remainingRealCredit" => $remainingRealCredit, "currency" => $Currency));
    }

    function getAllLimitedPlan($param, $userId, $userType) {

        if ($userType != 1) {
            return json_encode(array("msg" => "you have no permission to get all limited plan list", "status" => "error"));
        }
        ...
        if (strlen($param['data']) > 0) {
            if (!preg_match('/^[a-zA-Z0-9\@\_\-\s]+$/', $param['data'])) {
                return json_encode(array("msg" => "Record not found.", "status" => "error"));
            }
            $condition = "planName like '%" . $param['data'] . "%'";
        } else
            $condition = "1";

        $limitedPlan = array();
        $selRes = $this->selectData("*", "91_limitedPlan", $condition);

        if (!$selRes || $selRes->num_rows < 1) {
            return json_encode(array("msg" => "no record found.", "status" => "error"));
        }

        while ($row = $selRes->fetch_array(MYSQLI_ASSOC)) {
            $data['planId'] = $row['planId'];
            $data['planName'] = $row['planName'];
            $data['tariffRate'] = $row['tariffRate'];
            $data['minutes'] = $row['minutes'];
            $data['daysLimit'] = $row['daysLimit'];
            $data['hoursLimit'] = $row['hoursLimit'];
            $data['callsLimit'] = $row['callsLimit'];
            $limitedPlan[] = $data;
        }

        return json_encode(array("msg" => "successfully record found.", "status" => "success", "limitedPlan" => $limitedPlan));
    }

    function AddLimitedPlan($param, $userId, $userType) {

        if (!preg_match("/^[a-zA-Z0-9\s]+$/", $param['planName'])) {
            return json_encode(array("status" => "error", "msg" => "Plan name not valid! "));
        }

        if (!preg_match("/^[0-9]+(\.[0-9]{1,3})?$/", $param['tariffRate'])) {
            return json_encode(array("status" => "error", "msg" => "Please enter valid tariff rate!"));
        }

        if (preg_match("/[^0-9]+/", $param['minutes']) || strlen($param['minutes']) < 1) {
            return json_encode(array("status" => "error", "msg" => "Please enter valid minutes!"));
        }

        if (preg_match("/[^0-9]+/", $param['dayLimit']) || strlen($param['dayLimit']) < 1) {
            return json_encode(array("status" => "error", "msg" => "Please enter valid day limit!"));
        }

        if (preg_match("/[^0-9]+/", $param['hourLimit']) || strlen($param['hourLimit']) < 1) {
            return json_encode(array("status" => "error", "msg" => "Please enter valid hour limit!"));
        }
        if (preg_match("/[^0-9]+/", $param['callLimit']) || strlen($param['callLimit']) < 1) {
            return json_encode(array("status" => "error", "msg" => "Please enter valid call limit!"));
        }


        $Condition = "planName='" . $param['planName'] . "'";
        $planTable = "91_limitedPlan";
        $result = $this->selectData("*", $planTable, $Condition);

        if ($result->num_rows > 0) {
            return json_encode(array("status" => "error", "msg" => "Plan name already exist."));
        }

        $data = array("planName" => $param['planName'],
            "tariffRate" => $param['tariffRate'],
            "minutes" => $param['minutes'],
            "daysLimit" => $param['dayLimit'],
            "hoursLimit" => $param['hourLimit'],
            "callsLimit" => $param['callLimit']
        );
        $insResult = $this->insertData($data, $planTable);

        if (!$insResult) {
            return json_encode(array("msg" => "Plan data not updated.", "status" => "error"));
        }

        $plan = json_decode($this->getAllLimitedPlan($param, $userId, $userType), true);
        $limitedPlan = $plan['limitedPlan'];
        return json_encode(array("msg" => "Plan data successfully updated.", "status" => "success", "limitedPlan" => $limitedPlan));
    }

    function getLimitedPlanPrefix($param, $userId, $userType) {
        ...
        if (preg_match('/[^0-9]+/', $param['planId']) || $param['planId'] == "")
            return json_encode(array("msg" => "you have no permission to get limited plan Detail ", "status" => "error"));

        $planDetail = $this->getlimitedPlanDetail($param['planId']);
        $limitedPlan = array();
        $condition = "planId='" . $param['planId'] . "'";
        $selRes = $this->selectData("*", "91_limitedPlanPrefix", $condition);

        if (!$selRes || $selRes->num_rows < 1) {
            return json_encode(array("msg" => "no record found.", "status" => "error", "planName" => $planDetail['planName']));
        }

        while ($row = $selRes->fetch_array(MYSQLI_ASSOC)) {
            $data['countryPrefix'] = $row['countryPrefix'];
        ...
        return json_encode(array("msg" => "successfully record found.", "status" => "success", "limitedPlanPrefix" => $limitedPlan, "planName" => $planDetail['planName']));
    }

    function addPrefixAndCountry($param, $userId, $userType) {

        if ($userType != 1) {
            return json_encode(array("msg" => "you have no permission to add limited plan prefix", "status" => "error"));
        }
        ...
        if (preg_match('/[^0-9]+/', $param['planId']) || $param['planId'] == "")
            return json_encode(array("msg" => "you have no permission to add limited plan Detail ", "status" => "error"));

        if (!preg_match('/^[0-9]{1,18}$/', $param['prefix']))
            return json_encode(array("msg" => "please enter valid prefix.", "status" => "error"));

        if (!preg_match('/^[a-zA-Z0-9\s]+$/', $param['country']))
            return json_encode(array("msg" => "please enter valid country.", "status" => "error"));


        $limitedPlan = array();
        $condition = "planId='" . $param['planId'] . "' and countryPrefix='" . $param['prefix'] . "' and CountryName='" . $param['country'] . "'";
        $selRes = $this->selectData("*", "91_limitedPlanPrefix", $condition);

        if ($selRes->num_rows > 0) {
            return json_encode(array("msg" => "Prefix and country alrady added.", "status" => "error"));
        }


        #insert routedetail into database       
        $data = array("planId" => $param['planId'],
            "countryPrefix" => $param['prefix'],
            'CountryName' => $param['country']
        );

        $result = $this->insertData($data, "91_limitedPlanPrefix");


        if (!$result) {
            return json_encode(array("msg" => "Prefix and country not updated.", "status" => "error"));
        }

        return json_encode(array("status" => "success", "msg" => "Record updated successfully!"));
    }

    function getRoutePrefixProfit($request, $userId) {

        $now = time();

        if ($request['fromDate'] == '' || $request['toDate'] == '') {
            $nowDate = date('Y-m-d', $now);
            $preDate = date('Y-m-d', strtotime('-1 day', $now));
            $startDate = $preDate;
            $toDate = $nowDate;
        } else {
...
//		mail('vikas@walkover.in','date','startDate = '.$startDate.', toDate = '.$toDate);

		if($startDate == $toDate){
			$query = "SELECT q.route, p.prefix, sum(p.profit) as profit FROM 91_prefixRouteProfit p, 91_route q where p.date = '$startDate' and p.routeId = q.routeId group by q.route, p.prefix";
//			$query1 = "SELECT q.route, p.prefix, sum(p.profit) as profit FROM 91_prefixRouteProfit p, 91_route q where p.date = '$startDate' and p.routeId = q.routeId group by q.route, p.prefix order by p.profit asc limit 10";
		}else{
        	$query = "SELECT q.route, p.prefix, sum(p.profit) as profit FROM 91_prefixRouteProfit p, 91_route q where p.date between '$startDate' and '$toDate' and p.routeId = q.routeId group by q.route, p.prefix";
//			$query1 = "SELECT q.route, p.prefix, sum(p.profit) as profit FROM 91_prefixRouteProfit p, 91_route q where p.date between '$startDate' and '$toDate' and p.routeId = q.routeId group by q.route, p.prefix order by p.profit asc limit 10";
		}
