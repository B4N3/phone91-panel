...
//                        return json_encode(array("msg"=>"Error Invalid Password","status"=>"error"));
            ...
            else if(isset($systemType) && $systemType == "1")
            {
                ...
                if(preg_match("/[^a-zA-Z0-9\.\@\$]+/", $messengerId) || $messengerId == "")
                {
                    return json_encode(array("msg"=>"Error Invalid messenger Id","status"=>"error"));
                }

                if(!$this->checkMessengerId($messengerId))
                {
                    return json_encode(array("msg"=>"Messenger Id already Exists!","status"=>"error"));   
                }
           ...
           foreach($data as $key => $value)
           {
            ...
            if($res)
            {
                ...
                if($sip)
                {
                    ...
                    $condition = " on duplicate key update userName = '".$userName."',passwd =AES_ENCRYPT('".$password."','".ENCRYPT_KEY."')";
                    
                    $sql = "INSERT INTO ".$sipTable." (userId,userName,passwd,chainId,systemId,isCallShopUser) values('".$callShopId."','".$userName."',AES_ENCRYPT('".$password."','".ENCRYPT_KEY."'),'".$sipChainId."','".$systemId."',1) ".$condition;
                    ...
                    { 
//                        ob_start();
//                         if(isset($request['add']) && $request['add'] == 0)
...
//        $randomString = substr(str_shuffle("abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ"), 0, 1) . substr(str_shuffle("0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ"), 0, 10);
     ...
     * @return type
     */
    function getCallShopDetails($userId,$keyword = null,$tariffId)
    {
        $keyword = trim($keyword);
        if($userId == "" || !is_numeric($userId))
            return json_encode(array("msg"=>"Invalid User","status"=>"error"));
        if($tariffId == "" || !is_numeric($tariffId))
            return json_encode(array("msg"=>"Invalid User Information please try again","status"=>"error"));
        
        #initilize parameters 
        $table = "91_manageClient";
        $columns = " name,tariffId,currencyId,userId,planName,balance ";
        $condition  = " type = 7 and resellerId = ".$userId." and deleteFlag = 0  order by userId DESC";
...
function getSystemDetails($systemId,$resellerId,$callShopId)
{
    ...
    $result = $this->selectData("systemId,systemName,messengerId,userName,AES_DECRYPT(password,'".ENCRYPT_KEY."') as password,type,messengerType,callShopId,resellerId,date,isDeleted", "91_addCallShop",$condition);
    if($result)
    {
...
public function deleteCallShopSystem($callshopId,$resellerId,$type = 1,$systemId = NULL) {
 ...
 /*
    ...
    public function getCallShopRecord($param)
    {
	 ...
	 if(isset($param['shopId']) && preg_match(NOTNUM_REGX,$param['shopId']))
	    return $table.="<tr><td id = 'dummyTr' colspan='4'>Please enter a valid callShop id</td></tr>";
	 elseif(isset($param['systemId']) && preg_match(NOTNUM_REGX,$param['systemId']))
	 {
	     return $table.="<tr><td id = 'dummyTr' colspan='4'>Please enter a valid system id!</td></tr>";
       ...
       if(!preg_match('/^[0-9\-\s\:]+$/',$sdate) || !preg_match('/^[0-9\-\s\:]+$/',$edate))
            return $table.="<tr><td id = 'dummyTr' colspan='4'>Please enter a valid date!</td></tr>";
       ...
       if(!empty($param['shopId']))
            $sqlDate.=" AND callShopId=".trim($param['shopId']); 
       if(!empty($param['systemId']))
            $sqlDate.=" AND systemId=".trim($param['systemId']); 
