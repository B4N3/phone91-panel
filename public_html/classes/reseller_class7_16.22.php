...
//        $this->db->select('SQL_CALC_FOUND_ROWS *')->from($table)->where('type != 4 and type !=7 and type !=1 ' . $allcondition . ' order by ' . $order . ' limit ' . $start_limit . ',' . $limit);
	...
	$sql="SELECT SQL_CALC_FOUND_ROWS DISTINCT u.userId,u.userName, u.isBlocked, u.deleteFlag, u.type, u.beforeLoginFlag, u.sipFlag, u.userPin, u.callingStatus, u.password,ub.tariffId,ub.balance,ub.resellerId,pi.name,pln.planName,ub.currencyId FROM `91_userLogin` u JOIN `91_userBalance` ub ON u.userId=ub.userId JOIN `91_personalInfo` pi ON ub.userId = pi.userId JOIN `91_plan` pln ON ub.tariffId=pln.tariffId $this->joinQur WHERE  u.type NOT IN (4,7,1) $condition order by u.userId desc limit $start_limit,$limit";
//        $qur = $this->db->getQuery();
//        $result = $this->db->execute();
	...
	$sql="SELECT $qryPart DISTINCT u.userId,u.userName, u.isBlocked, u.deleteFlag, u.type, u.beforeLoginFlag, u.sipFlag, u.userPin, u.callingStatus, u.password,ub.tariffId,ub.balance,ub.resellerId,pi.name,pln.planName,ub.currencyId FROM `91_userLogin` u JOIN `91_userBalance` ub ON u.userId=ub.userId JOIN `91_personalInfo` pi ON ub.userId = pi.userId JOIN `91_plan` pln ON ub.tariffId=pln.tariffId $this->joinQur WHERE ub.resellerId='" . $user_id . "' and u.type NOT IN (4,7,1) $condition order by u.userId desc limit $start_limit,$limit";
