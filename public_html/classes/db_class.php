...
class db_class
{
	...
	{	
		...
		do
		{
			...
			$dbh=mysql_connect("192.168.1.174","voipswitchuser",'+4H8ZXcSyWn7CuX*');
			$i++;
			if($i==100)
			{
     ...
     try
                {
                    ...
                    $m = new Mongo("mongodb://127.0.0.1:27017/phone91");
//                    $db = $m->selectDB('giddh');
                    //$db->authenticate("giddh","giddhha!123");
                    $db=$m->phone91;
                    return  $db;
                    //exit();
                }
                ...
                {
//                    echo "1231";
                   logmonitor("phone91", " Mongo Connection error exception is ".$e);
                    die();
...
function mongoBulkInsert($collectionName,$dataArray)
{
        $db = $this->connectMongoDb();
        $db->$collectionName->batchInsert($dataArray);
        $status = $db->Command(array('getlasterror'=>1));
        return $status;//return status of current operation
        //Array ( [n] => 0 [connectionId] => 37 [err] => [ok] => 1 ) 
}
