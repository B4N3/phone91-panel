...
class reseller_class extends fun {
    ...
    function editClientInfo($parm,$userId){
        ...
        $currency_id = $funobj->getOutputCurrency($parm['currenctTariff']);
         ...
         $data=array("callLimit"=>$parm['callLimit'],"tariffId"=>$parm['currenctTariff'],"currencyId"=>$currency_id); 
...
//              $response[]="";	
    ...
    function allUserDetail(){
        ...
        $table = '91_manageClient';
        $this->db->select('*')->from($table)->where('type != 4 and type !=7 order by userId  desc limit 0,30');
        $this->db->getQuery();
        $result = $this->db->execute();
        $resultArray = array();
        if ($result->num_rows > 0) {
    ...
    function loadUsers($q,$user_id,$start_limit,$limit,$sort,$checkCondition = null)
	{
		...
		$sql="Select * from 91_manageClient where resellerId='".$user_id."'and type != 4 and type !=7 $condition order by ".$order." limit ".$start_limit.",".$limit;
