...
//      ($resellerSetting['mobile'] == 1? $beforeLoginFlag = 0 : $beforeLoginFlag = 1);
      (($this->currencyFlag == 1 || $this->singleCurrencyFlag == 1) ? $beforeLoginFlag = 1 : $beforeLoginFlag = 0);
            ...
            trigger_error("userbefore11".$beforeLoginFlag) ;  
     /*need to discuss this condition*/
       if($parm['domain'] == "phone91.com" && $parm['signupFrom'] != '1'){
          $beforeLoginFlag = 1;
         
      }
       trigger_error("userbefore12".$beforeLoginFlag) ;  
      /*
      ...
      if($parm['apiUser'] == "1")
      {
       ...
       trigger_error("userbefore13".$beforeLoginFlag) ;  
      ...
      if($parm['client_type'] == 7){
       ...
       trigger_error("userbefore14".$beforeLoginFlag) ;  
...
//      $beforeLoginFlag = 2; // for old signup funtion
         ...
         trigger_error("userbefore15".$beforeLoginFlag) ;  
...
//         $this->sendErrorMail("rahul@hostnsoft.com", "Phone91 signup_class 91_userBalance query fail : $tempsql ");
    ...
    public function signUp($param, $currencyFlag = 0)
    {
       ...
       trigger_error('saveUserDetail'. json_encode($param)."tariff".$this->tariffId.$balance.$currencyId);
...
//       echo $this->db->getQuery();
           ...
           logmonitor("phone91-google", $this->db->getQuery() );
...
//        	    var_dump($result);
    ...
    function check_email_avail($email) 
    {
        ...
        $table = '91_verifiedEmails';
        $this->db->select('email')->from($table)->where("email = '" . $email . "' and domainResellerId ='".$resellerIdServer."' "); // condition is removed because it may cause error.
        $result = $this->db->execute();
        // processing the query result
        if ($result->num_rows > 0) {
            return 0; //echo "Sorry username already in use";
