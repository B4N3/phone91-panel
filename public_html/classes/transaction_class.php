...
class transaction_class extends fun
{
   ...
   /**
     ...
     * @date: 18/12/2013
     * @modified by ANkit Patidar <anktipatidar@hostnsoft.com> on 10/10/2014 validation added
     ...
     * @modified by: nidhi<nidhi@walkover.in> 
     * @date: 18/12/2013
     * @Modified by Ankit Patidar <ankitpatidar@hostnsoft.com> on 10/10/2014 validation added
     * @param :  int $id Id Of User Or Reseller, 
    ...
    {
        if(!preg_match('/^[0-9]+$/', $id) && !preg_match(DECIMAL_REGX, $amount))
        {
            return 0;
        }   
   ...
   /**
     ...
     * @modified by: nidhi<nidhi@walkover.in> 
     * @date: 18/12/2013
     * @last update by Ankit Patidar <ankitpatidar@hostnsoft.com> on 10/10/2014 make optimization  
    ...
    function updateUserBalance( $toUser, $talktime, $sign = '+')
    {
	 ...
	 if(!preg_match('/^[0-9]+$/', $toUser) || !preg_match(DECIMAL_REGX, $talktime) || ($sign != '+' && $sign != '-'))
        {
            return 0;
        }   
   ...
   /**
    ...
    { 
        ...
        if($type == "prepaid")
        {
            $closingBalance = ((double)$closingBalance - (double)$clsamount);  
        ...
        else if($type == "partial")  #- If type is partial
        {
            ...
            $closingBalance = ((double)$closingBalance - (double)$partialbal);
    ...
    {  
        ...
        $this->newClosingAmount = ((double)$debitCntAmt + (double)$creditCntAmt); 
        ...
        $savedata = $this->insertData($data,$transactionlog);	
        $qur = $this->db->querry;
        ...
        else
        {
            ...
            $currName = $this->getCurrencyViaApc($currency);
            ...
            {
                //get currency
                $userCurName = $this->getCurrencyViaApc($userArr['currencyId']);
...
'.$commonMsg;
                ...
                else if($amount == 0 && ($credit != 0  || $debit != 0) )
                {
...
'.$commonMsg;
    ...
    {
	if(!preg_match('/^[0-9]+$/', $toUser) || !preg_match('/^[0-9]+$/', $currency) || !preg_match(DECIMAL_REGX, $amount))
        {
            return 0;
        }   
     ...
     * @filesource
     * @modified by: nidhi<nidhi@walkover.in> 
     * @last modified by Ankit patidar on 10/10/2014 validation added
     * @date: 18/12/2013
     * @param :  int $id , 
   ...
   /**
     ...
     * @filesource
     * @modified by: nidhi<nidhi@walkover.in> 
     * @last updated by Ankit Patidar <ankitpatidar@hostnsoft.com> on 10/10/2014 validation applied and optimized
     * @date: 18/12/2013
     * @param :  int $userId , 
     *           int $resellerTrans (Must be 0 ,1 or 2)
      *          int $paymentType (Payment Type is to select values for different cases.) 
     * @details : Function  for getting transaction log detail.
     * @return : int balance
     */
    
    function getPersonalTransaction($userId,$resellerTrans = 0, $fromUser = NULL,$pageNo=1,$isAdmin=0,$filterBy =NULL)
    {  
        
	//validations on parameters
        if(!preg_match('/^[0-9]+$/', $userId))
        {
            return json_encode(array("status"=>"error","msg"=>"Please select a valid user!"));
        } 
	else if($fromUser != NULL && !preg_match('/^[0-9]+$/',$fromUser))
	{
	    return json_encode(array("status"=>"error","msg"=>"Please select a valid user or login again!"));
	}
	else if(!preg_match('/^[0-9]+$/', $pageNo) || !preg_match('/^[0-9]+$/', $resellerTrans))
		return json_encode(array("status"=>"error","msg"=>"Please try with valid field!"));
      ...
      switch($resellerTrans)
      {
           ...
           case '3':
               $resellerTrans = 1;
                $condition = "fromUser = '" .$userId. "'  and paymentType!='voip' "; // or  paymentType='Fund Transfer'
              break;
      ...
      if($isAdmin == 1){
          ...
          if($filterBy != NULL && $filterBy != ''){
              if(preg_match('/^[a-zA-Z\s]+/', $filterBy)){
                  if($filterBy != "all")
                  $condition .=" and paymentType='".$filterBy."' "; 
                  
                  if($filterBy == "Share Talktime"){
                      $condition =" paymentType!='voip' and paymentType='".$filterBy."' ";
                  }
...
//      echo "sudhir".$SQL;
      //code to get count
       ...
       $transactionData['totalCount'] = ceil($countRes['totalRows']/$limit);
      ...
      if ($result->num_rows > 0)
      {
		  ...
		  if($row['changedBy'] == 1){
                        $userDetail = $this->getUserOrAcmUserName($row['changedBy'],$row['accManagerId']);
                          ...
                          if($row['paymentType'] == "Fund Transfer"){
                            if(strpos($row['description'],"by") == false){
	      ...
	      else
	      {
		     ...
		     $data['userName'] = $userDetail['userName'];     
              ...
              if($currencyViaApc == '' || $currencyViaApc == null)
              {
                  if($isAdmin == 1){
                      #- Get user currency id 
                      $toUserDetail = $this->getNameAndUserName($row['toUser']);//$currencyId
                      #- Get currency name 
                      $data['currencyName'] = $this->getCurrencyViaApc( $toUserDetail['currencyId'] , 1 ); 
    ...
    function checkTransactionValidation($parm)
    {
        ...
        if(isset($parm['currency']) && (!preg_match('/^[0-9]+/', $parm['currency'])))
        {
           return json_encode(array("status"=>"error","msg"=>"please select a valid currency !"));
        }
        
         if(isset($parm['toUser']) && (!preg_match('/^[0-9]+/', $parm['toUser'])))
        {
           return json_encode(array("status"=>"error","msg"=>"Please select a valid user."));
        }
    ...
    function addReduceTransaction($parm,$userId,$type)
    {
        ...
        {
             if(preg_match("/[^a-zA-Z\s]+/",$parm['transTypeOther']) || $parm['transTypeOther'] == "")
