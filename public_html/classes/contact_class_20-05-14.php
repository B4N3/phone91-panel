...
class contact_class extends fun{
	...
	function getUnConfirmEmail($userId)
	{
        ...
        function verifyEmailid($parm,$userid){
            ...
            $this->db->select('*')->from($temptable)->where("userid = '" . $userid . "' and confirm_code = '".$key."' and domainResellerId = '".$resellerIdServer."' and resellerId = '".$resellerId."'");
	    ...
	    $this->db->select('*')->from($table)->where("userid = '" . $userid . "' and confirm_code = '".$key."' and domainResellerId = '".$resellerIdServer."' and resellerId = '".$resellerId."' ");
            $this->db->getQuery();
	    $result = $this->db->execute();
            ...
            if ($result->num_rows == 0){
                ...
                $data=array("userid"=>(int)$userid,"email"=>$email,"confirm_code"=>$key,"newsFlag"=>$newsFlag , 'resellerId' => $resellerId ); 
       ...
       return json_encode(array("msgtype"=>"error","msg"=>"Country Code Is Not Valid!"));
        ...
        if ($this->checkNumberExist($country_code,$phone,$userid) == 1 ) {
            ...
            $resellerId = $this->getResellerId($userid);
            $table = '91_tempNumbers'; 
            $this->db->select('*')->from($table)->where("userId = '" . (int)$userid . "' and resellerId = '".$resellerId."' ");
            $result = $this->db->execute();
            if ($result->num_rows > 0) {
            ...
            $data=array("userId"=>(int)$userid,"tempNumber"=>$phone,"countryCode"=>(int)$country_code,"confirmCode"=>$confirm_code,"date"=>date('Y-m-d H:i:s') , "resellerId" => $resellerId ); 
    ...
    if ($result->num_rows > 0) {
         ...
         while($row = $result->fetch_array(MYSQLI_ASSOC)) {
		     $anotherUserId = $row['userId'];
                     ...
                     if($anotherResellerId == $resellerId || $anotherUserId == $resellerId){
                         $status = 1;
                         break;
                     }
		}
	...
	if (strlen($parm['key']) > 0 || !preg_match('/[^0-9]+/', $parm['key'])) 
        {
            ...
            $this->db->select('*')->from($temptable)->where("userId = '" . $userid . "' and confirmCode = '".$key."' and resellerId = '".$resellerId."' ");
	    ...
	    if ($result->num_rows <= 0){
            ...
            $this->db->select('*')->from($table)->where("userId = '" . $userid . "' and resellerId = '".$resellerId."' "); //userId = '" . $userid . "' and 
            $this->db->getQuery();
	    $result = $this->db->execute();
             if ($result->num_rows > 0){
                 $isdefault = 0;
            ...
            $data=array("userId"=>(int)$userid,"countryCode"=>(int)$countryCode,
