        ...
        if(!$result)
         ...
         if ($result->num_rows > 0) {
          while ($row= $result->fetch_array(MYSQL_ASSOC) ) {
              ...
              $bulkUserData['detail'][] = $userdata;
              
          }
         
          }else
              $bulkUserData['detail'][] = array();
...
//                    $table.='</span></div><div>&nbsp;</div>';
                ...
                $table.='<div><span><b>'.$key.'</b></span> - <span>'.$value.'</span></div>';
        }
        $table.=$volDetail;
        $table.='</div>';
       ...
       //$this->sendMail($table, "sudhir@hostnsoft.com" , 'verify user email');
        ...
        //$result = $db_obj->mongo_insert("feedBack",$reqParam);
         ...
         $response = array( "status" => "success" , "message" => $message ,"table"=>$table);
//        if(!$result)
//        {
