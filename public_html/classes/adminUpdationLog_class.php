...
class adminUpdationLog_class extends fun
{
    ...
    function getEditFundLog($pageNo = 1)
    {
            ...
            $this->db->select('SQL_CALC_FOUND_ROWS *')->from($table)->where("amount !=0 and changedBy = 1 and debit !=0 order by date desc ")->limit($limit)->offset($skip);;
            $this->db->getQuery();
            $result = $this->db->execute();
            ...
            if(!empty($allUserIds))
            {
                ...
                $result =  $this->selectData("currencyId,userId", "91_manageClient" , " userId  IN ($allUserIds)");
                
                $allCurrency = array();
                
                while ($row = $result->fetch_array(MYSQLI_ASSOC) ) 
                {
                    $allCurrency[$row["userId"]] = $this->getCurrencyName($row["currencyId"]);
                }
                
               $transactionData['currencyId'] = $allCurrency;
                
            }
            else
            {
                 $transactionData['currencyId'] = array();
            }
