	...
	function Call($paramarg) 
        {
            ...
            $connect_url = CALLSERVERURL1."clicktocall2.php"; // Do not change	
...
//                            $status = $get_userinfo['reason'];
        ...
        function cutCurrentCall($uniqueId,$userId,$clientType = 0){
            $this->status = "error";
//            echo $uniqueId = trim($uniqueId);
//            echo $uniqueId = trim($userId);
            if(preg_match(NOTNUM_REGX, $uniqueId) || empty($uniqueId)){
                $this->msg = "Error invalid unique ID";
                $this->code = "501";
                return false;
            }
            if(preg_match(NOTNUM_REGX, $userId) || empty($userId)){
                $this->msg = "Error invalid userId";
                $this->code = "502";
		...
		$condition = "uniqueId='".$uniqueId."' and id_client='".$userId."'";
            $result = $this->selectData('id_active_call', '91_currentCalls', $condition);
//            echo $this->querry;
            if(!$result  ){
                $this->code = "503";
                $this->msg = "Error unable to get the details";
                return false;
            }
...
//            die();
            if($result->num_rows < 1){
                $this->code = "507";
                $this->msg = "Error the call is not currently active";
                return false;
            }
            $row = $result->fetch_array(MYSQLI_ASSOC);
            
            if($row['id_active_call'] != "")
            {
                $url = "http://127.0.0.1:8888/cutcall?idActiveCall=".$row['id_active_call']."";
                $ch = curl_init();
...
//                $resultRecheck = $this->selectData('id_active_call', '91_currentCalls', "uniqueId='".$uniqueId."' and id_client='".$userId."'");
                if($curl_scraped_page != "success" ){
                    $this->msg = $curl_scraped_page;
                    $this->code = "504";
                    return false;
                }
//                if($resultRecheck->num_rows > 0){
//                $this->msg = "Error unable to cut the call please contact support";
//                $this->code = "506";
//                    return false;
//                }
                $this->msg = "Successfuly cut the call";
                $this->status = "success";
                $this->code = "505";
