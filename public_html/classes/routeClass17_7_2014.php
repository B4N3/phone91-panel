   ...
   */
    ...
    function setUserDialPlanOrRoute($param,$userId,$userType){
       ...
       if(preg_match('/[^0-9]+/', $param['clientId']) || $param['clientId'] == "")
        ...
        if(!preg_match('/^[a-zA-Z0-9\_\-]+$/', $param['routeDialplan'])){
           return json_encode(array("status" => "error", "msg" => "Please select valid option."));
        }
      ...
      if($resellerId != $userId)
      {
       ...
       if($param['routeDialplan'] == "route"){
           ...
           if(preg_match('/[^0-9]+/', $param['routeList']) || $param['routeList'] == "")
             return json_encode(array("msg"=>"Please select valid route.","status"=>"error"));
           ...
           $this->accountManagerLog($param['clientId'],13,$oldStatus,$newStatus,$userId,"update route and dial plan");
            ...
            if(preg_match('/[^0-9]+/', $param['dialPlanList']) || $param['dialPlanList'] == "")
             return json_encode(array("msg"=>"Please select valid dial plan.","status"=>"error"));
