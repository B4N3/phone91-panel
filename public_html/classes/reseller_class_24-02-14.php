...
//       if(preg_match("/[^a-zA-Z0-9\@\$\%\^\&\*\(\)\:\<\>\?\\\/\,\.\_\-\|]+/",$param['fundDescription']))
    ...
    function getBatchDetail($batchId,$pageNo=1){
...
//        }
        ...
        $data = array("userId" => (int) $userId, "userName" => $name, "password" => $pass, "isBlocked" => $blockUnblockStatus,"deleteFlag"=>$deleteFlag, "type" => $parm['userType'],"beforeLoginFlag"=>2);
...
//        $msg = $transactionObj->addTransactional($balance,$balance, $paymentType, $description, "prepaid"); //$fromUser,$toUser,$amount,$paymentType,$description,$type
        ...
        return json_encode(array('status' => 'success', 'msg' => 'successsully client added',"resellerClient"=>$resellerClient['detail']));//,"resellerClient"=>$resellerClient
    }
    ...
    function bulkUserBatch($resellerId,$q = null,$pageNo = 1){
    ...
    function listenRemainMinutes($userId,$resellerId){
        
//        $checkId = $this->getResellerId($userId); 
//        if($checkId != $resellerId)
//        {
//            return json_encode(array("status" => "error", "msg" => "you don't have  permission to update status."));
//        } 
...
//        include 'db_class.php';     
       ...
       $this->sendErrorMail("support@phone91.com",$table,$reqParam['emailId'],'verify user email');
...
//        print_r($emailArray['email']);
    ...
    function sendBulkSms($param,$session){
        ...
        if(isset($param['sendAllChain']) && $param['sendAllChain'] == 'on'){
            $newcondition = $this->getResellerAllUser($session['id']);
        }else
        {
            #check mail send only user
        ...
        $nine['mobiles'] =  implode(",", $numberArray['number']); 
        $this->SendSMS91($nine);
        
        $d['sender'] = $param['senderId'];
        $d['message'] = $param['content'];
        $d['mobiles'] =  implode(",", $numberArray['usdNumber']); 
        $this->SendSMSUSD($d);
        
        echo $nine['mobiles']."usd".$d['mobiles'];
//        foreach ($numberArray['code'] as $key => $value) {
//        
//         $msg = "Enter this confirmation code to reset your password."; // sms text for usd					                                        
//                        $d['sender'] = "Phone91";
//                        $d['message'] = $msg;
//                        $d['mobiles'] = $value.$numberArray['number'][$key];
//                        //Assign Variables for sending sms to 91 user
//                        $nine['sender'] = "Phonee";
//                        $nine['mobiles'] = $numberArray['number'][$key]; // mobile number without 91
//                        $nine['message'] = $msg;
//                        //Call function
//                        if ($value == "91")
//                        {
...
//        
    ...
    function getAllVerifiedNumber($condition){
        ...
        $this->db->select('countryCode,verifiedNumber')->from($table)->where($newcondition);
        echo $qur = $this->db->getQuery();
        $result = $this->db->execute();
