...
include_once(CLASS_DIR."validation_class.php");
...
class profile_class extends validation_class
{
	...
	function load_total_searched_users($user_id,$q, $unique = null)
	{       $condition = "like '%".$q."%'";
		if($unique)
		    $condition = "='".$q."'";
		$sql="Select count(*) from clientsshared where id_reseller='".$user_id."' and lower(login)".$condition ;
		$dbh=$this->connect_db();
		$result=mysql_query($sql,$dbh);
		mysql_close($dbh);
		if (!$result)
	...
	function load_searched_users($user_id,$q,$start=0,$limit=10, $unique)
	    {   $condition = "like '%".$q."%'";
		if($unique)
			$condition = "='".$q."'";
		$dbh=$this->connect_db();
                ...
                if(is_numeric($q))
                {
                    $sql_number = "select * FROM contact where contact_no".$condition;
			...
			die ("Fatal Error in Loading Total Number of User");
                ...
                elseif(strlen($q)>0)
                {
                    $sql="Select * from clientsshared where id_reseller='".$user_id."' and lower(login) ".$condition."  order by login asc limit $start, $limit";
                }
