...
//            }
   ...
   #created by sudhir pandey (sudhir@hostnsoft.com)
     ...
     function updateContact($parm, $userid,$type='0',$agentEdit='0') 
    {
        ...
        {
             $condition['contactNo'] = $allcontact;
        }
       ...
       $subcondition = array('$ne' => new mongoId($contactId));
        ...
        $conditionSearch = array( 'contactNo' => $allcontact ,'code' =>$code , 'userId' => $_SESSION['id'] ,'contact_id' => $subcondition);
        
        $result = $dbobj->mongo_count($collectionName , $conditionSearch);
        ...
        if(!preg_match(NOTPHNNUM_REGX,$accessNo)  && strlen($accessNo) > 10 &&  strlen($accessNo) < 20 )
        {
           //echo ' Accessno '.$accessNo;
        ...
        catch (MongoCursorException $ex) 
        {
            ...
            return json_encode(array("status" => "success", "msg" => "Something went wrong please contact provider."));   
        } 
        
        $addLaststatus = $this->addLastUpdateTime($userid);
	
	if(!$addLaststatus)
	{
	    trigger_error('Problem while update contact updation time!,userId:'.$userid);
	} 
