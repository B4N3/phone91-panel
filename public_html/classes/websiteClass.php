...
class websiteClass {
    ...
    protected $domainName;
     ...
     /**
     * @uses $compEmail Description class varible for storing company email
     * @var type
     */
    protected $compEmail;
    ...
    /**
     * @use $websiteFolder Description class variable is for storing the website folder name in which all the website data stored
     */
    protected $websiteFolder = "manageWebsiteImage/";
    /**
     * @use $websiteFolder Description class variable is for storing the source code in string format to render the mangewebsite variables
     */
    protected $sourceCodeString = 'foreach($data as $key => $val)
                                    {
                                        if(is_array($val))
                                        {
    ...
    public function setDomainFolder($domain) {
        return $this->domainFolder = str_replace(".", "_", $domain); 
    }
   ...
   *Last modified by ankit patidar <ankitpatidar@hostnsoft.com>  on 14/05/2014
   */
   function addManageWebsite($parm,$userId){
        ...
        */
        ...
        $this->compEmail = trim($parm['compEmail']);
       ...
       if(preg_match(NOT_COMPANY_NAME_REGX, $this->companyName)){
            return json_encode(array("status" => "error", "msg" => "company name is not valid !"));
       }
       
       #check for domain name is valid or not 
       if(preg_match(NOT_DOMAIN_NAME_REGX, $this->domainName) || strlen($this->domainName) < 5){
            return json_encode(array("status" => "error", "msg" => "domain name is not valid !"));
       }
        

      if(!preg_match(EMAIL_REGX, $this->compEmail) || strlen($this->compEmail) < 5){
            return json_encode(array("status" => "error", "msg" => "email is not valid !"));
       } 
       #check for Theme name is valid or not 
       if(preg_match(NOT_THEME_REGX, $this->theme)){
            return json_encode(array("status" => "error", "msg" => "Theme name is not valid !"));
       }
       
       #check for language is valid or not 
       if(preg_match(NOT_LANGUAGE_REGX, $this->language)){
            return json_encode(array("status" => "error", "msg" => "language is not valid !"));
       }
       
       $this->companyName = base64_encode($this->companyName);
       ...
       #loop through the param for validating the tariff please and currency with balance 
        for($i = 0 ; $i < $tariffIdCnt ; $i++)
        {
            ...
            if(preg_match("/[^0-9]+/", $parm['tariffPlan'][$i]) || preg_match("/[^0-9]+/", $parm['currency'][$i]) || preg_match("/[^0-9\.]+/", $parm['balance'][$i]) || strlen($parm['balance'][$i]) > 10){
                return json_encode(array("status" => "error", "msg" => "Invalid tariff or default value for balance provided please fill valid input!"));
            }
       ...
       $result = $funobj->selectData("domainName",$table,"domainName = '".$this->domainName."'");
       ...
       $data = array("companyName"=>$this->companyName,"domainName"=>$this->domainName,"resellerId"=>$this->resellerId,"language"=>$this->language, "compEmail" => $this->compEmail);
...
//       $domainFolder = str_replace(".","_",$this->domainName);
       ...
       #check if file exist
       if(!file_exists('../manageWebsiteImage/'.$this->domainFolder))
       {
            #create directory for domain data 
            $dirResult = mkdir('../manageWebsiteImage/'.$this->domainFolder."/", 0777,true);
            if (!$dirResult) {
...
//       echo $funobj->querry;
       ...
       $dataForFile = $data;
       $dataForFile['companyName'] = base64_decode($dataForFile['companyName']);
       $dataForFile['id'] = $lastInsertId;
       $dataForFile['theme'] = "template1";

       $dataForFile = $this->changeEncoding($dataForFile);
       # if directory created then add main file in same directory
       $fp = fopen('../manageWebsiteImage/'.$this->domainFolder.'/main.php', 'w');
       $fwriteResult = fwrite($fp, '<?php $data='.var_export($dataForFile,1) ." ; $this->sourceCodeString ?>");
       fclose($fp);
       ...
       if($fwriteResult === FALSE)
       {
           ...
           return json_encode(array("status" => "error", "msg" => "Error in writting data to the database please try again!"));
       }
       ...
       if($defaultCurrResult && $funobj->db->affected_rows > 0){
           ...
           /**************UPDATE APC**************** */
            ...
            $resultDomainDetails = $funobj->selectData("*", $table);
            if ($resultDomainDetails && $resultDomainDetails->num_rows > 0) {
               
                while ($row = $resultDomainDetails->fetch_array(MYSQLI_ASSOC)) {
                    $dataDomainDetials[$row['domainName']] = $row;
                }
                
                $apcStore = apc_store("domainDetails", $dataDomainDetials);
                ...
                $delResDefCur = $funobj->deleteData("91_resellerDefaultCurrency", " domainId=".$lastInsertId);

                if (!$delResDefCur || $funobj->db->affected_rows < 1)
                    return json_encode(array("status" => "error", "msg" => "Error in domain addition please contact provider else it wont function properly!"));

                return json_encode(array("status" => "error", "msg" => "Error Fetching data please try again later"));
            }
            /********end here ************* */
    ...
    *last updated by Ankit Patidar <ankitpatidar@hostnsoft.com> on 14/05/2014
    * @return type
    */    
   public function updateDomainDetails($param,$resellerId) {
        ...
        */
       $companyName = base64_encode($param['companyName']);
       $compEmail = trim($param['compEmail']);
...
//       var_dump(preg_match(NOTALPHABATESPACE_REGX, $companyName));
//       if(preg_match(NOTALPHABATESPACE_REGX, $companyName) || $companyName == "")
       ...
       if(!preg_match(EMAIL_REGX, $compEmail) || $compEmail == "")
       {
           return json_encode(array("msg"=>"Error invalid email please try with a valid email address ","status"=>"error"));
       }
       ...
       #data to be updated 
       $data = array("companyName"=>$companyName,"domainName"=>$domainName,'compEmail' => $compEmail);
       ...
       $selData = $funobj->selectData("domainName,theme",$table,$condition);
       if($selData)
       {
           $row = $selData->fetch_array(MYSQLI_ASSOC);
           $currenctDomainName = $row['domainName'];
           $currenctTheme = $row['theme'];
       }
       else
       {
       ...
       if($updateResult)
       {
           if($affectedRows > 0)
           {
               $currenctDomainName = $this->setDomainFolder($currenctDomainName);// str_replace(".", "_", $currenctDomainName);
               $domainName = $this->setDomainFolder($domainName); //str_replace(".", "_", $domainName);
               
            if(file_exists(ROOT_DIR."/manageWebsiteImage/".$currenctDomainName))
            {
                $dirResult =rename(ROOT_DIR."manageWebsiteImage/".$currenctDomainName, ROOT_DIR."manageWebsiteImage/".$domainName);
                
                if ($dirResult) {
                    $data['companyName'] = base64_decode($data['companyName']);
                    $data['language'] = 'English';
                    $data['resellerId'] = $resellerId;                    
                    $data['id'] = $domainId;                    
                    $data['theme'] = $currenctTheme;                    
                    $data = $this->changeEncoding($data);
                    # if directory created then add main file in same directory
                    $fp = fopen(ROOT_DIR."manageWebsiteImage/".$domainName.'/main.php', 'w');
                    fwrite($fp, '<?php $data='.var_export($data,1) ." ; $this->sourceCodeString ?>");
                    fclose($fp);

                } else {
                    return json_encode(array("status" => "error", "msg" => "Error changing the domain name, your website will not function properly please contact support"));
               }
                
                
            }
            else
                return json_encode(array("msg"=>"Error changing the domain name, folder not found","status"=>"error"));
    ...
    * @param type $resellerId
    * @uses used to get the current selected theme of the domain 
    */
   public function getCurrentTheme($domainName,$resellerId) {
       ...
       $funobj = new fun();
       $domainName = $funobj->db->real_escape_string($domainName);
       $result = $funobj->selectData("theme", "91_domainDetails","domainName='".$domainName."'");
       if(!$result)
           return false;
   ...
   public function updateTheme($param,$resellerId) {
       ...
       $themeData = array("theme"=>$themeName);
       $table = "91_domainDetails";
       $condition = " resellerId=".$resellerId." and domainName='".$domainName."'";
       
       $this->setDomainFolder($domainName); // str_replace(".","_",$domainName);
       $domianDataPath = ROOT_DIR.$this->websiteFolder.$this->domainFolder."/";
       
       $currentTheme = $this->getCurrentTheme($domainName,$resellerId);
       if(!$currentTheme)
           return json_encode(array("status" => "error", "msg" => "Error fetching details please try again"));
       
       
       /* be carefull this fiel will create many variables 
        * such as resellerId etc that can replace some existing
        * values
        */
       include_once($domianDataPath."main.php");
       $data['theme'] = base64_encode($themeName);
       
       #update data domain details 
       $updateResult = $funobj->updateData($themeData, $table,$condition);
       ...
       {
            # if directory created then add main file in same directory
            $fp = fopen($domianDataPath.'main.php', 'w');
            $writeResult = fwrite($fp, '<?php $data='.var_export($data,1) ." ; $this->sourceCodeString ?>");
            fclose($fp);
            
            if($writeResult === FALSE)
            {
                $themeData = array("theme"=>$currentTheme);
                $revertResult = $funobj->updateData($themeData, $table,$condition);
                if(!$revertResult)
                    return json_encode(array("status" => "error", "msg" => "Error reverting changes please contact surrport"));
                
                return json_encode(array("status" => "error", "msg" => "Error changing the theme, please try again"));
            }
                
           return json_encode(array("msg"=>"successfully updated the records","status"=>"success"));
       }
       elseif($affectedRows == 0)
           return json_encode(array("msg"=>"Theme already selected Nothing to update","status"=>"warning"));
       else
           return json_encode(array("msg"=>"Error updating record pelase try again later","status"=>"error"));
   ...
   function getManageWebsite($resellerId){
       ...
       while($row = $result->fetch_array(MYSQLI_ASSOC))
        {
           $row['companyName'] = base64_decode($row['companyName']);
           
           $allWebsite[] = $row;
        }
        $json = json_encode($allWebsite);
        return $json;
   ...
   #get the reseller id of the domain
    ...
    $this->dbFileName = '../manageWebsiteImage/'.$this->domainFolder.'/'.$type.'.php';
       ...
       if(file_exists($this->dbFileName))
        {
           #get the data from the file 
            $string = $this->getFileData($this->domainFolder.'/'.$type.'.php');
...
//    } ';
   ...
   switch($type)
    {
        ...
        case "pricing":
        {
            $this->sourceCodeString = ' foreach($data as $key => $val)
                                        {
                                            if(is_array($val))
                                            {
                                                foreach($val as $ikey => $val2)
    ...
    $updateData = $this->changeEncoding($updateData);
    
    
    #save general page Data ( json formate ) into generalData.js file  
    $fp = fopen($this->dbFileName, 'w');
    fwrite($fp, '<?php $data='.var_export($updateData,1)." ; $this->sourceCodeString ?>");
    fclose($fp);
   ...
   private function changeEncoding($data)
   {
       foreach($data as $key => $value)
       {
           if(is_array($value))
               $updateArr[$key] = $this->changeEncoding($value);
           else
              $updateArr[$key] = base64_encode($value);
       }
       return $updateArr;
   }
...
//        if(preg_match(NOTTEXT_REGX, $param['welcomeContent']) || strlen($param['welcomeContent']) > 350)
        if(strlen($param['welcomeContent']) > 350)
        {
...
//        if(preg_match(NOTTEXT_REGX, $param['subHeading']) || strlen($param['subHeading']) > 250)
        if(strlen($param['subHeading']) > 250)
        {
...
//        if(preg_match(NOTTEXT_REGX, $param['whoUR']) || strlen($param['whoUR']) > 300)
        if( strlen(utf8_decode($param['whoUR'])) > 300)
        {
...
//        if(preg_match(NOTTEXT_REGX, $param['vision']) || strlen($param['whoUR']) > 300)
        if(strlen(utf8_decode($param['vision'])) > 300)
        {
...
//        if(preg_match(NOTTEXT_REGX, $param['mission']) || strlen($param['whoUR']) > 300)
        if( strlen(utf8_decode($param['mission'])) > 300)
        {
...
//        if(preg_match(NOTTEXT_REGX, $param['gMapEmbededCode']) || strlen($param['gMapEmbededCode']) > 250)
        if(($param['gMapEmbededCode'] != "" && !preg_match(URL_REGX, $param['gMapEmbededCode'])) || strlen($param['gMapEmbededCode']) > 250)
        {
   ...
   function addHomeData($param){
    ...
    $updateData = array("welcomeImage" => $this->fileName,
                        "homeMeta" => array("mKeyword" => $param['mKeyword'],
                        "mDescription" => $param['mDescription'],
                        "title" => $param['title']),
                        "welcomeContent" => $param['welcomeContent'],
                        "homebannerDetail" => array("heading" => $param['heading'],
   ...
   function addAboutData($param){
    ...
    $updateData = array('domainId' => $param['domainId'],
                        ...
                        "aboutMeta" => array("title"=>$param['title'],
                        "mKeyword"=>$param['mKeyword'],
                        "mDescription"=>$param['mDescription']),
                        "bannerStatus"=>$bannerStatus,
                        "aboutbannerDetail"=>array("heading"=>$param['heading'],
                                              "subHeading"=>$param['subHeading'],
                                              "text"=>$param['text'],
                                              "link"=>$param['link']),
                        "whoUR"=>$param['whoUR'],
   ...
   function addContacPageData($param){
    ...
    #update data array
    $updateData = array('domainId' => $param['domainId'],
            "contactMeta" => array("title" => $param['title'],
            "mKeyword" => $param['mKeyword'],
            "mDescription" => $param['mDescription']),
            "cntbannerStatus" => $cntBnrStatus,
            "contactbannerDetail" => array("heading" => $param['heading'],
                "subHeading" => $param['subHeading'],
                "text" => $param['text'],
                "link" => $param['link']),
   ...
   function addPricingData($param){
    ...
    for ($i = 0; $i < count($bankName); $i++) {
        ...
        if(preg_match(NOTALPHANUM_REGX, $ifsc[$i]) || $ifsc[$i] == "" || strlen($ifsc[$i]) > 15)
        {
    ...
    $updateData = array('domainId' => $param['domainId'],
        "welcomeImage" => $this->fileName,
        "pricingMeta" => array("title"=>$param['title'],
            "mKeyword"=>$param['mKeyword'],
        "mDescription"=>$param['mDescription']),
        "pricingbannerDetail" => array("heading" => $param['heading'],
                "subHeading" => $param['subHeading'],
                "text" => $param['text'],
                "link" => $param['link']),
