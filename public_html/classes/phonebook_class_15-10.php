...
class phonebook_class {
    ...
    function addContact($parm, $userid) {
        ...
        if ($status) {
            ...
            return json_encode(array("status" => "success", "msg" => "total ".$countTotalSave." Contact successfully add !"));
              ...
              if($alreadyExist > 0){
                return json_encode(array("status" => "error", "msg" => "Contact number alreadey exists !"));  
              }else
                return json_encode(array("status" => "error", "msg" => "Invalid contact number please provied atleast one valide contact number !"));
            }  
        }
    ...
    function updateContact($parm, $userid) {
        ...
        if(!preg_match("/^[a-zA-Z_@-\s]+$/", $allname)){
            return json_encode(array("status" => "error", "msg" => "Please enter valide contact name!"));
        }
...
//     
    ...
    function checkContactNull($allcontact,$allemail){
       #check first contact no is valide or not
        if(count($allcontact) == 1){
            ...
            if ($allemail[0] != '' || $allemail[0] != null) {
                if (!preg_match("/^[_A-Za-z0-9-\\+]+(\\.[_A-Za-z0-9-]+)*@[A-Za-z0-9-]+(\\.[A-Za-z0-9]+)*(\\.[A-Za-z]{2,})$/ix", $allemail[0])) {
                    return $msg = "please enter valide email";
                }
            }
