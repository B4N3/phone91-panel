...
//        }
    ...
    function getAccessNumberLogDetail($request , $pageNo=1)
    {
        ...
        $qString = (isset($request['q']) && $request['q']) != ''?$this->db->real_escape_string($request['q']):'';
          ...
          if($qString !='')
          {
               $likeQ = "callerId LIKE '%$qString%' or  accessNumber LIKE '%$qString%'  or  uid LIKE '%$qString%'   and";
          }
          else
          {
               $likeQ = "";
        ...
        $table = '91_accessNumberLog';

	#get all access number from 91_longCodeNumber table  
	//$result = $this->selectData('uid,step,accessNumber,callerId,userInfo,date',$table,"1");
        ...
        $this->db->select('SQL_CALC_FOUND_ROWS *')->from($table)->where($likeQ.' date BETWEEN "'.$sDate.'" AND "'.$eDate.'"')->orderBy("date")->limit($limit)->offset($skip);
        
         $SQL= $this->db->getQuery();
        $result = $this->db->execute();
        
        
	$accessDetailArr = array();
	...
	if($result->num_rows > 0)
	{
        ...
        $resultTotal = $this->selectData('uid,step,accessNumber,callerId,userInfo,date',$table, $likeQ.' date BETWEEN "'.$sDate.'" AND "'.$eDate.'"');
        
        
        $count = $resultTotal->num_rows;
        $totalRecords = ceil($count/$limit);
      
        return json_encode( array( "status"=> "success" ,   "message"=>"Records found successfully." ,  "data" => $accessDetailArr , "totalCount" => $totalRecords) );
