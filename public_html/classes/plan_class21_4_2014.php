...
//                $outputTariifId = $request['plantype'];
        ...
        if ($execution != 1 && $append == 0) {
            ...
            return json_encode(array("msg" => "Plan Inserted successfuly", "status" => "success", "insertId" => $this->last_id,"errorCode"=>"1026"));
        }
    }
...
//            $value['count'] = $this->countData('91_plan', " userId=" . $userId . " ");
    ...
    private function loadTariffRates($tariffId, $first_limit, $limit,$search = 0,$keyword="", $select_type = NULL, $select_field = NULL) {
...
//        $tariffId = $this->db->real_escape_string($tariffId);
        ...
        if($search == 1 && $keyword != "")
        {
         ...
         $condition .= $substr . " limit " . $first_limit . "," . $limit;
//         echo $condition;
        $r_rp_detail = $this->selectData("*", "91_tariffs", $condition);
        if(!$r_rp_detail)
        {
        ...
        {
            $this->msg = "Error No Records found with this search";
            $this->status = "error";
            return false;
        }
    ...
    public function countTotalTariffs($tariffId)
    {
     ...
     public function searchTarriff($keyword, $tariffId,$userId,$isAdmin = 0,$userDefaultTariffId) 
    {
        /* @author : SAMEER
         * @created : 16-09-2013
...
//        $res = $this->db->query($sql);
    ...
    public function getTarrifDetails($request,$userDefaultTariffId, $userId,$isAdmin = 0) {
        /* @author : Sameer Rathod <sameer@hostnsoft.com>
        ...
        $r_rp_detail = $this->loadTariffRates($tariffId, $first_limit, $limit);
    ...
    public function getPlanName($fields, $userId,$type,$tariffId = null,$isAdmin=0) 
    {
        ...
        if(is_null($userId) || is_null($fields) || preg_match('/[^a-zA-Z\,]/', $fields))
            return json_encode (array("msg"=>"Error Invalid data User","status"=>"error"));
        
        if(preg_match('/[^0-9]+/', $tariffId))
            return json_encode (array("msg"=>"Error Invalid tariff Id ","status"=>"error"));
        ...
        if(!$defaultTariffId)
            return array("msg" => "Error Unable to fetch the user defautl plan","status"=>"error");    
        ...
        if ($result)
        {
            ...
            elseif($type == 2)
            {
                while ($row = $result->fetch_array(MYSQLI_ASSOC)) 
                {
                    ...
                    $row['currency'] = $this->getCurrencyName($row['outputCurrency']);
                    $planNameArr[] = $row;
                }
