...
include_once(dirname(dirname(__FILE__)).'/sendmail.php');
...
class payment_class extends fun{
   ...
   function paymentResponse($param,$userId){
	...
	$currency = $this->getCurrencyViaApc($currencyId['currencyId']);
		...
		$tranxId = $charge->id;
                ...
                $res = $this->updateUserStatus($userId,$talktime,$recharge,$orderId,$currency,$tranxId);   
   ...
   function updateUserStatus($idClient,$dbTalktime,$dbRechare,$orderId,$currency,$tranxId){
    ...
    $updatedFlag =  $tranxObj->updateUserBalance($idClient,$dbRechare,'+');//update balance
    ...
    //get username
	$userName = $this->getUserId($idClient,1);
	if(!isset($userName))
	    $userName = 'Unknown userId :'.$idClient;
    //set mail msg
	$mailMsg="One phone91 user with id ".$userName." did stripe payment of ".$dbTalktime." ".$currency." and got recharged by ".$dbTalktime." ".$currency.' tranx id:'.$tranxId.'';

	
	
	//send mail via mandrill
        $subject = "Phone91 Stripe payment";
        $message = $mailMsg;
	$mailErr = new MailAndErrorHandler();//create object
        if(!$mailErr->sendmail_mandrill(unserialize(SUCCESSMAILS), $subject, $message,BUSINESSMAIL))
        {
