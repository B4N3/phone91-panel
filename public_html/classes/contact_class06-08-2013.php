...
class contact_class extends fun{
	...
	function getUnConfirmEmail($userid)
	{
	    ...
	    if ($result->num_rows <= 0){
                  ...
                  return json_encode(array('msgtype' => 'error', 'msg' => 'verification code not march !'));             
	    ...
	    $result = $this->db->execute();
            ...
            if ($result->num_rows == 0){
                ...
                return json_encode(array('msgtype' => 'success', 'msg' => 'Email id successfuly verify !'));
             
                               
                                      
            }else 
                { //in case of number assign to another user
                 return json_encode(array('msgtype' => 'error', 'msg' => 'Sorry this email id is already used !'));
                }
            }
            
	}
        return json_encode(array('msgtype' => 'error', 'msg' => 'Please Enter Varify Code !'));
       ...
       return json_encode(array("msgtype"=>"error","msg"=>"Contact number is not valid!"));
        ...
        if ($result->num_rows == 0) {
            ...
            if ($result->num_rows > 1) {
                 return json_encode(array('msgtype' => 'error', 'msg' => 'firstly verify your unverify contact number !'));
...
//		    $confirm = $row['confirm'];
            ...
            $table = '91_verifiedNumbers';
	    $this->db->select('*')->from($table)->where("userId = '" . $userid . "' and verifiedNumber = '".$tempNumber."'");
            $this->db->getQuery();
	    $result = $this->db->execute();
            if ($result->num_rows == 0){
...
//            var_dump($result);
	    ...
	    $result = $this->db->execute();
             ...
             return json_encode(array('msgtype' => 'success', 'msg' => 'number successfuly verify !'));
