...
//                $outputTariifId = $request['plantype'];
        ...
        if ($execution != 1 && $append == 0) {
            ...
            return json_encode(array("msg" => ($msg == ""?"Error inserting plan please try again":$msg), "status" => "error", "type" => $execution));
    ...
    public function getPlans($request, $userId,$isAdmin = Null) {
       ...
       /*****************************************/
        ...
        if ($request['mnpln'] == 'search') {
            if(preg_match('/[^a-zA-Z]+/', $request['planName']) || $request['planName'] == "")
                return json_encode (array("msg"=>"Invalid Plan Name Please provide a valid name alphabet only","status"=>"error"));
...
//            $value['count'] = $this->countData('91_plan', " userId=" . $userId . " ");
    ...
    public function getTarrifDetails($request, $session) {
        ...
        if(!isset($request['pid']) || preg_match('/[^0-9]+/', $request['pid']) || $request['pid'] == "")
...
//        }
        $jade['pages'] = $pages;
        $tariffId = $this->db->real_escape_string($request['pid']);
    ...
    public function addPlanFromAdmin($planName,$outputCurr,$billingSec,$userId,$type = 'add',$tariffId=Null)
    {
        ...
        else
        {
            ...
            if ($result->num_rows == 0) 
            {
                ...
                if($insertPlanRes)
                    return json_encode(array("msg"=>"Successfuly inserted plan ","status"=>"success","lastId"=>$insertPlanRes));
                else
                    return json_encode(array("msg" => "Error Inserting Plan Please try again", "status" => "error"));
            }
            else
