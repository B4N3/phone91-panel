...
//        	    var_dump($result);
    ...
    function addNewClientBatch($request,$resellerId){
        ...
        if(isset($request['listenTime']) && $request['listenTime'] !='1' || !isset($request['listenTime']))
        {
            $request['listenTime'] = "0";
        }
        ...
        $data = array("batchName" => $request['batchName'],"numberOfClients"=>(int)$request["totalClients"],"expiryDate"=>$request["batchExpiry"],"userId"=>$resellerId,"batchBalance" =>$request['balance'],"listenTime" => $request['listenTime'] , "acmId" => $_SESSION['acmId']);
        ...
        for($request['totalClients'];$request['totalClients']>0;$request['totalClients']--){
            ...
            $password= $this->createUsername($batchId);  
                ...
                $data = array("userId" => (int) $userid,"chainId"=>$chainId , "tariffId" => (int) $request['tariff'], "balance" => $balance, "currencyId" => (int) $currency_id, "callLimit" => (int) $call_limit, "resellerId" => (int) $resellerId, "userBatchId"=>(int)$batchId ,"routeId"=>$userRoutDetail['routeId'],"isDialPlan"=>$userRoutDetail['isDialPlan'],"getMinuteVoice" =>  $request['listenTime'] );
                #insert query (insert data into 91_userLogin table )
                ...
                */
                ...
                $msg = $transaction_obj->addTransactional_sub($balance,$balance,"Bulk Add",0,0,0,"Bulk User Added, BatchName :".$batchName,0);
...
//            $tempVar = false;
   ...
   function loginAsInAdmin($request,$session)
   {
      ...
      //get user id and validate that
       if(!isset($request['userId']) || !is_numeric($request['userId']))
       {
           return json_encode(array('status' =>"error",'msg' => 'Internal error please try again.'));
           
       }
       
       $updateToken = $this->updateLoginAsToken($request['userId']);
       
       $tokenDetail = json_decode($updateToken,true);
       
       if($tokenDetail['status'] == "error"){
           return json_encode(array('status' => "error",'msg' => $tokenDetail['msg']));
       }
       
       # update token of login as and redirct to another domain
     
       return json_encode(array('status' => "success",'msg' => $tokenDetail['msg'],"token"=>$tokenDetail['loginToken']));
//       
//       
//       
...
//       $column = "userId";
   ...
   function updateLoginAsToken($userId){
       ...
       $loginToken = $this->randomNumber(30);
       
       $table = '91_userLogin';
       $date = date('Y-m-d H:i:s');
       $data = array("loginAsToken"=>$loginToken,"loginAsTime"=>$date);
       
       $result = $this->updateData($data,$table,"userId='".$userId."'");
       if($result)
       {
           return json_encode(array('status' => "success",'msg' => 'Successfuly updated login token.',"loginToken"=>$loginToken));
       }
       else{
           return json_encode(array('status' => "error",'msg' => 'Login token not updated.'));
        }
