...
class contact_class extends fun{
    ...
    function getConfirmMobile($userId)
	{
	    ...
	    #select query 
            $this->db->select('`varifiedNumber_id`, `userId`, `domainResellerId`, `countryCode`, `verifiedNumber`, `confirmCode`, `verifiedDate`, `isDefault`, `description`, `resellerId`, `status`')->from($table)->where("userId = '" . $userId . "'");
            $this->db->getQuery();
     ...
     * @return int
     */
    function getVerifiedNumber($particular,$type=1,$coloumn=NULL,$resellerId=NUll)
    {
        
        
        if(is_array($particular))
        {
            foreach($particular as $val)
            {
                if(preg_match(NOTNUM_REGX,$val) || empty($val))
                {
                    $this->msg = "Error Invalid Number provided";
                    $this->status = "error";
                    $this->code = 403;
                    return false;
                }
            ...
            //prepare condition
            $particularString = implode("','", $particular);
            $condition = "CONCAT(countryCode,verifiedNumber) IN ('".$particularString."') ";
        }
        elseif(preg_match(NOTNUM_REGX,$particular) || empty($particular))
        {
            $this->msg = "Error Invalid Number provided";
            $this->status = "error";
            $this->code = 403;
        ...
        else
        {
            //prepare condition
            if(2 == $type)
                $condition = "CONCAT(countryCode,verifiedNumber) = '" . $particular . "'";
            else
                $condition = "userId = '" . $particular . "'";
        }
        ...
        if(!is_null($resellerId))
        {
            ...
            {
                $this->msg = "Error Invalid reseller Id please try with a valid resellerId ";
                $this->status = "error";
                $this->code = 403;
                return false;
            }
        ...
        {
            if(preg_match(NOTALPHABATECOMMA_REGX,$coloumn) || empty($coloumn))
            {
                $this->msg = "Error Invalid Input provided";
                $this->status = "error";
                $this->code = 403;
                return false;
            }
...
//        echo $condition;
        ...
        if(!$result){
            $this->msg = "Error unable to get details please try again";
            $this->status = "error";
            $this->code = 403;
            return false;
        }
...
//        var_dump($condition);
        ...
        {
            $this->msg = "Error No Number found";
            $this->status = "error";
            $this->code = 4031;
            //this is because the above funciton have this login and is user in many places 
            $contact = array();
        }
    ...
    function getUnconfirmMobile($userId)
	{
	    ...
	    $this->db->select('`slNo`, `userId`, `domainResellerId`, `countryCode`, `tempNumber`, `confirmCode`, `date`, `description`, `resellerId`')->from($table)->where("userId = '" . $userId . "'");
            $this->db->getQuery();
	    $result = $this->db->execute();
	    // processing the query result  
           // var_dump($result);
	...
	function getConfirmEmail($userId,$fields = NULL)
	{
	    ...
	    $contact=array();
            if(is_null($fields))
                $fields = "`verifiedEmail_id`, `userid`, `email`, `confirm_code`, `default_email`, `newsFlag`, `date`, `resellerId`, `domainResellerId`";
	...
	function getUnConfirmEmail($userId)
	{
	    $tempEmail = '91_tempEmails'; 
            $this->db->select('`tempEmail_id`, `userid`, `date`, `email`, `confirm_code`, `domainResellerId`, `newsFlag`, `resellerId`')->from($tempEmail)->where("userid = '" . $userId . "'");
            $result = $this->db->execute();
	    // processing the query result           
	    if ($result->num_rows > 0) {
		while($row= $result->fetch_array(MYSQLI_ASSOC)) {
        ...
        #last modified by sameer on email id is added 
		...
		return json_encode(array( "msg" => "Please Enter a valid email!","status" => "error" ));
            ...
            $this->db->select('`tempEmail_id`, `userid`, `date`, `email`, `confirm_code`, `domainResellerId`, `newsFlag`, `resellerId`')->from($temptable)->where("userid = '" . $userid . "' and confirm_code = '".$key."' and domainResellerId = '".$resellerId."'");
	    ...
	    $this->db->select('`verifiedEmail_id`, `userid`, `email`, `confirm_code`, `default_email`, `newsFlag`, `date`, `resellerId`, `domainResellerId`')->from($table)->where("userid = '" . $userid . "' and confirm_code = '".$key."' and domainResellerId = '".$resellerIdServer."' ");
            $this->db->getQuery();
	    $result = $this->db->execute();
       ...
       return json_encode(array("msgtype"=>"error","msg"=>"Country Code Is Not Valid!"));
        ...
        if ($this->checkNumberExist($country_code,$phone,$userid) == 1 ) {
            ...
            #table name 
            $table = '91_tempNumbers'; 
            $this->db->select('`slNo`, `userId`, `domainResellerId`, `countryCode`, `tempNumber`, `confirmCode`, `date`, `description`, `resellerId`')->from($table)->where("userId = '" . (int)$userid . "'");
            $result = $this->db->execute();
            if ($result->num_rows > 0) {
...
function checkNumberExist($country_code,$phone,$userId = '', $status = 0 , $type = 0)
{
//    $resellerIdServer = $this->getDomainResellerId($_SERVER['HTTP_HOST']);
    $status = 0;    
    #table name 
    ...
    $table = '91_verifiedNumbers'; 
    $this->db->select('`varifiedNumber_id`, `userId`, `domainResellerId`, `countryCode`, `verifiedNumber`, `confirmCode`, `verifiedDate`, `isDefault`, `description`, `resellerId`, `status`')->from($table)->where("countryCode = '" . $country_code . "' and verifiedNumber='".$phone."' and resellerId = '".$resellerId."'  ".$condition.' '.$userIdCon);
...
//private function validateCotactNumberOrUserName($particular)
//{
//    if(preg_match(NOTALPHANUM_REGX,$particular) || strlen($particular) < 5 || strlen($particular) > 40){
//        $this->msg = "Invalid user Information please provide a valid data";
//        $this->status = "error";
//        $this->code = "403";
//        
//        return false;
 ...
 * @return boolean
 */
function checkVerifiedNumberExist($particular,$resellerId)
{
//    $resellerIdServer = $this->getDomainResellerId($_SERVER['HTTP_HOST']);
//    $status = 0;    
    #table name 
...
//    error_reporting(-1);
    if(!$this->validateCotactNumberOrUserName($particular)){
        return false;
    }
    ...
    if(ctype_digit($particular))
    {
        ...
        $result = $this->getUserId($particular);
        if(!$result){
            $this->msg = "Error Invalid user please try again";
            $this->status = "error";
            $this->code = "4032";
        ...
        $condition = "userId = '" . $result . "' and isDefault=1 ";
         $type = 2;
    }
...
//    $resellerId = $this->getResellerId($userId);
    ...
    if($result && $resultCount < 1 && ctype_digit($particular))
    {
        //check the number exist as a user name in case of full number in particular
        $userId = $this->getUserId($particular);
        if(!$userId){
            $this->msg = "Error No record found for this user";
            $this->status = "error";
            $this->code = "4033";
            return false;
        }
        $condition = "userId = '" . $userId . "'  and resellerId=".$resellerId;
        //overwrite the result variable to use the below code
        $result = $this->selectData($coloumn,$table,$condition);
         $type = 1;
        //handling of result is under
    }
...
//            unset($row['confirmCode']);
            ...
            $updateConfirmCodeResult = $this->updateConformationCode($userId,$numberToSendMsg,1);
           
            if(!$updateConfirmCodeResult)
            {
                $this->msg = "Error unable to initilize the confirmation process";
                $this->status = "error";
                $this->code = "403";
                return false;
            }
            $this->sendSmsCall($row['verifiedNumber'],$row['countryCode'],$this->data,'SMS' );
...
//    $resellerId = $this->getResellerId($userId);
    ...
    $table = '91_verifiedEmails'; 
    $result = $this->selectData('userid',$table,"email = '" . $email . "' and resellerId ='".$resellerId."'");
...
function addnew_emailid($new_emailid, $userid  ) {
    ...
    #table name 
    $verifyEmail = '91_verifiedEmails';
    $this->db->select('`verifiedEmail_id`, `userid`, `email`, `confirm_code`, `default_email`, `newsFlag`, `date`, `resellerId`, `domainResellerId`')->from($verifyEmail)->where("email = '" . $new_emailid . "' and resellerId='".$resellerId."' ");
    $result = $this->db->execute();
    ...
    $tempEmail = '91_tempEmails';
    $this->db->select('`tempEmail_id`, `userid`, `date`, `email`, `confirm_code`, `domainResellerId`, `newsFlag`, `resellerId`')->from($tempEmail)->where("email = '" . $new_emailid . "' and resellerId = '".$resellerId."' ");
    $result = $this->db->execute();
	...
	if (strlen($parm['key']) > 0 || !preg_match('/[^0-9]+/', $parm['key'])) 
        {
            ...
            $this->db->select('`slNo`, `userId`, `domainResellerId`, `countryCode`, `tempNumber`, `confirmCode`, `date`, `description`, `resellerId`')->from($temptable)->where("userId = '" . $userid . "' and confirmCode = '".$key."'");
	    ...
	    if ($result->num_rows <= 0){
            ...
            $this->db->select('`varifiedNumber_id`, `userId`, `domainResellerId`, `countryCode`, `verifiedNumber`, `confirmCode`, `verifiedDate`, `isDefault`, `description`, `resellerId`, `status`')->from($table)->where("userId = '" . $userid . "'"); //userId = '" . $userid . "' and 
            $this->db->getQuery();
	    $result = $this->db->execute();
             if ($result->num_rows > 0){
                 $isdefault = 0;
    ...
    function addMeContact($userid) 
    {
        ...
        if ($result <= 0) 
        {
            ...
            if($confirmNo[0]['verifiedNumber'] == '' || $confirmNo[0]['verifiedNumber'] == NULL)
            {
                ...
                $meCode = $contactno['countryCode'];
            }
            else{
                $meContact = $confirmNo[0]['verifiedNumber'];    
                $meCode = $confirmNo[0]['countryCode'];
            }

            $data = array("userId" => $userid, "emailId" => $emailId , "contact_id" => new mongoId() ,"name" => htmlentities("me", ENT_QUOTES, 'UTF-8'), "email" => $emailId, "contactNo" => $meContact,"code"=>$meCode );
...
function makeDefaultNumber($parm,$userid){
    ...
    $this->db->select('`varifiedNumber_id`, `userId`, `domainResellerId`, `countryCode`, `verifiedNumber`, `confirmCode`, `verifiedDate`, `isDefault`, `description`, `resellerId`, `status`')->from($table)->where("varifiedNumber_id = '" . (int)$parm['contactId']."'");
...
function resendConfirm_code($parm,$userid){
    ...
    $this->db->select('`resendCode_id`, `type`, `userid`, `date`, `resend_by`, `user_ip`')->from($table)->where("userid = '" . $userid . "' and type = 1 and resend_by = 1 and date = '".date('Y-m-d')."'");
    $this->db->getQuery();
    $result = $this->db->execute();
    // processing the query result  
   // var_dump($result);
...
function callmeConfirm_code($parm,$userid){
    ...
    #find total time of resend confirm code and it is not more then 5 times
    $this->db->select('`resendCode_id`, `type`, `userid`, `date`, `resend_by`, `user_ip`')->from($table)->where("userid = '" . $userid . "' and type = 1 and resend_by = 2 and date = '".date('Y-m-d')."'");
    $this->db->getQuery();
    $result = $this->db->execute();
    // processing the query result  
...
function deleteEmailId($emailid,$userId)
  {
    ...
    $this->db->select('`verifiedEmail_id`, `userid`, `email`, `confirm_code`, `default_email`, `newsFlag`, `date`, `resellerId`, `domainResellerId`')->from($table)->where("userid = '" . $userId . "' and verifiedEmail_id  = '".$emailid."' and default_email = 1 ");
    $result = $this->db->execute();
    if ($result->num_rows > 0){
...
function getUserDefaultNumber($userId , $type = 0)
{
    ...
    $table='91_verifiedNumbers';
    #find total time of resend confirm code and it is not more then 5 times
    $this->db->select('`varifiedNumber_id`, `userId`, `domainResellerId`, `countryCode`, `verifiedNumber`, `confirmCode`, `verifiedDate`, `isDefault`, `description`, `resellerId`, `status`')->from($table)->where("userId = '" . $userId . "' and isDefault = 1 ");
    $result = $this->db->execute();
    if($result->num_rows > 0)
    {
        $row = $result->fetch_array(MYSQLI_ASSOC);
...
function deletephone($contactid,$userId,$status = NULL,$resellerId = NULL)
{
   ...
   // echo $contactid;
    ...
    $table='91_verifiedNumbers';
    #find total time of resend confirm code and it is not more then 5 times
    $coloumnName = '`varifiedNumber_id`, `userId`, `domainResellerId`, `countryCode`, `verifiedNumber`, `confirmCode`, `verifiedDate`, `isDefault`, `description`, `resellerId`, `status`';
    if($status == 1)
    {
        $this->db->select($coloumnName)->from($table)->where("userId = '" . $userId . "' and domainResellerId  = '".$resellerId."' and  isDefault = 1 and CONCAT(countryCode,verifiedNumber)= '".$contactid."'");
    }
    else 
    {
        $this->db->select($coloumnName)->from($table)->where("userId = '" . $userId . "' and varifiedNumber_id  = '".$contactid."' and isDefault = 1 ");
    }
...
function resendEmailConfirm_code($parm,$userid){
    ...
    #find total time of resend confirm code and it is not more then 5 times
    $this->db->select('`resendCode_id`, `type`, `userid`, `date`, `resend_by`, `user_ip`')->from($table)->where("userid = '" . $userid . "' and type = 2 and resend_by = 3 and date = '".date('Y-m-d')."'");
    $this->db->getQuery();
    $result = $this->db->execute();
    // processing the query result  
   // var_dump($result);
...
function getOneVerifiedEmail($userId)
{
    ...
    $verifyEmail = '91_verifiedEmails'; 
    $this->db->select('email')->from($verifyEmail)->where("userid = '" . $userId . "' and default_email=1");
    $result = $this->db->execute();
    // processing the query result           
    if ($result->num_rows > 0) 
    {
    ...
    $this->db->select('email')->from($verifyEmail)->where("userid = '" . $userId . "'")->limit(1);
    $result = $this->db->execute();
    // processing the query result           
    if ($result->num_rows > 0) 
    {
