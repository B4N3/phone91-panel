...
class phonebook_class extends fun
{
  ...
  function getAllContact($userid) 
     ...
     */
     function getMatchedContact($request,$session) {
        ...
        foreach ($result as $res)
        {
            ...
            if(preg_match("/^$q/",$res['name']) || preg_match("/^$q/",$res['contactNo']) || preg_match("/^$q/",$res['email']))
            {
                $data['label']= $res['name'];
                $data['value']=$res['contactNo'];
                $allcontact[] = $data;
            }
    ...
    function getUserAccessNumbers($userId , $contactNo=NULL)
    {
        ...
        {
            if(isset($res['accessNo']) && $res['hash'] == 100  && $res['contactNo'] != $contactNo )
            {
