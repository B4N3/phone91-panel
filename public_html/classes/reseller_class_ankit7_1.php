...
//            }
	...
	$sql="SELECT DISTINCT u.userId,u.userName, u.isBlocked, u.deleteFlag, u.type, u.beforeLoginFlag, u.sipFlag, u.userPin, u.callingStatus, u.password,ub.tariffId,ub.balance,ub.resellerId,pi.name FROM `91_userLogin` u JOIN `91_userBalance` ub ON u.userId=ub.userId JOIN `91_personalInfo` pi ON ub.userId = pi.userId $joinCond WHERE  u.type NOT IN (4,7,1) $condition  order by ub.balance desc limit $start_limit,$limit";
...
//		 die('dfd');
        ...
        if (isset($request['reNotUsingCall']) && $request['reNotUsingCall'] == 'on') {
            ...
            $rechargedButNotUsing.="JOIN 91_calls cl ON pi.userId=cl.id_client and cl.status != 'ANSWERED'";
