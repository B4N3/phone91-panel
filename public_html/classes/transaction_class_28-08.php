...
class transaction_class extends fun
{
    ...
    function addTransactional($fromUser,$toUser,$amount,$talktime,$paymentType,$description,$type,$partialAmt = 0){
        ...
        $result = $this->addTransactional_sub($fromUser,$toUser,$amount,$currentBalance,"voip91",$amount,0,$closingBalance,$description);
        ...
        if($type == "prepaid"){
            ...
            $result2 = $this->addTransactional_sub($fromUser,$toUser,$amount,$currentBalance,$paymentType,0,$amount,$closingBalance,$description);
        ...
        if($type == "partial"){
            ...
            $result2 = $this->addTransactional_sub($fromUser,$toUser,$amount,$currentBalance,$paymentType,0,$partialAmt,$closingBalance,$description);
...
//        $this->updateUserBalance($toUser,$currentBalance);
    ...
    function getPersonalTransaction($userid){
      ...
      $table = '91_transactionLog';
      #get data form transaction log table where form user and touser are given
      $this->db->select('*')->from($table)->where("fromUser = '" .$userid. "' OR toUser = '".$userid."'");
      $this->db->getQuery();
      $result = $this->db->execute();
    ...
    function addReduceTransaction($parm,$userid){
        ...
        $description = $parm['description'];
         ...
         #add  partial transaction with given payment type (cash,memo,bank or other).
         $result = $this->addTransactional_sub($fromUser,$toUser,$amount,$currentBalance,$paymentType,$debit,$credit,$closingBalance,$description);
         
         #update closing balance of user 
         $result3 = $this->updateClosingBalance($toUser,$closingBalance);
         
         if($result == 1){
             $str = $this->transactionTabledesign($fromUser,$toUser);
