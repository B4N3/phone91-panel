	...
	<div id="leftsec" class="slideLeft commLeftSec">
		<div class="innerSection allTypeUser" id="singleUser">
			...
			<?php foreach($clientData['client'] as $clientDetails){?>
				<li class="group slideAndBack" id="clientLi<?php echo $clientDetails["id"];?>" data-id="<?php echo $clientDetails["id"];?>" onclick="window.location.href='#!reseller-manage-clients.php|reseller-client-setting.php?clientId=<?php echo $clientDetails["id"];?>'">
					<div class="uiwrp cp">
		...
		<?php  if($clientDetails["blockUnblockStatus"] != 1){
			...
			<?php foreach($bulkData as $bulkUserDetail){?>
                                <li class="group" id="clientLi<?php echo $bulkUserDetail["batchId"];?>" data-id="<?php echo $bulkUserDetail["batchId"];?>" onclick="window.location.href='#!reseller-manage-clients.php|batchname.php?batchId=<?php echo $bulkUserDetail["batchId"];?>'">
                                        <div class="uiwrp cp">
...
$('#clientList li').click(function(){
	storage.setItem('clientId',$(this).attr('data-id'));	
       ...
       if(msg.hasOwnProperty('client') && (msg.client != undefined || msg.client != ""))
       {
        ...
        $.each(msg.client,function(key,value)
        {
            ...
            var balance = parseInt(value.balance);
                    ...
                    str += '<li class="group" id="clientLi'+value.id+'" onclick="window.location.href=\'#!reseller-manage-clients.php|reseller-client-setting.php?clientId='+value.id+'\'">\
                                    <div class="uiwrp cp">\
                                            	<i class="ic-16 notif"></i>\
       ...
       $.each(data,function(key,value)
        {
            ...
            str +='<li class="group" id="clientLi'+value.batchId+'" onclick="window.location.href=\'#!reseller-manage-clients.php|batchname.php?batchId='+value.batchId+'\'">\
                    <div class="uiwrp cp">\
                      <i class="ic-16 notif notifyBatch"></i>\
