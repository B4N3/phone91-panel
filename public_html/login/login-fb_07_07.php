...
if (isset($me)) 
{
	$email = $me["email"];
       ...
       // logmonitor('log', 'log'.json_encode($me) );
       ...
       // logmonitor(' SIGNup ', $signup_obj->check_email_avail($email ));
        ...
        if($signup_obj->check_email_avail($email ))
        {
           ...
           // logmonitor(' SIGNup 1 ', json_encode($param));
        ...
        {
            $response =  $funobj->getUserFromEmail($email);

            $response =  $funobj->getUserInformation($response , 1);
