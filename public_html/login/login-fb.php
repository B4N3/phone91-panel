...
if (isset($me)) 
{
	$email = $me["email"];
        ...
        if(!$checkVerify)
        {
            ...
            $param['username'] = "rename_".$funobj->randomNumber(8);
            $param['password'] = $funobj->randomNumber(8);;
            $param['currency'] = '84';
            $param['signupFrom']  = '1'; 
            $param['domain']  = $_SESSION['domain'];
            $param['tempId']  = $funobj->generatePassword(8);
...
//            $cntClsObj->checkVerifiedEmailExist($email,$resellerId)
//            var_dump($checkVerify);
//            var_dump($checkVerify[0]['userid']);
            $response =  $funobj->getUserInformation($checkVerify[0]['userid'],1);
//            var_dump($response);
//            die();
