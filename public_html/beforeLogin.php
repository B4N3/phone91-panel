...
$commomFun = new commonFunction();

//if session user set then redirect to userhome
if(isset($_SESSION['id']))
{

    $beforeLogin = $commomFun->getBeforeLoginFlag($_SESSION['id']);   
    
   
    
    if($beforeLogin)
        $funobj->redirect("/userhome.php#!contact.php");       
}
...
$hisVerifiedNo = (  isset($_REQUEST['verifiedNumber']) && !preg_match(NOTNUM_REGX,$_REQUEST['verifiedNumber'] ) ? $_REQUEST['verifiedNumber'] : '');
$countryCode = (  isset($_SESSION['countryCode']) && !empty($_SESSION['countryCode'] ) ? $_SESSION['countryCode'] : 0 );
?>
<!DOCTYPE HTML>
<html>
    <head>
                        ...
                        <div class="signUpfields">  
                            ...
                            <select tabindex="1"  name="location" id="location" class="besnlodrdo">
                                ...
                                foreach ($country as $key => $countryNames) {
                                    ...
                                    if(!empty($countryCode) && $countryCode == $key )
                                    echo "<option selected='selected' value='$key'>$countryNames</option>";
                                    else
                            ...
                            <input name='countryCode' value="<?php echo $countryCode; ?>" type="text" id="code"  class="besnlodrdocode" onkeyup="selectOption($(this).val())" />
                            <input type="text" name='mobileNumber' class="besignnumber" id='mobileNumber' placeholder="Phone number" value="<?php echo $hisVerifiedNo; ?>" />
    ...
    <?php
//    echo _DOMAIN_THEME_;
    if(_DOMAIN_THEME_ == "voip" || _DOMAIN_THEME_ == "defaultTheme") include_once('inc/incFooter.php'); ?>
