...
<div class="logWrap">
	<div class="logLeft">
		...
		<div class="logGrid">
			...
			<div class="cols2 col">
				<div class="colInner">
				 <div style="width: 205px;position: relative;top: 29px;left: 420px;" id="changeSearchType"><span>Select Criteria: </span> 
                        <input type="radio" name='searchType' class='searchcri' checked value="1">Date
                        <input type="radio" name='searchType' class='searchcri' value="2">Monthly
                    </div>			
					<div id="getResellseTimelineDetailsGraphTitle" class="gridTitle">Timeline</div>
					<div id="getResellerTimelineDetailsGraphContainer"></div>
				</div>
			</div>
			<div class="cols2 col countryGraph" style="width:100%;">
				<div class="colInner">			
					<div id="getResCountryPieDetailsGraphTitle" class="gridTitle">Country Call Log</div>
					<div id="getResCountryPieDetailsGraphContainer">
						 <iframe id="countryifrm" src="<?php echo PROTOCOL.HOST_NAME;?>/resellerCountryLog.php?fromDate=&toDate=" style="width:1400px;height:600px;position: relative;top: 1px;left: 51px;border-width: 0px;">
						 </iframe>
						</div>
					</div>
				</div>
...
function getDetails(userId, type,fromDate,toDate)
{
	$.ajax({
		...
		success: function(response)
		{
        	...
        	getResellerTimeLineDetail('getResellerTimeline',searchType);
			...
			$.each(response, function(key, item ) {
					...
					callBackDrawGraph(item, key,'pie','');
...
function showChartByDate(){
    ...
    getDetails(<?php echo $_SESSION['id']; ?>, 1,fromDate,toDate);
     ...
     $('#countryifrm').attr('src','<?php echo PROTOCOL.HOST_NAME;?>/resellerCountryLog.php?fromDate='+fromDate+'&toDate='+toDate);
...
/**
    ...
    */
    function getResellerTimeLineDetail(action,searchType)
    {
        $.ajax({
            url: "/controller/userCallLog.php",
            data: {"call": action,
            ...
            success: function(response)
            {
                ...
                callBackDrawGraph(response, 'getResellerTimelineDetails','line',xText);
    ...
    function callBackDrawGraph(data, call,chartType,xText) {
        ...
        switch(chartType)
        {
        	...
        	case 'line':
        	{
        		console.log(data.data.dayArr);
        			categories = data.data.dayArr;
                	series = [{name:'My Call Duration',
                          data:data.data.totalCallDuration},
                          {name:'Customer call duration',
                           data:data.data.customerCallDuration },
       ...
       console.log('#'+call+'GraphContainer');
		createHighchart('#'+call+'GraphContainer',chartType,call,series,categories,xText);		
		...
		switch(call)
		{
			...
			case 'getResellerTimelineDetails':
			title = 'Timeline';
			break;		
		}
    ...
    function createHighchart(id,chartType,Title,dataArray,datas,xText)
    { 
