...
//	}
        ...
        $actUserCharge = base64_decode($reqPara['charge']);
         ...
         { 
            $actUserCharge = (int)$funobj->currencyConvert($convertCurrency,$userCurrency,$recharge);
         } 
                 
        //get order details by order id
        $query = "SELECT * FROM confirmOrder WHERE order_id='$orderId'";
        
        $rs1 = mysqli_query($db,$query) or $error=mysqli_error();
       ...
       if($error!="") 
       {
        ...
        $dbIp = $row['ip'];
        //get details from clientsshared
        ...
        if($dbIp == $ip and $dbRechare == $actUserCharge)//validate with ip and talktime and recharge amount
        {
            //insert details in transactoin tables
            $sqlP = "insert into transactions(userId,date,amount,description,transactionId,orderId,balance,paymentType) values('$id_client',now(),$actUserCharge,'Automatic recharge by paypal','$txn_id','$orderId',$totalBal,'paypal')" ;
            mysqli_query($db,$sqlP) or $error=mysqli_error();
            //update balance
            $status= $funobj->updatebalance($db,$id_client,$dbTalktime,'Add');
            if($status != 'success')
            {
                //send mail via mandrill
                $mailTo = array('AnkitPatidar@hostnsoft.com');
                $subject = "error occur while balance update";
                $message = $status.'tt'.$dbTalktime.'id'.$id_client.'ac'.$actUserCharge.print_r($reqPara,1);
                if(!$mailerr->sendmail_mandrill($mailTo, $subject, $message,$buninessEmail))
                {
                    $mailerr->errorHandler('problem in mail:\nmailto'.print_r($mailTo,1).'\nsub:'.$subject.'\nmsg:'.$message);
                }
        ...
        //$txn_id = $orderId;
        $curSql = "SELECT name FROM currency_names WHERE id='$cid'";
        $result=mysqli_query($db,$curSql);
        $get_userinfo=mysqli_fetch_array($result);
	...
	$mail_msg="One phone91 user with id ".$userName." and paypal emailid ".$payer_email." did paypal payment of ".$recharge." ".$mc_currency." and got recharged by ".$dbTalktime." ".$currency.' tranx id:'.$txn_id;
	//send mail via mandrill
