        ...
        else 
        {
            ...
            if($result && $result->num_rows > 0)
            {
                ...
                if(isset($_SESSION['domain']))
                {
                    ...
                    if(isset($_SESSION['otherLogin']) &&  $_SESSION['otherLogin'] == '1')
                    {
                        
                        $parameters = '?msg=101&error='.base64_encode(base64_encode(json_encode( array( base64_encode(base64_encode($_SESSION["signup_email"])) ,base64_encode(base64_encode($_SESSION["signup_first_name"])), base64_encode(base64_encode($_SESSION["signup_last_name"]) )))));
                    }
                    
                    header("location:"."http://".$_SESSION['domain']."/signup-step.php".$parameters);
                    exit();
                }
                else
                    //return 1;
...
//end delete_client() function
    ...
    function checkEmail($email , $redirectUrl=NULL) 
    {
        ...
        if ($userid) 
        {
            ...
            if(isset( $redirectUrl) && $_SESSION['domain'] == 'phone91.com' && !empty($redirectUrl))
            {
               $response =  $this->validateBeforeLogin();
               
                unset( $_SESSION['page'] );
                header("location: http://".$redirectUrl."?msg=". base64_encode(base64_encode(json_encode($_SESSION))));
                exit();
            }  
            else 
            {
...
//       echo $this->querry;
  ...
  function login_user($userId, $pwd, $remember_me,$host = NULL,$signUp = 0) 
  {
        ...
        $response = $this->checkValidation();
      
        if(!$response)
        {
           
            header("location:".$host."?error=".$_SESSION['error']);
        }
        ...
        if ($result->num_rows > 0 ) 
        {
           ...
           // print_r($getUserInfo);
            ...
            if($host == 'http://phone91.com/basic.php' || $host == 'http://phone91.com/reseller-api.php' || $host == 'http://phone91.com/two-way-calling-api.php' )
            {
               $_SESSION['domain'] = 'phone91.com'; $_SESSION['currentHost'] = 'phone91.com';;
                header("location: ".$host."?msg=". base64_encode(base64_encode(json_encode($_SESSION))));
                exit();
            }
        ...
        $mailData = <<<EOF
            ...
            <div style="width:625px; margin:0 auto;  background:#fff;">
                ...
                <div align="center" style="background-color: #FBFBFB; border: 1px solid #F8F8F8;  color: #666666; font-size: 15px; line-height: 29px; margin: 20px 0; padding: 10px;">
                             ...
                             <a style=" color:#296FA2; text-decoration:none" href="#">Link will come here...</a><br/>	
