   ...
   $i = 0;
     foreach($responseArr['feed']['entry'] as $contact)
     {
         ...
         if(isset($numberArr['$t']) && $numberArr['$t'] != "")
         {
            ...
            if(!in_array($number, $parm['contact']) && $number != '')
	    {
		
		if(strpos($number,'+') === FALSE)
		{
		     $parm['countryCode'][$i] = 91;
              $parm['contact'][$i] = trim($number);
		}  
		else 
		{
		    $gNumberArr = explode(' ', $number);
		     $parm['countryCode'][$i] = substr($gNumberArr[0], 1,strlen($gNumberArr[0])-1);
		     if(isset($gNumberArr[1]))
                $parm['contact'][$i] = trim($gNumberArr[1]);
             else
                $parm['contact'][$i] = trim($gNumberArr[0]);
	       ...
	       #all name array   
            $parm['name'][$i] = trim($name);
            #all email array
            $parm['email'][$i] = trim($emailId);
           }
//           //
//            else
//              $parm['contact'][$i] = '';
...
//    print_r($parm);
//    die('fkdjf');
     $queryString =  http_build_query($parm); 
    //$phnbClsObj = new phonebook_class();
     
    apc_add('importData', $parm);
...
//    header("location: http://voip91.com");
    header("location:".PROTOCOL.$httpHostArr[0].'/userhome.php#!importContact.php');
    exit();
}
...
function get_oauth2_token($grantCode,$grantType) 
{
    ...
    //set post fields
    $clienttoken_post = array("client_id" => GCLIENTID,
    ...
    if($grantType === "online")
    {
        ...
        $clienttoken_post["redirect_uri"] = GREDIRECTURL;
