		...
		<div class="innerSection allTypeUser" id="singleUser" style="top:53px;">
			...
			<?php foreach($clientData['client'] as $clientDetails){                          
					...
					<div class="uiwrp cp">
						...
						<span title="Login As" class="ic-24 login loginAs clientLi<?php echo $clientDetails["id"].' '.$hideIcons;?>" onclick="var url = window.location.hash;window.location.href='/controller/signUpController.php?call=loginAs&userId=<?php echo $clientDetails['id']; ?>&url='+url.substring(1)" ></span>
                                    <?php }
                                     ?>            
       ...
       if(msg.hasOwnProperty('client') && (msg.client != undefined || msg.client != ""))
       {
        ...
        $.each(msg.client,function(key,value)
        {
            ...
            if(value.deleteFlag == 0){
                ...
                loginAs='<span title="Login As" class="ic-24 login loginAs clientLi'+value.id+'" onclick="window.location.href=\''+hrefLocation+'\'"></span>';
            }
            else
            {
            	hideIcons = 'dn';
