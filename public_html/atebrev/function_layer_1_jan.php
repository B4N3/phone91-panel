...
//        $this->db = new SuperMySQLi('localhost', 'voipswitchuser', '+4H8ZXcSyWn7CuX*', 'voipswitch');
    ...
    public function validateBeforeLogin() {
        ...
        if(isset($_SESSION['loginFlag']) && !empty($_SESSION['loginFlag']))
        {
            ...
            {
                header("location:".$_SESSION['domain']."/signup-step.php");
            }else
        ...
        else 
        {
            ...
            if($result && $result->num_rows > 0)
            {
                ...
                if($_SERVER['HTTP_HOST'] == "phone91.com" || $_SERVER['HTTP_HOST'] == "10.42.43.174")
                {
                    header("location:".$_SESSION['domain']."/signup-step.php");
                }else
                    header("location: /beforeLogin.php");
...
//end delete_client() function
  ...
  function login_user($userId, $pwd, $remember_me,$host = NULL,$signUp = 0) 
  {
        ...
        {			
               ...
               if($host != NULL)
               {
                   header("location:".$host."/sign-in.php");
               }
               else
...
//    }
    ...
    function verifyCode($code,$number) 
    {
        ...
        if(!is_numeric($code) || !is_numeric($number))
...
//                    $result = mysql_query("select tempNumber,countryCode from 91_tempNumbers where userId=" . $uid) or die("Error");
    ...
    private function sendSmsCall($contactNo,$countryCode,$confirmCode,$smsCallFlag)
    {
...
//              $updRes2 = $this->updateData($data, "91_tempNumbers" , $condition);
    ...
    public function forgotPassword($userName,$smsCall)
    {
        if(preg_match('/[^a-zA-Z0-9]+/',$userName) || $userName == "")
        {
        ...
        if(is_numeric($userName) || ctype_digit($userName))
        {
            ...
            $resultVerSel = $this->selectData('userId,verifiedNumber,countryCode,confirmCode', '91_verifiedNumbers',"verifiedNumber='". $userName ."' and domainResellerId = '".$resellerIdServer."'");
            $verifiedCount = $resultVerSel->num_rows;
                
            if(!$resultVerSel || $verifiedCount < 1)
            {
                $userName = substr_replace($userName, "X", 4, 3);
                $showMsg = "Sorry, $userName is not registered with us. You can register by SignUP";
        ...
        else
        {
           ...
           if(!$userId)
           {
               $showMsg = "Sorry, username ".$userName." is not registered with us. You can register by SignUP";
               $status = "error";
           }
           else
           {
...
//        mysqli_close($con);
    ...
    function send_verification_mail($newEmailid, $pwd,$balance,$currencyId) 
    {
 ...
 */
   ...
   function getDomainResellerId($domianName)
    {
        /* @author sameer rathod 
        ...
        $result = $this->selectData("resellerId", "91_domainDetails","domain like '".$domianName."'");
        if($result && $result->num_rows > 0)
        {
            $row = $result->fetch_array(MYSQLI_ASSOC);
