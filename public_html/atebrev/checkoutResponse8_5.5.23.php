...
$funObj->sendErrorMail(DEFEMAIL1,'api details'.print_r($PayPalConfig,1).' dfdf '.print_r($_SESSION,1).'REQUEST'.print_r($_REQUEST,1));
...
switch ($action)
{
    case 'process':
    {
        ...
        $GECDResult = $PayPal -> GetExpressCheckoutDetails($_REQUEST['token']);
        //$GECDResult = $PayPal -> GetExpressCheckoutDetails($_SESSION['PayPalResult']['TOKEN']);

        //mail details
        $funObj->sendErrorMail(DEFEMAIL1,'order details'.print_r($GECDResult,1));
        ...
        $Payment = array(
                    ...
                    'notifyurl' => NOTIFY_URL_EC.'?action=ipn',  						// URL for receiving Instant Payment Notifications
       ...
       elseif( $response['ACK'] != 'Success') //check for success
       {
           ...
           //apply tracker
            ...
            header("location: https://".$_SERVER['HTTP_HOST']."/userhome.php#!setting.php|buymore.php");
            exit();
...
//       }
        ...
        if($paymentStatus == 'Reversed') //Reversed
	{
        ...
        if($dbIp == $ip and $recharge == $actUserCharge)//validate with ip and talktime and recharge amount
        {
            ...
            if($updatedFlag == 0)
            {
                trigger_error('Balance not updated while paypal Debit Credit  recharge Avialable details:userId:'.$idClient.' talktime:'.$dbTalktime);
                $sub = 'Balance not updated while paypal Debit Credit recharge';
            ...
            $updatedClosingBalance = $closingBalance-$dbRechare;
            //maintain transaction log
            $tranxObj->addTransactional_sub(0,$balance,'paypal',0,$dbRechare,$updatedClosingBalance,'txnId:'.$txnId,$currencyId);
	...
	//send mail via mandrill
        $subject = "Phone91 Paypal payment By Express Checkout";
        $message = $mailMsg;
    ...
    case 'ipn':
    {
        $funObj->sendErrorMail(DEFEMAIL1,'api IPN'.print_r($_REQUEST,1));
        break;
    }
