...
//         print_R($data);
    ...
    function getUserCurrencyAndRate($userInfo , $currency )
    {
        ...
        if($userId)
        {
            ...
            return json_encode(array("msgStatus" => "success" , "message" => ""  , "content" => array("currency" => $usersCurrency , "rate" => $rate , "userId" => $userId)));

        }
        else 
        {
             return json_encode(array("msgStatus" => "error" , "message" => "No user found for this Information." ));
        }
    }
    ...
    function getUserLoginDetails($userId)
    {
        ...
        $userInfo = $this->selectData('*','91_userLogin',$condition); //
