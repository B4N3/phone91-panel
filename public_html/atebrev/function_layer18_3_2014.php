...
//        var_dump($resellerId);
    ...
    {
        $url = "https://voice.phone91.com/currency/index.php?from=$from&to=$to&amount=$amount";  //nedd to change after 1500 request per month
        $ch = curl_init($url);
...
//end delete_client() function
    ...
    function checkLogin($userName, $pwd) 
    {
        ...
        $result = $this->selectData('userId,userName,password,isBlocked,deleteFlag,type,resellerId,sipFlag','91_manageClient',"userName='" . $userName . "' and password='" . $pwd . "'");
//       echo $this->querry;
        return $result;
     ...
     * @return boolean
     */
  function login_user($userId, $pwd, $remember_me,$host = NULL,$signUp = 0,$tempVar= false) 
  {
        session_start();
        session_unset();
        session_destroy();
...
//        mysql_close($dbh);
    ...
    function SendSMSUSDnew($tempparam) 
    {
        ...
        $apiId = "3451976";
        if(isset($param["apiId"]) && !empty($param["apiId"]))
        {
              $apiId = $param["apiId"];
        }
            
        $connect_url = "https://api.clickatell.com/http/sendmsg?user=phone91&password=".$password."&api_id=".$apiId."&to=".$param["to"]."&text=".$param["text"]."&concat=4"; // 
    ...
    function SendSMS91($tempparam) 
    {
        ...
        //prepare details for curl
        $connect_url = "http://control.msg91.com/sendhttp.php"; // Do not change
        ...
        { //check in tempcontact table code exist or not
           ...
           // $resultTemp = $this->selectData("userId","91_tempNumbers","CONCAT(countryCode,tempNumber) = '".$number."' and confirmCode=".$code);
            ...
            if($type == 'EMAIL')
                 ...
                 $resultTemp = $this->selectData("userId","91_tempNumbers","CONCAT(countryCode,tempNumber) = '".$number."' and confirmCode='".$code."' domainResellerId=".$resellerIdServer);
...
//              $updRes2 = $this->updateData($data, "91_tempNumbers" , $condition);
    ...
    public function forgotPassword($userName,$smsCall ,$countryCode = NULL)
    {
        ...
        else if($smsCall == 'MAIL')
        {
            ...
            $resultVerSel = $this->selectData('verifiedEmail_id,userid,email,confirm_code,default_email', '91_verifiedEmails',"email='".$userName."' and domainResellerId = '".$resellerIdServer."' ");
            $verifiedCount = $resultVerSel->num_rows;
            #- modification ends.
        ...
        $mailData = <<<EOF
            ...
            <div style="width:625px; margin:0 auto;  background:#fff;">
                ...
                <div align="center" style="background-color: #FBFBFB; border: 1px solid #F8F8F8;  color: #666666; font-size: 15px; line-height: 29px; margin: 20px 0; padding: 10px;">
                            ...
                            <a href="https://voice.phone91.com/verify_email.php?email=$newEmailid&confirmationCode=$pwd" style=" color:#296FA2; text-decoration:none">Confirm</a><br/>	
                             <div style="font-size:20px; padding:10px 0">OR</div>	
                            You can alternatively verify it by entering the confirmation code: xxxx in your account settings option. 	
                </div>
                    <div>
...
EOF;
    ...
    function mobile_verification_api($mobile_no, $vcode) {
        $connect_url = 'https://voice.phone91.com/phone91_verification/mobile_verify_api.php'; // Do not change
    ...
    {
        $connect_url = 'https://voice.phone91.com/phone91_verification/mobile_verify_response.php'; // Do not change
        $param["uid"] = $uid; 
...
//                    $insertVerifySipArr = array("userId"=>$userId,"userName"=>$userName,"passwd"=>$password,"chainId"=>$sipChainId,"isCallShopUser"=>0);
    ...
    function generateZipFile($param)
    {
        ...
        if ($zip->open('zip/'.$zipName,  ZipArchive::CREATE)) 
        {
            ...
            return json_encode(array("msgStatus" => "success" , "message" => "Zip file successfully" ));
        } 
        else 
        {
...
//972,39,876,81,962,7,254,965,996,856 ,371,961,370,352,389,60,223,356,230,52,373,212,258,95,264,977,31,599,64,505,227,234,47,968,92,507,675,595,51,63,48,351,974,40,7,250,966,221,381,65,421,386,27,34,94,46,41,886,992,255,66,228,868,216,971);
        ...
        $allNumbersArray = array(376,971,93,1268,1264,355,374,599,244,672,54,1684,43,61,297,994,387,1246,880,32,226,359,973,257,229,590,1441,673,591,55,1242,975,267,375,501,1,61,243,236,242,41,225,682,56,237,86,57,506,53,238,61,357,420,49,253,45,1767,1809,213,593,372,20,291,34,251,358,679,500,691,298,33,241,44,1473,995,233,350,299,220,224,240,30,502,1671,245,592,852,504,385,509,36,62,353,972,44,91,964,98,354,39,1876,962,81,254,996,855,686,269,1869,850,82,965,1345,7,856,961,1758,423,94,231,266,370,352,371,218,212,377,373,382,1599,261,692,389,223,95,976,853,1670,222,1664,356,230,960,265,52,60,258,264,687,227,234,505,31,47,977,674,683,64,968,507,51,689,675,63,92,48,508,870,1,351,680,595,974,40,381,7,250,966,677,248,249,46,65,290,386,421,232,378,221,252,597,239,503,963,268,1649,235,228,66,992,690,670,993,216,676,90,1868,688,886,255,380,256,1,598,998,39,1784,58,1284,1340,84,678,681,685,381,967,262,27,260,263);
    ...
    function longCodeResellerId($accessNumber)
    {
        ...
        $accNoResult = $this->selectData("resellerId"," 91_longCodeNumber","longCodeNo =".$accessNumber);
        ...
        return  json_encode(array("msgStatus" => "success" , "resellerId" => $resellerId ));;
    }
    ...
    {   
        ...
        foreach($result['userIpAddress'] as $key => $value)
        {
            ...
            if ($value) 
            {
                $sqlMan .= "('".$_SESSION['userid']."','".trim($result['type'][$key]) ."','" .trim($value). "','". trim($result['reason'][$key]) ."'),";
            }
        }
