...
function edit_voiceper(id,val)
{
	var per=$("#voiceper_"+id).val();
	$.ajax({
		type: "POST",
		url: "../action_layer.php?action=111",
		data: "id="+id+"&per="+per,
		success: function(msg){
		show_message("Sent Percentage Updated","success");
		}
...
{
	deleteCookie(c_name);
	var value=document.getElementById(c_name).checked;
	var exdate=new Date();
	var exdays=1;
