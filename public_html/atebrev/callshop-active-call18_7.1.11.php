            	...
            	<div class="col-5">
              		<div class="addform"> 
                        <form name="addSystemForm" id="addSystemForm" action="" method="post">
                            ...
                            <div class="rdrow"><input type="radio" id="messenger" name="sysID" value="1" onclick="messengerSetting();$('#submitButtonCS').show();$('#submitButtonCS').val('Add System');$('#messengerDiv').show();"/><label for="messenger">Messenger Id</label></div>
...
function editSystemDetails(systemId,callShopId)
{
    $.ajax({
        ...
        success: function(response){
            ...
            $( "#edit-shop-dialog" ).dialog({ modal: true, resizable:false, width:600, height:445,close: function(event, ui) 
                                        { 
                                            $('#edit-shop-dialog').dialog('destroy');
                                        }
                                 });
...
function getCallShopSummary(callShopId,type,systemIdArr,systemId,systemName)
{
	...
	$('#addshop').unbind('click').click(function(){
		$( "#add-shop-dialog input" ).not(':button,:hidden,:submit').val("");
		$( "#add-shop-dialog" ).dialog({ modal: true, resizable:false, width:600, height:445,close:function(event,ui){ $('#add-shop-dialog').dialog('destroy').remove(); }});
	})
