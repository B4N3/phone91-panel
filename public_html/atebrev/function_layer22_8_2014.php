...
//    }
    ...
    public function validateAdmin() {
        if($this->check_admin())
        {
            header("location:"."http://".HOST_NAME."/admin/index.php");
            exit();
        }
        else
            return 0;
...
//        var_dump($resellerId);
    ...
    function currencyConvert($from, $to, $amount) 
    {
        ...
        if(CALLSERVERURL == 'https://192.168.1.191/')
...
//end delete_client() function
    ...
    function checkLogin($userName, $pwd,$type=null) 
    {
        ...
        if(is_null($type))
        {
            ...
            $condition = "userName='" . $userName . "' and password='" . $pwd . "'";
        }
        else if(1 == $type)
        {
...
//        $pwd = $this->db->real_escape_string($pwd);
        ...
        $result = $this->selectData('userId,userName,password,isBlocked,deleteFlag,type,resellerId,sipFlag,tariffId,beforeLoginFlag','91_manageClient',$condition);
     ...
     * @return boolean
     */
  function login_user($userId, $pwd, $remember_me,$host = NULL,$signUp = 0,$tempVar= array(),$domainName = '') 
  {
        session_start();
        session_unset();
        session_destroy();
...
//            die();
    ...
    function loginUserForLoginAs($userName,$host = NULL,$tempVar= array())
    {
        ...
        if(!empty($tempVar))
         ...
         trigger_error('enter in loginAs2:'.$userName); 
        $getUserInfo = array();
        $getUserInfo = $this->getUserInformation($userName);
        
        trigger_error('newUser3:'.json_encode($getUserInfo));
        if (!empty($getUserInfo)) 
        {
            ...
            trigger_error('newUser4:'.json_encode($getUserInfo));
            #check batch user expiry date 
            if($getUserInfo['type'] == 4 && !isset($_SESSION['tempVar']))
            {
                $batchExpiryDate = $this->getUserBatchExpiryDate($getUserInfo['userId']);
            ...
            trigger_error('newUser5:'.json_encode($getUserInfo));
            #function call for assign value of session 
            $this->initiateSession($getUserInfo["userId"]);
...
//    }
     ...
     * @return int
     */
    function verifyCode($code,$number,$type=NULL, $resellerId = NULL ) 
    {
...
//              $updRes2 = $this->updateData($data, "91_tempNumbers" , $condition);
    ...
    public function forgotPassword($userName,$smsCall ,$countryCode = NULL , $userId = NULL)
    {
        ...
        if(is_numeric($userName) || ctype_digit($userName))
        {
            ...
            if(isset($userId) && !empty($userId))
                $condition =  "userId = '".$userId."' and domainResellerId = '".$resellerIdServer."' ";
            else
                $condition =  "domainResellerId = '".$resellerIdServer."'";
        ...
        else
        {
           ...
           else
           {
               ...
               ///$resellerId = 2;
               $resultVerSel = $this->selectData('verifiedNumber,countryCode,confirmCode , resellerId ,'.$userIdField, '91_verifiedNumbers',"".$userIdField."='" . $userId . "'");
   ...
   function getDomainResellerId($domianName,$type = 1)
    {
       ...
       if( $type == 3 )
        ...
        else
        {
            ...
            $condition = " domainName like '".$domianName."' ";
        }
...
//                        sip_add($userName,$password);
                        $sqlBuddies = "INSERT INTO sip_buddies (name,sippasswd) values ('".$userName."','".$password."') on duplicate key update sippasswd='".$password."'";
...
//                            ob_end_clean(); 
    ...
    function getUserInformation($userName , $reqFlag = NULL)
    {
        ...
        trigger_error('enter in loginAs3:'.$userName); 
       ...
       //apply query for login flag
        ...
        $result = $this->selectData("*", "91_userLogin",$condition);
         trigger_error('enter in loginAs4:'.$userName); 
        trigger_error('newUser2:,qur:'.$this->querry);
       //validate result
       if($result && $result->num_rows > 0)
       {
