...
class actionClass {
   ...
   function showEditContact($request, $session){
    ...
    $pbookobj = new phonebook_class();
    echo $result = $pbookobj->showEditContact($request); 
   ...
   function addReduceTransaction($request, $session){
	...
	echo $msg=$trans_obj->addReduceTransaction($request,$userid);
   ...
   function editClientInfo($request, $session){
       ...
       $client_obj = new reseller_class();
       $userid=$session["id"];	
       echo $msg=$client_obj->editClientInfo($request,$userid); 
   ...
   function feedbak($request, $session){
        ...
        if(strlen($request['mailid'])>8) //if user have his or her email then mail send by user email
        {
                $header .= 'From: '.$request['mailid']."\n"	;	
                 $header .= "Reply-To: ".$request['mailid']."\r\n";
        }
        else
        {
        ...
        mail("business@phone91.com,shubh124421@gmail.com","Feedback","Name:".$request['mailid']."\n
            Username/Number ".$request["number"]."\n
            Message:".$request['msg'],$header);
    ...
    function verifyConfirmation($request, $session){
       ...
       // echo $code;
        ...
        if(isset($_REQUEST['domain']) && $_REQUEST['domain'] == 'phone91.com')
        {
            ...
            header('Location: http://phone91.com/forget-password.php?error='.$errorMessage);
        }
        echo $response;
    }
...
//        die();
   ...
   function signupP91($request, $session){
     ...
     if($msg->status == "success")
         ...
         header("Location: "."http://".$domain."/signup.php?msg=".$msg->msg."&status=".$msg->status);
     }
     exit();
   }
   ...
   function resetClientPassword($request, $session){
    ...
    $check_parrent=$res_obj->checkParentReseller($request,$session);
    //    if($pro_obj->check_admin() || $check_parrent)
    if($check_parrent)
    {
