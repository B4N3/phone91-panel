...
class userCallLog {
    ...
    function getStatusDetails($request, $session)
    {
        ...
        if(!$chainId)
        {
         ...
         if(!isset($request['fromDate']) || $request['fromDate']==''){
            $sdate  = date('Y-m-d',strtotime("-250 days")); 
        }else
    ...
    function getCallViaDetails($request, $session)
    {
        ...
        if(!$chainId)
        {
         ...
         if(!isset($request['fromDate']) || $request['fromDate']==''){
            $date  = date('Y-m-d',strtotime("-250 days")); 
        }else
    ...
    function getResellerLossDetails($request, $session)
    {
        ...
        return $logClsObj->getResProfitDurationGraphDetails($chainId,$date,"loss");
    ...
    */
    ...
    function getCallDetailsAdmin($request, $session){
        ...
        $result = $logClsObj->getCallDeatilsAdmin($userId,$keyword,$type,$route,$status);
