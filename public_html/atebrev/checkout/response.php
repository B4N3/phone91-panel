  ...
  switch ($root) 
  {
      case "authorization-amount-notification": {
          ...
          if($data[$root]["order-summary"]["risk-information"]["eligible-for-protection"]["VALUE"])
          {
            ...
            if(!isset($orderId))
            {
                mail("ankkubosstest@gmail.com","google checkout","problem with orderId id in response.php");
...
//              }
              ...
              if($talktime == $dbTalktime and $dbIp == $ip and $dbRechare == $actUserCharge)//validate with ip and talktime and recharge amount
              {
                    ...
                    if($status != 'success')
                        mail('ankkubosstest@gmail.com','error occur while balance update',$status);
                    else
                    {
        ...
        if($error!="")
             ...
             //get currency
              ...
              //set msg to mail
              $mail_msg="One phone91 user with id ".$user_uname." and Googlecheckout emailid :".$buyerEmail." did google checkout payment of ".$amount." ".$currency." and got recharged by ".$talktime." ".$currency;

              mail("ankkubosstest@gmail.com","Phone91 GoogleCheckout",$mail_msg);
              mail("shubh124421@gmail.com","Phone91 Googlecheckout",$mail_msg);

             mail("ankkubosstest@gmail.com","in responce.php","response14".$sqlPay.'::'.$sqlClient.'orderval:'.$orderTotal.'sql::'.$sql.print_r($get_userinfo,1));
            }
