      ...
      case "authorization-amount-notification": {
         ...
         // if($data[$root]["order-summary"]["risk-information"]["eligible-for-protection"]["VALUE"])
          //{
              ...
              $currencyCode = $data[$root]["authorization-amount"]["currency"];
              //order amount validation
              if(!isset($orderTotal))
                  die("NOT valid amount");
              //get payment id
             $orderId = base64_decode($data[$root]["order-summary"]["shopping-cart"]["merchant-private-data"]["oId"]["VALUE"]);
             $ip = base64_decode($data[$root]["order-summary"]["shopping-cart"]["merchant-private-data"]["uip"]["VALUE"]);
            ...
            {
                $actUserCharge = (int)$funobj->currencyConvert($convertCurrency,$userCurrency,$orderTotal);
            }
...
//              }
            ...
            $error = 'problem in talktime calculation or ip authentication!!!'.'tt'.$talktime.'dtt'.$dbTalktime.'dbip:'.$dbIp.'ip:'.$ip.'dbr:'.$dbRechare.'act'.$actUserCharge;
        if($error!="")
        {
            ...
            die("error occur in update payment detail phone91");
             ...
             //get currency
              ...
              //set msg to mail
              $mail_msg="One phone91 user with id ".$uname." and Googlecheckout emailid :".$buyerEmail." did google checkout payment of ".$orderTotal." ".$currencyCode." and got recharged by ".$talktime." ".$userCurrency;
    ...
    case "checkout-redirect": {
        ...
        if(!$mailerr->sendmail_mandrill($mailTo, $sub, $msg,$buninessEmail))
        {
            $mailerr->errorHandler('problem in mail:\nmailto'.print_r($mailTo,1).'\nsub:'.$sub.'\nmsg:'.$msg);
            die('problem in mail:\nmailto'.print_r($mailTo,1).'\nsub:'.$sub.'\nmsg:'.$msg);
        }
    ...
    case "charge-amount-notification": {
        ...
        $mailTo = array('AnkitPatidar@hostnsoft.com');
        if(!$mailerr->sendmail_mandrill($mailTo, $sub, $msg.print_r($response,1),$buninessEmail))
        {
            $mailerr->errorHandler('problem in mail:\nmailto'.print_r($mailTo,1).'\nsub:'.$sub.'\nmsg:'.$msg);
            die('problem in mail:\nmailto'.print_r($mailTo,1).'\nsub:'.$sub.'\nmsg:'.$msg);
        }
