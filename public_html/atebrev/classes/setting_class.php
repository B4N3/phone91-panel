...
{  
   #function connecting from action_layer.php 
    ...
    function update_newdetails($parm,$userid) {
       ...
       #validate gender
       if($parm['panel_time'] == ''){
           return json_encode(array('msgtype'=>'error','msg'=>'Please select your Time Zone!!!'));
       }
       
       /**
        * code to get time zone diffence 
        */
       
       //get server timezone offset
        $serverTime = date('d-m-Y h:i:s');//$this->getServerTimeZoneOffset();
       ...
       $timeZone = $this->db->real_escape_string($parm['panel_time']);
               
       #table name in that the action to be taken      
       $table = '91_personalInfo';
       #$data contains the array data 
       $data= array("name"=>$name,"sex"=>$parm['gender'],"ocupation"=>$ocupation,"dob"=>$parm['dob'],"address"=>$address,"city"=>$city,"zipCode"=>$zip,"country"=>$country,'offset' => $timeZone); 
...
function check($date){
    ...
    function addRegisterId($param,$userId){
       ...
       elseif($type == 2)
       {
           //valid skype username
           if(strpos($param['emailId'],'@'))
           {
               return json_encode(array("status" => "error", "msg" => "please Enter valid skype username!!!"));
           }
...
function getProfileDetails($userid)
{
    ...
    return json_encode(array("name"=>$name,"dob"=>$dob,"ocupation"=>$ocupation,"country"=>$country,"city"=>$city,"zip"=>$zip,"address"=>$address,"gender"=>$sex,'timeZone' => $time));
...
function convertToUserTimeZone($dateTime, $userTimeZone = NULL) 
{
        ...
        if (is_null($userTimeZone)) {
            //$userTimeZone = $this->getPanelTime($panelId);
            $userTimeZone = $_SESSION['panel_time'];
        }
        ...
        if ($sign == "-")
            $timeNdate = gmdate($dateTime - $offset);
        else
            $timeNdate = gmdate($dateTime + $offset);
...
function getUserTimeZoneOffset($code)
{
    include_once(dirname(dirname(__FILE__)).'/csvtoarray/csv.inc.php');
    //echo dirname(dirname(__FILE__)).'/csvtoarray/csv.inc.php';
    $csv = new csv_uploder(dirname(dirname(__FILE__)).'/csvtoarray/iso.csv', 2000 , ',');
    $countryCodes=$csv->getCsv();
    //var_dump($countryCodes);
    $count = count($countryCodes);
    for($k = 0 ; $k < $count ; $k++)
    {
        $countryDetails = $countryCodes[$k];
        if($countryDetails['CountryCode'] == $code)//this is the country
        {
            $iso = substr($countryDetails['ISO'],0,2);
            break;
        }
    }

    $timezones = (DateTimeZone::listIdentifiers(DateTimeZone::PER_COUNTRY, $iso));
    $d = new DateTime("now", new DateTimeZone($timezones[0]));
    $time = substr(date("c",strtotime($d->format(DateTime::W3C))),19);
...
*@
*/
function changeListenStatus($request,$userId)
{

  if(!isset($request['userId']) || $request['userId'] != $userId || !is_numeric($request['userId']))
  {
    return json_encode(array('status' => 'error','msg' => "You don/'t have permission to update!!!"));
  ...
  if(!$result || $this->db->affected_rows == 0)
  {
      trigger_error('problem while update listen time during time option,Query:'.$this->querry);
      return json_encode(array('status' => 'error','msg' => "problem while update listen time during time option!!!"));      
  }
...
*@author Ankit patidar <ankitpatidar@hostnsoft.com>
*@since 13/05/2014
*@uses function to get payment gateway details
*
*/
function getPaymentGatewayDetails($userId)
{

    if(empty($userId) || preg_match(NOTNUM_REGX,$userId)) 
    {
      return json_encode(array('status' => 'error','msg' => 'Not a valid user!!!'));
    }
...
*@author Ankit patidar <ankitpatidar@hostnsoft.com>
*@since 13/05/2014
*@uses function to edit payment gateway details
*
*/
function editPaymentGatewayDetails($request,$userId)
{
    ...
    if(empty($request['paypal']) || strlen($request['paypal']) > 50)
      return json_encode(array('status' => 'error','msg' => 'You exceeded the max length for merchant id!!!')) ;

    if(!isset($request['status']))
      return json_encode(array('status' => 'error','msg' => 'Please select the activation status!!!'));

    //prevnet from sql injection
    $merchantId = $this->db->real_escape_string($request['paypal']);
    $status = $this->db->real_escape_string($request['status']);
    ...
    $resultExists = $this->selectdata('userId',$table,'userId='.$userId.' and type=1');
    ...
    if($resultExists->num_rows == 0)
    {
      ...
      $insertData = $this->insertData($data,$table);

      if(!$insertData || $this->db->affected_rows == 0)
      {
        trigger_error('problem while payment detail insertion,qur:'.$this->querry);
        return json_encode(array('status' => 'error','msg' => 'problem while saving details!!!'));   
      }
      else
        return json_encode(array('status' => 'success','msg' => 'Payment details successfully saved!!!'));    

    }

    //update details
    $result = $this->updateData($data,$table,'userId='.$userId.' and type = 1');

   
    if(!$result || $this->db->affected_rows == 0)
    {
      return json_encode(array('status' => 'error','msg' => 'Change payment details to save!!!'));
    }
