    ...
    function addContact($parm, $userid , $type = 0 ) 
    {
        ...
        for ($i = 0; $i < $count; $i++) 
        {
            if(!in_array($i, $errorKey))
            {
                ...
                if ($result <= 0  && !in_array($allcontact[$i], $allcontact)) 
                {
...
//    }
     ...
     function updateContact($parm, $userid,$type='0',$agentEdit='0') 
    {
       ...
       $subcondition = array('$ne' => new mongoId($contactId));
        ...
        $condition = array( 'contactNo' => $allcontact ,'code' =>$code , 'userId' => $_SESSION['id'] ,'contact_id' => $subcondition);
    ...
    function checkContactValidation($allcode,$allemail, $allcontact , $allname) 
    {
        ...
        for ($i = 0; $i < count($allcontact); $i++) 
        {
           ...
           // echo ' nidhi '.preg_match(NOTALPHANUM_REGX, $allname[$i]);
            if (preg_match(NOTALPHANUM_REGX, $allname[$i])  ||  strlen( $allname[$i]) < 1 ||  strlen( $allname[$i]) > 30 )
            {
                 $errorKey[] = $i;
                // echo "sdfsdhf";
