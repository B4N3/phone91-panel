...
//         $this->sendErrorMail("rahul@hostnsoft.com", "Phone91 signup_class 91_userBalance query fail : $tempsql ");
    ...
    public function validateSignUp()
    {
        ...
        if (strlen($this->userName) < 1 || strlen($this->password) < 1 ||  strlen($this->email) < 1) 
        {
    ...
    public function signUp($param)
    {
        ...
        $this->firstName = $param['firstName']." ".$param['lastName'];
        $this->email = $param['email'];
       
        #user select tariff id not currency, its see currency but values are tariff id 
        $tariffId = $param['currency'];
        
        if (!preg_match("/^[0-9]+$/", $tariffId)) 
        {
            return json_encode(array("status" => "error","msg" => "please select valid currency!"));
        }
        $currencyId = $this->getOutputCurrency($tariffId);
        if($currencyId == 0){
      ...
      $resellerId = $param['reseller_id'] = $this->getDomainResellerId($param['domain']);
      else
      $resellerId = $param['reseller_id'] = $this->getDomainResellerId($_SERVER['HTTP_HOST']);  
      ...
      #get user balance data into table 
      $balance = 0;
      $balanceData = $this->getResellerDefaultCurrency($resellerId , $currencyId);
      
       if(!$balanceData)
          trigger_error('response not found from getResellerDefaultCurrency function');
      
      if($balanceData && $balanceData->num_rows > 0)
      {
     ...
     $createUserResult = $this->createUser($param, $tariffId,$balance, $currencyId);
     ...
     * @return type json
     */
    public function updateUserCurrencySetPlan($currencyId,$resellerId,$userId) {
  ...
  /********************END HERE*****************************************************/   
      ...
      $data = array("tariffId"=>$tariffId,"balance"=>$balance);
    ...
    function sign_up($parm)
    {
       ...
       #check for all value inserted or not in signup form    
        ...
        if(!preg_match('/^[a-zA-Z0-9\@\_\-\s]*[A-Za-z][a-zA-Z0-9\@\_\-\s]*$/', $parm['username']))
        {
           return json_encode(array("status" => "error",
