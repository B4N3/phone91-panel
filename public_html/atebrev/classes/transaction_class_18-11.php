...
class transaction_class extends fun
{
    ...
    function addTransactional($fromUser,$toUser,$amount,$talktime,$paymentType,$description,$type,$partialAmt = 0,$currency = null)
    {
...
//        $this->updateUserBalance($toUser,$currentBalance);
    ...
    #created by sudhir pandey (sudhir@hostnsoft.com)
    function addTransactional_sub($fromUser,$toUser,$amount,$currentBalance,$paymentType,$debit,$credit,$closingBalance,$description,$currency = null)
    {
        ...
        $data=array("fromUser"=>(int)$fromUser,"toUser"=>$toUser,"date"=>"".date('Y-m-d H:i:s'),"amount"=>$amount,"currentBalance"=>$currentBalance,"debit"=>$debit,"credit"=>$credit,"paymentType"=>$paymentType,"closingBalance"=>$closingBalance,"description"=>$description,"currency"=>$currency); 
    ...
    function getTransactionLogDetail($fromUser,$toUser)
    {
      ...
      if ($result->num_rows > 0)
      {
          while ($row= $result->fetch_array(MYSQL_ASSOC) ) 
          {
              ...
              $data['creditActualCurrency'] = $data['credit']. "  ". $currencyName; ;//$this->currencyConvert($data['currencyName'], $currencyName, $data['credit']) . "  ". $currencyName;
              $data['debitActualCurrency'] =  $data['debit']. "  ". $currencyName;//$this->currencyConvert($data['currencyName'], $currencyName, $data['debit']) . "  ". $currencyName;
              }else
              {
              $data['creditActualCurrency'] = round($this->currencyConvert($data['currencyName'], $currencyName, $data['credit']),2) . "  ". $currencyName;
              $data['debitActualCurrency'] = round($this->currencyConvert($data['currencyName'], $currencyName, $data['debit']),2) . "  ". $currencyName;
