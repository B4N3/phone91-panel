...
//        	    var_dump($result);
    ...
    function sign_up($parm){
     ...
     #check for all value inserted or not in signup form    
      ...
      $data=array("userId"=>(int)$userid,"tariffId"=>(int)$tariff_id,"balance"=>$balance,"currencyId"=>(int)$currency_id,"callLimit"=>(int)$call_limit,"resellerId"=>(int)$reseller_id); 
      ...
      # Assign Variables for sending sms to user
        ...
        $data=array("userId"=>(int)$userid,"date"=>date('Y-m-d H:i:s'),"amount"=>$balance,"credit"=>1,"debit"=>0); 
