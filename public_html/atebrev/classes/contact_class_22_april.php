...
class contact_class extends fun{
	...
	function getUnConfirmEmail($userId)
	{
        ...
        function verifyEmailid($parm,$userid){
            ...
            $this->db->select('*')->from($temptable)->where("userid = '" . $userid . "' and confirm_code = '".$key."' and domainResellerId = '".$resellerIdServer."'");
	    ...
	    $result = $this->db->execute();
            ...
            if ($result->num_rows == 0){
                ...
                $data=array("userid"=>(int)$userid,"email"=>$email,"confirm_code"=>$key,"newsFlag"=>$newsFlag); 
...
function addnew_emailid($new_emailid, $userid) {
    ...
    if ($result->num_rows == 0) 
    {
        ...
        $resellerIdServer = $this->getDomainResellerId($_SERVER['HTTP_HOST']);
        #- value for store in database 
        $data = array("userid" => (int) $userid, "email" => $new_emailid, "confirm_code" => $confirm_code, "date" => date('Y-m-d H:i:s') , 'domainResellerId' => $resellerIdServer);
        #- modification ends.
...
function verifyNumber($parm,$userid,$type = NULL){
	...
	if (strlen($parm['key']) > 0 || !preg_match('/[^0-9]+/', $parm['key'])) 
	    ...
	    $result = $this->db->execute();
            ...
            $data=array("userId"=>(int)$userid,"countryCode"=>(int)$countryCode,"verifiedNumber"=>$tempNumber,"isDefault"=>$isdefault,"verifiedDate"=>date('Y-m-d H:i:s'),"domainResellerId"=>$domainResellerId,"confirmCode"=>$key); 
