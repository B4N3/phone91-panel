...
class dialPlanClass extends fun{
    ...
    var $msg = "";
    private $dialPlanPrefixCountArr ;
...
//        echo $this->querry;
        if($planInsertResult && $this->db->affected_rows > 0)
        {
            ...
            return json_encode(array("msg"=>"successfuly added the plan name","status"=>"success","lastId"=>$planId));
        }
        else
    ...
    {
        $this->db->select(" count(*) as total,dialPlanId ")->from("91_dialPlanRoute")->groupBy("dialPlanId");
        $res = $this->db->execute();
        if(!$res)
            return false;
        ...
        {
            $data[$row['dialPlanId']] = $row['total'];
        }
        
        $this->dialPlanPrefixCountArr = $data;
        return true;
    }
    ...
    function getPlanList($type = 1,$pageNo,$userId,$planName=NULL)
    {
        ...
        else
        {
            if(!$this->validatePlanName($planName) || $this->validatePlanNameFlag == false)
                return json_encode(array("msg"=>$this->msg,"status"=>"error"));
            $columns = " SQL_CALC_FOUND_ROWS id,userId,planName";
...
//            echo $this->querry;
            ...
            if(!$this->countPrefix())
                return json_encode (array("msg"=>"Error fetching prefix count please contact provider","status"=>"error"));
            ...
            {
                $row['count'] = $this->dialPlanPrefixCountArr[$row['id']];
                $data[] = $row;
            }
    ...
    function addDialPlanCountry($countryName,$userId,$planId)
    {
        ...
        $selResult = $this->selectData("*",$table,"dialPlanId='".$planId."' and country='".$countryName."' and userId='".$userId."'");
        if(!$selResult)
...
//        echo $this->querry;
    ...
    function getPrefixDetails($planId, $userId, $pageNumber, $search = NULL, $keyword = NULL, $export = 0) {
        $planId = trim($planId);
        $userId = trim($userId);
        ...
        $selRouteRes = $this->selectData(" SQL_CALC_FOUND_ROWS * ", "91_dialPlanRoute", $conditionForRouteTable);

        if ($selRouteRes) {

             $field = array(" FOUND_ROWS()" => "totalRows");
             $this->db->select($field);
             $selRouteCountRes = $this->db->execute();
    ...
    function addPrefixDetails($dialPlanId,$countryId,$prefix,$routeId,$userId,$slno,$type = 0)
    {
        ...
        if($type == 1 && (preg_match(NOTNUM_REGX,$slno) || $slno == ""))
                return json_encode(array("msg"=>"Error invalid data please contact provider","status"=>"error"));
...
//                return json_encode(array("msg"=>"Error Invalid Country Please Contact Provider","status"=>"error"));
        ...
        $data = array("dialPlanId"=>$dialPlanId,"userPrefix"=>$prefix,"routeId"=>$routeId);
        $table = "91_dialPlanRoute";
        
        if($type == 0)
        {
            
            $insertRes =$this->insertData($data,$table);//->onDuplicate("routeId='".$routeId."'");
...
//            $insertRes = $this->db->execute();
        ...
        {
            $condition = "slNo = ".$slno;
            $insertRes = $this->updateData($data, $table,$condition);
            $lastInsertId = $slno;
//            echo $this->querry;
            $action = "updated";
        }   
...
//        $this->db->addCommand('ON DUPLICATE KEY UPDATE',"routeId='".$routeId."', country='".$countryId."'");
        ...
        {   
            if($this->db->affected_rows > 0)            
                return json_encode(array("msg"=>"Successfuly $action data","status"=>"success","id"=>$lastInsertId));
            else
                return json_encode(array("msg"=>"Entry already exist nothing to update","status"=>"warning"));
        }
        else
        {
            if($this->db->errno == 1062)
                return json_encode(array("msg"=>"Error duplicate entry please try again","status"=>"error"));
            else
    ...
    function deletePrefixDetails($dialPlanId,$serialNum,$userId)
    {
        ...
        if(!$selRes || $selRes->num_rows < 1 )
            return json_encode(array("msg"=>"You don't have permission to edit this dialPlan","status"=>"error"));
        $table = "91_dialPlanRoute";
