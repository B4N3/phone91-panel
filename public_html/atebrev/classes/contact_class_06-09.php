        ...
        if ($result->num_rows == 0) {
            ...
            if($result)
            {
                ...
                if($error!=1){
                    ...
                    return json_encode(array('msgtype' => 'success', 'msg' => 'Contact Added Successfully! Please Enter Verification Code.',"unverifiedContact"=>$unverifiedContact));
                   // $delete = $this->db->query("delete from tempcontact where userid='" . $session['id_cl'] . "'");
                   }else{
                   return json_encode(array('msgtype' => 'success', 'msg' => 'Contact Added But Unable To Send SMS!',"response"=>$sendSmsResponse,"unverifiedContact"=>$unverifiedContact));
                }
            }
            else{
                echo $sql;
...
//		    $confirm = $row['confirm'];
	    ...
	    $result = $this->db->execute();
            ...
            $data=array("userId"=>(int)$userid,"countryCode"=>(int)$country_code,"verifiedNumber"=>$tempNumber,"isDefault"=>$isdefault); 
  ...
  */  
       ...
       if ($_POST['country_code'] == $contact['countryCode'] && $_POST['resend_phone'] == $contact['tempNumber'] && strlen($contact['confirmCode']) > 0) 
       {
		...
		if ($contact['countryCode'] == "91")
			...
			$funobj->SendSMSUSD($d);
		return json_encode(array('msgtype' => 'success', 'msg' => 'Confirmation Code Send'));    
