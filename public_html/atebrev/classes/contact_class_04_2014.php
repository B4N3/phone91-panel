...
//    $resellerIdServer = $this->getDomainResellerId($_SERVER['HTTP_HOST']);
  ...
  //  if($status)
    ...
    $table = '91_verifiedNumbers'; 
    $this->db->select('*')->from($table)->where("countryCode = '" . $country_code . "' and verifiedNumber='".$phone."' and resellerId = '".$resellerId."'  ".$condition);
    $result = $this->db->execute();
...
function getUserDefaultNumber($userId , $type = 0)
{
    ...
    if($result->num_rows > 0)
    {
        ...
        return $row['verifiedNumber']; //$row['countryCode'].
    }
    else
        return 0;
}
