...
//echo json_encode(array('trdata'=>$str,'paging'=>$pageStr));
    ...
    function addNewClient($parm, $resellerid) {
        ...
        $this->db->select('*')->from($table)->where("name = '" . $parm['username'] . "'");
        $this->db->getQuery();
        $result = $this->db->execute();
        if ($result->num_rows > 0) {
        ...
        $data = array("userId" => (int) $userid, "tariffId" => (int) $parm['tariff'], "balance" => 0, "currencyId" => (int) $currency_id, "callLimit" => (int) $call_limit, "resellerId" => (int) $resellerid);
        ...
        $funobj = new fun();
        $msg = $funobj->addnew_emailid($parm['email'], $userid);
        ...
        $msg = $transaction_obj->addTransactional($resellerid, $userid, $balance, $paymentType, $description, "prepaid"); //$fromUser,$toUser,$amount,$paymentType,$description,$type
...
//        //var_dump($savedata);
        ...
        return json_encode(array('status' => 'success', 'msg' => 'successsully client added'));
    }
...
//		return $result;
    ...
    function editFund($parm,$userid){
        /**
         * [action] => editFund
    [changefunderEditFund] => add
    [fundAmount] => 23000
    [currency] => USD
    [balance] => 35000
    [pType] => partial
    [partialAmt] => 5000
    [fundPaymentType] => cash
    [otherPaymentType] => 40
    [fundDescription] => uuuuuu
    [save] => Save Changes
         */
      ...
      $result = $transaction_obj->addTransactional($userid,$parm['toUserEditFund'],$parm['balance'],$parm['fundPaymentType'],$parm['fundDescription'],$parm['pType'],$parm['partialAmt']);
