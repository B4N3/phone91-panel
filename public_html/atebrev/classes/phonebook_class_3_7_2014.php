    ...
    function addContact($parm, $userid , $type = 0 ) 
    {
        ...
        for ($i = 0; $i < $count; $i++) 
        {
            if(!in_array($i, $errorKey))
            {
                ...
                if ($result <= 0) 
                {
                    ...
                    if(!in_array($i, $accessErrorKey) && !empty ($allAccessNumber[$i]) && !empty($hashArray[$i]) )
                    {
        ...
        if(!empty($validContacts))
        {
            ...
            { 
                ...
                {
                      $message.= ' You can not Assign same access number with same hash to multiple contacts.';
                }
                ...
                return json_encode(array("status" => "success", "msg" => "sucessfully  contact add ! ".$message , "str" => $alreadyExistHash));
            }
            else
            {
                if($alreadyExist > 0)
