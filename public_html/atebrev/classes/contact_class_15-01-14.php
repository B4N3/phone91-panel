...
function verifyNumber($parm,$userid,$type = NULL){
	...
	if (strlen($parm['key']) > 0 || !preg_match('/[^0-9]+/', $parm['key'])) 
	    ...
	    $result = $this->db->execute();
            ...
            $data=array("userId"=>(int)$userid,"countryCode"=>(int)$countryCode,"verifiedNumber"=>$tempNumber,"isDefault"=>$isdefault,"verifiedDate"=>date('Y-m-d H:i:s'),"domainResellerId"=>$domainResellerId); 
