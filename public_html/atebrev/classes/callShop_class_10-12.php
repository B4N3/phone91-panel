...
//                        return json_encode(array("msg"=>"Error Invalid Password","status"=>"error"));
            ...
            if(isset($systemType) && $systemType == "0")
            {
                ...
                {   
                    $userName = "CALL_".$callShopId.$this->randomNumber(8);
           ...
           foreach($data as $key => $value)
           {
            ...
            if($res)
            {
                ...
                if($sip)
                {
                    ...
                    $insertVerifySipArr = array("userId"=>$userID,"userName"=>$userName,"passwd"=>$password,"chainId"=>$sipChainId,"systemId"=>$systemId,"isCallShopUser"=>1);

                    $resSip = $this->insertData($insertVerifySipArr, $sipTable);
...
//        echo $result->num_rows;
        ...
        else if($type == 2)
        {
            ...
            $callShopRes = $this->selectData("systemId,systemName,type,userName,resellerId,callShopId", "91_addCallShop","resellerId = ".$resellerId." and callShopId=".$callShopId);//,"systemId IN ('".$sytemIdStr."')"
            ...
            if($callShopRes)
            {
                ...
                while($rowRes = $callShopRes->fetch_array(MYSQLI_ASSOC))
                {
                   ...
                   $arr['cost'] = "";
                    ...
                    $arr['currency'] = $this->getCurrencyViaApc($reponse[$rowRes['systemId']][0]['currency'],1);
...
//            return json_encode(array("msg"=>"Invalid callshop ","status"=>"error"));
    ...
    if(!$delResSip)
      ...
      return  $this->insertSystemDetails($request,$resellerId,$systemId);
