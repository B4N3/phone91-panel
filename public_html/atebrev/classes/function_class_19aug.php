...
//              $cmd = 'pygsend '.$email.' '.$text;
	...
	function get_currency($cid){
	        $con = $this->voip_connect();
	        $result = mysql_query("select name from currency_names where id='".$cid."'",$con) or die('Query error');
	        mysql_close($con);
...
//    		$timeout = '';
	...
	function refund_chain_balance($chainId, $telNum, $startTs, $endTs, $nextTs){
		...
		if ($real_deduct <= $expected_deduct){
			...
			if ($amount > 0){
				...
				$con = $this->voip_connect();
                                $sql = "insert into 91_chainBalanceReport (chainId,deductBalance) VALUES ('$chainId','$expected_deduct')";
                                $result = mysql_query($sql,$con) or die('Query error');
                                mysql_close($con);
			}
		}
		...
		if (strlen($chainId) <= 4){
                        ...
                        $newchainId = substr($chainId,0,-4);
                        return $this->refund_chain_balance($newchainId, $telNum, $startTs, $endTs, $nextTs);
                }
	}
