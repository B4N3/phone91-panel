...
class phonebook_class extends fun
{
    ...
    function addContact($parm, $userid , $type = 0 ) 
    {
        ...
        $errorKey = $this->checkContactValidation($allcode,$allemail, $allcontact);
...
//            }
    ...
    */
    ...
    function updateContact($parm, $userid,$type='0') 
    {
...
accessType	1
        ...
        if (!preg_match("/^[0-9]{1,5}$/", $code)) 
        {
            ...
            return json_encode(array("status" => "error", "msg" => "Please select valid country code!"));
        }
        ...
        if( isset($accessNo) && !empty($accessNo) && $accessNo != '' && $accessNo != NULL )
        {
            ...
            {
                return json_encode(array("status" => "error", "msg" => "Please select valid extension number!" ));
            }
    ...
    */
	...
	$conDtl = array();
        ...
        return json_encode(array('countryList' => $data, 'countryDetail' => $conDtl));        
    }
    ...
    */
	...
	$data = array();
	 if(empty($resId) || preg_match(NOTNUM_REGX, $resId))
        {
            return json_encode(array('status' => 'error','msg' => 'Session expired ,Please login again!','data' => $data));
        }

        if(empty($prefix) || preg_match(NOTNUM_REGX, $prefix))
        {
            return json_encode(array('status' => 'error','msg' => 'Invalid country selected!','data' => $data));
        }
	...
	$data = array();
	 ...
	 if(empty($state) || preg_match(NOTALPHABATESPACE_REGX, $state))
        {
            return json_encode(array('status' => 'error','msg' => 'Invalid state selected!','data' => $data));
        }
	 //get reseller Chain id
        $chainId = $this->getUserChainId($resId);
        ...
        #get all access number from 91_longCodeNumber table  
        $result = $this->selectData('*',$table,"hidden=0 and  type=2 and resellerChainId='$chainId' and state='$state'");
	
	
	 if(!$result || $result->num_rows == 0)
        {
                trigger_error('problem while get access number by prefix,Qur:'.$this->querry);
               return json_encode(array('status' => 'error' ,'msg' => 'Problem while getting access number!!!'));
	 ...
	 return json_encode(array('status' => 'success' ,'msg' => 'Record successfully found!!!','data' => $data)); 
    ...
    function getAllContact($userid , $type = '0' ,$accStatus ='0',$searchStr ='') 
    {
        $allcontact = array();
	...
	$condition['contact'] = array('$exists' => false);
        ...
        if($searchStr != ''){
            $cond[] = array('name' =>  array('$regex' =>"^$searchStr"));
            $cond[] = array('contactNo' => array('$regex' =>"^$searchStr"));
            $cond[] = array('email' =>  array('$regex' => "^$searchStr"));
            $condition['$or'] = $cond;
        }
    ...
    function checkContactValidation($allcode,$allemail, $allcontact) 
    {
			  ...
			  <div class="cntInfo slideAndBack" onclick="dest(\''.$res['contactNo'].'\',this)">
					<div class="innerCol clear">
                          ...
                          <div class="fpinfo"> <i class="call"></i>
							 <label><i class="ic-16 callA"></i>'.$accessNo.'</label>
						  </div>
					</div>
				</div>		      
			</li>';            
