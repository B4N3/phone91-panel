...
class transaction_class extends fun
{
    ...
    function updateUserBalanceOld($toUser,$currentBalance){
    ...
    /**
     * last updated by Ankit Patidar <ankitpatidar@hostnsoft.com> 
     * @since 9/12/2013
     * @uses to update balance
     * @param int $toUser
    ...
    function updateUserBalance($toUser,$talktime,$sign='+')
    {
        ...
        if($talktime > 0)
            $sqlC = "UPDATE 91_userBalance SET balance=balance$sign$talktime WHERE userId='" . $toUser . "'";
        else
        {
            return 0;
        }
        
        $result = mysqli_query($this->db, $sqlC) or $error = 'error::' . mysqli_error();
         
        if(!$result)
        {
             trigger_error('problem while udpate user balance  ,query:'.$sqlC); 
             $this->sendErrorMail('ankitpatidar@hostnsoft.com','problem while update user balance,query:'.$sqlC.' error:'.$error);
             return 0;
        }
    ...
    function addTransactionalOld($fromUser,$toUser,$amount,$talktime,$paymentType,$description,$type,$partialAmt = 0,$currency = 0,$partialCurrency = 0)
    {
        ...
        if($type == "partial")
        {
            ...
            $partialbal = $this->closingBalCurrencyCnvt($toUser,$partialCurrency,$partialAmt);
            $closingBalance = ((int)$closingBalance - (int)$partialbal);
            
            #add  partial transaction with given payment type (cash,memo,bank or other).
            $result2 = $this->addTransactional_sub($fromUser,$toUser,$talktime,$currentBalance,$paymentType,0,$partialAmt,$closingBalance,$description,$partialCurrency);
...
//        $this->updateUserBalance($toUser,$currentBalance);
    ...
    #creation date 01/07/2013
    #function use for add Transaction entry into transactionLog table 
    function addTransactional($amount,$talktime,$paymentType,$description,$type,$partialAmt = 0,$currency = 0,$partialCurrency = 0)
    {
        ...
        #add transaction in case of voip91(payment type).
        $result = $this->addTransactional_sub($talktime,$currBalance,"voip",$amount,0,$closingBalance,$description,$currency);
        ...
        if($type == "prepaid")
        {
            ...
            #add transaction with given payment type (cash,memo,bank or other).
            $result2 = $this->addTransactional_sub(0,$currBalance,$paymentType,0,$amount,$closingBalance,$description,$currency);
        ...
        if($type == "partial")
        {
            ...
            #add  partial transaction with given payment type (cash,memo,bank or other).
            $result2 = $this->addTransactional_sub(0,$currBalance,$paymentType,0,$partialAmt,$closingBalance,$description,$partialCurrency);
                        
        }
        
        #update closing balance of user 
        $result3 = $this->updateClosingBalance($this->toUser,$closingBalance);
...
//        $this->updateUserBalance($toUser,$currentBalance);
    ...
    #created by sudhir pandey (sudhir@hostnsoft.com)
    function addTransactional_subOld($fromUser,$toUser,$amount,$currentBalance,$paymentType,$debit,$credit,$closingBalance,$description,$currency = 0)
    {
    ...
    function addTransactional_sub($amount,$currentBalance,$paymentType,$debit,$credit,$closingBalance,$description,$currency = 0)
    {
        ...
        //***********check payment currency type ******
        #get user currency id 
        extract($this->getNameAndUserName($this->toUser));//$currencyId
        if($currency != $currencyId)
        {
            $paymentCurrency = $this->getCurrencyViaApc($currency, 1);
            $userCurrency = $this->getCurrencyViaApc($currencyId, 1);
            
            $creditCntAmt = $debitCntAmt = 0;
        ...
        $paymentType = $this->db->real_escape_string($paymentType);
        $description = $this->db->real_escape_string($description);
        $data=array("fromUser"=>(int)$this->fromUser,"toUser"=>$this->toUser,"date"=>"".date('Y-m-d H:i:s'),"amount"=>$amount,"currentBalance"=>$currentBalance,"debit"=>$debit,"credit"=>$credit,"paymentType"=>$paymentType,"closingBalance"=>$closingBalance,"description"=>$description,"currency"=>$currency,"debitConvert"=>$debitCntAmt,"creditConvert"=>$creditCntAmt); 
...
//        $response= $sesObj->mailAwsSes($to, $subject, $message, $from);
    ...
    #creation date 02-08-2013
    #function use for get transaction log detail 
    #param resellerTrans use to reseller transaction log 
    function getPersonalTransaction($userid,$resellerTrans = 0)
    {
      
      #get user currency id 
      extract($this->getNameAndUserName($userid));//$currencyId
      ...
      if ($result->num_rows > 0)
      {
          while ($row= $result->fetch_array(MYSQL_ASSOC) ) 
          {
              ...
              {
              extract($this->getNameAndUserName($row['toUser'])); // name and userName 
              $data['name'] = $name;
              $data['userName'] = $userName;
              }
              $data['toUser'] = $row['toUser'];
    ...
    function addReduceTransaction($parm,$userid)
    {
      ...
      if($resellerId != $userid){
        ...
        $clsamount = $this->closingBalCurrencyCnvt($toUser,$parm['currency'],$amount);
        ...
        $this->toUser = $toUser;
         #add  partial transaction with given payment type (cash,memo,bank or other).
         $result = $this->addTransactional_sub(0,$currentBalance,$paymentType,$debit,$credit,$closingBalance,$description,$parm['currency']);
