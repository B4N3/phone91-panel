    ...
    function getTransactionLogDetail($fromUser,$toUser)
    {
      ...
      if ($result->num_rows > 0)
      {
          while ($row= $result->fetch_array(MYSQL_ASSOC) ) 
          {
              ...
              $data['creditActualCurrency'] = $data['credit']. "  ". $currencyName; 
              $data['debitActualCurrency'] =  $data['debit']. "  ". $currencyName;
              }else
              {
              $data['creditActualCurrency'] = round($row['creditConvert'],3) . "  ". $currencyName;
              $data['debitActualCurrency'] = round($row['debitConvert'],2) . "  ". $currencyName;
