...
//            return json_encode(array("msg"=>"Error fetching details","status"=>"error"));
    ...
    function getResProfitDurationGraphDetails($chainId,$date,$type,$endDate = NULL)
    {
        ...
        if($type == "duration")
        {
            $query = "select sum(durationCharged) as clmValue,chainId,resellerId from (SELECT * FROM `91_durationCharged` where resellerId = '".$chainId."' and date(date) between '".$date."' and '".$endDate."' order by durationCharged DESC ) as durationTable group by chainId  limit 100";
        }
        elseif($type == "profit" || $type == "loss")
        {
          $query = "select sum(profit) as clmValue ,childId as chainId,currencyDesc as currency from (SELECT * FROM `91_chainProfit` where parentId = '".$chainId."' and date(date) between '".$date."' and '".$endDate."' order by profit DESC ) as t group by childId limit 100";
        }
...
//            return json_encode(array("msg"=>"Error fetching Details please try again later","status"=>"error"));
    ...
    function callFailedErrorLog($request)
    {
        ...
        $this->db->select('*')->from($table)->where($likeQ.' date BETWEEN "'.$sDate.'" AND "'.$eDate.'"')->orderBy('date DESC')->limit(50);
