...
//        $paymentType = $parm['payType'];
        ...
        $data = array("userId" => (int) $userId,"chainId"=>$chainId, "tariffId" => (int) $parm['tariff'], "balance" => 0, "currencyId" => (int) $currency_id, "callLimit" => (int) $call_limit, "resellerId" => (int) $resellerId,"routeId"=>$userRoutDetail['routeId'],"isDialPlan"=>$userRoutDetail['isDialPlan']);
...
//              $allUserCondition ="and lower(userName) like '%".$q."%'";
              ...
              $result =  $this->allUserDetail($newcondition,$first_limit,$limit); 
            }
            else
...
//            }
    ...
    function allUserDetail($condition = null, $start_limit=0,$limit=10)
    {
        ...
        $table = '91_manageClient';
        $this->db->select('SQL_CALC_FOUND_ROWS *')->from($table)->where('type != 4 and type !=7 and type !=1 '.$allcondition.' order by userId  desc  limit '.$start_limit.','.$limit);
        $qur = $this->db->getQuery();
        $result = $this->db->execute();
