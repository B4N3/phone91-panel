...
//        die();
        ...
        if(isset($request['type']) && $request['type'] == 'EMAIL')
        {
            ...
            $code =$funobj->verifyCode($request["code"],$request["number"],$domain);    
        }
...
//            $host = $host.'/sign-in.php';
        ...
        if(!empty($userName) && ctype_digit($userName))
        {
            ...
            $numberResult = $cntClsObj->checkVerifiedNumberExist($userName,$domainResellerId);
...
//                var_dump($cntClsObj->data);
                $userName = $cntClsObj->data[0]['userName'];
            }
