    ...
    if (strlen($_REQUEST['confirmationCode']) > 0) {
        ...
        if ($tempresult) {
            ...
            if ($confirm_flag == 1) {
               ...
               $domainResellerId =  $funobj->getDomainResellerId($_SERVER['HTTP_HOST'],2); 
              // print_r($domainResellerId);
                #insert the email di from temp to verified table 
                $insertSql = "INSERT INTO 91_verifiedEmails (userid, email,confirm_code,resellerId,domainResellerId) VALUES ('" . $userId . "','" . $email . "','" . $confirmCode . "' , '".$domainResellerId['resellerId']."' , '".$domainResellerId['resellerId']."' )";
