	...
	private function json($data) {
        ...
        */
        public function sendMesages()   
        {
            ...
            if($message == 'forget' || $message == 'forgot' || $message == 'pin' )
            {
               ...
               $userInfo = json_decode($result ,true ); //msgStatus
                ...
                if( $userInfo['msgStatus'] == "success")
                {
                    if(!empty($userInfo['userInfo']['userId']))
                    {
                       ...
                       if($userInfornation)
                       {
                           ...
                           {
                                $tempparam['text'] = $userInfornation['userPin'];
                                $this->funobj->SendSMSUSDnew($tempparam);
                                echo "1";
                           }
                           else 
                    ...
                    else 
                    {
                        ...
                        $error[RESPONSE] = "0";
                        $error[MESSAGE] = array("code"=>"141","message"=>"Invalid User Id");
                        $this->response($this->json($error), 200);
                    }
                }
                else 
                {
                    $this->funobj->SendSMSUSDnew($tempparam);
                    $error[RESPONSE] = "0";
                    $error[MESSAGE] = array("code"=>"140","message"=>"You are not registered with us");
                    $this->response($this->json($error), 200);
                }   
            }
            else
