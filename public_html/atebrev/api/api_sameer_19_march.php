        ...
        */
        public function sendMesages()   
        {
            ...
            {  
               ...
               {
                    $response = $this->getUserPin($fromUser,$toUser,$tempparam);

                    if($response)
                    {
                       $error[RESPONSE] = "1";
                       $error[MESSAGE] = array("message"=>"Pin successfully sent");
                    }
                    else 
                    {
                       $error[RESPONSE] = "0";
               ...
               else
               {
                   $this->funobj->SendSMSUSDnew($tempparam);
                   $error[RESPONSE] = "0";
                    $error[MESSAGE] = array("code"=>"139","message"=>"Your Request Could Not Be Processed");
                    $this->response($this->json($error), 200);
               }
            }
            else
            {
                ...
                if(preg_match( NOTALPHANUM_REGX, $message ))
                {
                   // $this->funobj->SendSMSUSDnew($tempparam);

                  //  $error[RESPONSE] = "0";
                //    $error[MESSAGE] = array("code"=>"139","message"=>"Invalid Text");
                   // $this->response($this->json($error), 200);
                ...
                {
                    $response = $this->getUserPin($fromUser,$toUser,$tempparam,$message);
                    
                    if($response)
                     {
                        $error[RESPONSE] = "1";
                        $error[MESSAGE] = array("message"=>"Information successfully sent for ".$message);
                     }
                     else 
                     {
                        $error[RESPONSE] = "0";
                ...
                else if( preg_match( '/[^a-z_\-0-9]/i', $message ) )
                {
                ...
                {
                    $tempparam['text'] = 'Hey there, follow the below instructions to try our help.
                ...
                else if($message == 'delete' || $message == 'deactiv' || $message == 'deactivate' || $message == 'deactive' )
                {
                    ...
                    if($result['prefix'] == '' || $result['prefix'] == 0 || $result['prefix'] == NULL)
                    {
                        $tempparam['text'] =  "Incalid Mobile Number. Please enter valid mobile number. ";
                        $this->funobj->SendSMSUSDnew($tempparam);
                    }
                    
                    $fromUser = $this->funobj->senderIdPrefixMatch($result['prefix'] , $fromUser);
                    ...
                    if($result)
                    {
                       ...
                       if($deleteResult['msgtype'] == 'success')
                       {
                           ...
                           $tempparam['text'] = " Your Number ".$deleteResult['msg'].'! for more info please contact support@phone91.com.'; 
                       }
                       else 
                       {
                           $response = 0;
                           $errorMessage = $deleteResult['msg'];
                           $tempparam['text'] = $deleteResult['msg'].' for more info please contact support@phone91.com.';
                       }
                       $this->funobj->SendSMSUSDnew($tempparam);
                    }
                    else 
        ...
        {
            $resellerId =  $this->funobj->longCodeResellerId($toUser,1);
            ...
            if($resellerId['prefix'] == '' || $resellerId['prefix'] == 0 || $resellerId['prefix'] == NULL)
            {
             ...
             $fromUser = $this->funobj->senderIdPrefixMatch($resellerId['prefix'], $fromUser);
            
            
            $result = $this->funobj->checkVerifiedNumber($fromUser,$resellerId['resellerId']);
            ...
            if( $userInfo['msgStatus'] == "success")
            {
                ...
                if(!empty($userInfo['userInfo']['userId']))
                {
                   ...
                   if(in_array( $message , $messageArray ))
                   {
                     ...
                     // $tempparam['text'] = "hello, Your Account balance is ".round($response['balance'],2).' '.$currency.'';
                      $tempparam['text'] = 'Hey there, your account balance is '.round($response['balance'],2).'.
                   ...
                   else 
                   {
                        ...
                        if($userInfornation)
                        {
                            ...
                            {
                                 //$tempparam['text'] = "Your 4 digit pin is ".$userInfornation['userPin']." you can use this pin to make a call";
        ...
        {    
            ...
            if($userDetails['prefix'] == '' || $userDetails['prefix'] == 0 || $userDetails['prefix'] == NULL)
            {
                $tempparam['text'] =  "Invalid Mobile Number. Please enter valid mobile number. ";
                $this->funobj->SendSMSUSDnew($tempparam);
                
                $error[RESPONSE] = 0;
                $error[MESSAGE] = "Invalid Mobile Number. Please enter valid mobile number. ";
                $this->response($this->json($error), 200);
                
            }
            
             $param['senderNo'] = $this->funobj->senderIdPrefixMatch($userDetails['prefix'], $param['senderNo']);
           ...
           // print_r($contactArray);
            ...
            else 
            {
               ...
               $allAccessNoArr = $phoneObj->allAccessNo();
         ...
         */
        function getUserIdSms($userNumber,$status = 1, $toUser)
        {
            ...
            if( $userInfo['msgStatus'] == "success")
            {
                return array( 'userId' => $userInfo['userInfo']['userId'] , 'resellerId' => $resellerId['resellerId'] , "prefix" => $resellerId['prefix'] );
            }
            else 
            {
                return 0; 
