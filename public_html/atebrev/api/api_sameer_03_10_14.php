...
//                            $error = array(RESPONSE => "0", MESSAGE => "Unable to fetch user details please try again later");
            ...
            if ($res == '0') {
                ...
                $get_userinfo = $result->fetch_array(MYSQLI_ASSOC);
...
//                $tempData[] = array('type'=>$get_userinfo["type"],'userName'=>$get_userinfo["userName"],'balance'=>$get_userinfo["balance"],'tariffId'=>$get_userinfo["tariffId"],'name'=>$get_userinfo["name"]);
                unset($get_userinfo["password"]);
                $respnse[CONTENT][] = $get_userinfo;
