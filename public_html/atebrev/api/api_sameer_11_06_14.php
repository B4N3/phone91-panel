...
//					}
	...
	private function twowaycalling() {
		...
		if ($response[RESPONSE] == "1") {
			...
			$returnResult["user"] = $this->_request['user'];
			$returnResult["data"] = $newArrMsgId;
			
		}
                else
                {
                    $returnResult = array(RESPONSE => "0", MESSAGE => array("code"=>"108","message"=>"Incomplete Details"),"user"=>$this->_request['user']);
//                    $this->response($this->json($error), 200);
                }
                
                include_once(CLASS_DIR."db_class.php");
                $dbClsObj = new db_class();
                $dbClsObj->mongo_insert("twowaycalling", $returnResult);
                $this->response($this->json($returnResult), 200);
	}
