      ...
      function listOfAllContacts()
        {
            ...
            if ($response[RESPONSE] == "1") 
            {
                ...
                if($time && strtotime($contactTime) < strtotime($time) || $time == 0)   
                {  
		...
		$parm['contact'][] = $this->_request['contact'];
...
//                if(isset($loginType) && ($loginType == "" || preg_match(NOTNUM_REGX,$loginType )|| $loginType != 1))
                if(isset($loginType) && ($loginType == ""  || $loginType != 1))
                {
                    ...
                    if (preg_match(NOTNUM_REGX, $user) ||  $userNameLen < 7 || $userNameLen > 18) {///[^a-zA-Z0-9\_\@\.]+/
                        $error[RESPONSE] = "0";
                        $error[MESSAGE] = array("code" => "102", "message" => "Invalid Number please use a valid Number");
                        $this->response($this->json($error), 200);
                    }
                    ...
                    $result = $cntClsObj->checkVerifiedNumberExist($user,1,$resellerId);
                    if($result)
                    {
                        $user  = $cntClsObj->data['userId'];
                        $checkLoginFlag = 1;
                    }
                    elseif(!$result && $cntClsObj->code != "404")
                    {
                        $error[RESPONSE] = "0";
...
//			include_once $_SERVER["DOCUMENT_ROOT"] . "/function_layer.php";
                    ...
                    if(is_null($type) && $checkLoginFlag != 1)                        
                        $result = $this->funobj->checkLogin($user, $password);
                    else
                    {
                        if($checkLoginFlag){
                            $result = $this->funobj->checkLogin($user, $password,2);
...
//                                    $this->funobj->loginFailed($user); 
                                    ...
                                    $error = array(RESPONSE => "0", MESSAGE => array("code"=>"1007","message"=>"User Account Notverified"),CONTENT=>array("beforeLoginFlag"=>$get_userinfo["beforeLoginFlag"]));
                                    if ($return) 
                                    {
                                    $error = array(RESPONSE => "0", MESSAGE => array("code"=>"1007","message"=>"User Account Notverified"),CONTENT=>array("beforeLoginFlag"=>$get_userinfo["beforeLoginFlag"],"userId"=>$get_userinfo["userId"],"resellerId"=>$get_userinfo["resellerId"]));
                                    return $error;    
                                    }else
                                    $this->response($this->json($error), 200);
                            }
...
//            #this line is for security in pricingController
            ...
            $error = array(RESPONSE => $response , MESSAGE => array( "code" => "2002" , "message" => "Currency Successfully Sent."  ) , 
                CONTENT => $content);
            }
            $this->response($this->json($error), 200);
        }
...
//             $fileName = $this->_request['fileName'];   
		...
		if ($response[RESPONSE] == "1") 
         ...
         private function checkVerifiedNumber()
         {
             
             include_once(CLASS_DIR."contact_class.php");
             error_reporting(-1);
             $cntClsObj = new contact_class();
             $contactNumber = $this->_request['number'];
             $type = $this->_request['type'];
             ...
             $result = $cntClsObj->checkVerifiedNumberExist($contactNumber,$resellerId);
             if(!$result)
             {
             ...
             {
                  $finalResponseArr[RESPONSE] = "1";
                  $finalResponseArr[MESSAGE] =  array("code"=>200,"message"=>'success');
