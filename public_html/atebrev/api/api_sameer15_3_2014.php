...
//         $this->sendErrorMail("rahul@hostnsoft.com", "Phone91 signup_class userlogin  table query fail : $qur ");
	...
	#check sender id length 
        ...
        $result = $this->funobj->selectData("userId"," 91_verifiedNumbers","CONCAT(countryCode,verifiedNumber) = '".$senderId."' and domainResellerId=".$resellerId);
        ...
        }else
        {
           ...
           $result = $this->updateTransLogAndPinStatus($resellerId,$userId,$pinDataDetail['pinBalance'],$cardNumber);
    ...
    function updateTransLogAndPinStatus($resellerId,$userId,$pinBlanace,$pin){
        ...
        $transactionObj->addTransactional_sub($pinBlanace,$pinBlanace,"Calling Card",0,0,0,"Recharge by Pin");
	...
	private function json($data) {
        ...
        */
        public function sendMesages()   
        {
            ...
            else
            {
                ...
                else if($message == 'delete' || $message == 'deactiv' || $message == 'deactivate' || $message == 'deactive' )
                {
                    ...
                    $result = $this->getUserIdSms($fromUser , 1 );
        ...
        {
            $resellerId =  $this->funobj->longCodeResellerId($toUser);
        ...
        {    
            ...
            $param['userId'] = $this->getUserIdSms( $param['senderNo']);
         ...
         */
        function getUserIdSms($userNumber,$status = NUll)
        {
            $resellerId =  $this->funobj->longCodeResellerId($userNumber);
            $resellerId = json_decode($resellerId ,true );
            ...
            {
                if($status)
                     return array( 'userId' => $userInfo['userInfo']['userId'] , 'resellerId' => $resellerId['resellerId'] );
                else
