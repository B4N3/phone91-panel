...
//                if(isset($loginType) && ($loginType == "" || preg_match(NOTNUM_REGX,$loginType )|| $loginType != 1))
                if(isset($loginType) && ($loginType == ""  || $loginType != 1))
                {
                    ...
                    include_once(CLASS_DIR."contact.php");
                    $cntClsObj = new contact_class();
                    $result = $cntClsObj->checkVerifiedNumberExist($user,1);
                    if($result)
                    {
                    ...
                    elseif(strpos($user, "@") !== false)
                    {
                        include_once(CLASS_DIR."contact.php");
                        $cntClsObj = new contact_class(); 
                        $result = $cntClsObj->checkVerifiedEmailExist($email);
...
//             $fileName = $this->_request['fileName'];   
		...
		if ($response[RESPONSE] == "1") 
         ...
         private function checkVerifiedNumber()
         {
             ...
             $type = $this->_request['type'];
             $result = $cntClsObj->checkVerifiedNumberExist($contactNumber,$type);
             if(!$result)
             {
...
Team Phone91';
   ...
   function getAllCountry(){
        $resellerId = $this->_request['resellerId'];
         ...
         $respnse[CONTENT] = array($countryRes['countryList']); 
         $this->response($this->json($respnse), 200);
