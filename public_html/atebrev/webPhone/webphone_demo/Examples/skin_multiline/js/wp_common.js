...
displaystatus: function (statustr)	//	display status massages
{
	...
	try{
	if (webphoneStatus !== null && typeof(statustr) !== 'undefined' && statustr !== null)
	{
...
initcheck: function ()	// getting applet handle
{
	...
	try{
		...
		if (browser.indexOf('msie') > 0) // for IE the values of strApplethandle is the same regardless if the applethandle is found or not
		{
...
reRegister: function ()	//	calls API_Register() every "reRegisterTime" miliseconds
{
...
// if parameter is passed; used for wp_api.Mute();
	...
	var boolMute = applethandle.API_MuteEx(wp_common.currLine, mute, 0);
...
wp_StartStopRec: function ()
{
	...
	if (recording) // if API_VoiceRecord was successful
	{
		if (!recordingStatus)
		{
			document.getElementById('btn_recording').innerHTML = 'Stop Rec';
		}else
		{
			document.getElementById('btn_recording').innerHTML = 'Start Rec';
		}
