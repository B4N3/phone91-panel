...
class SuperMySQLi extends mysqli
{
	 ...
	 *
	 * <code>
	 *  onDuplicate("id > 10"); // WHERE id > 10
	 *  onDuplicate("id > 10")->where("id < 20"); // WHERE id > 10 AND id < 20
	 *  onDuplicate(array("id > 10", "id < 20")); // WHERE id > 10 AND id < 20
	 * </code>
	 *
	 * @param string|array $restrictions
	 *
	 ...
	 * @throws InvalidArgumentException
	 */
	public function onDuplicate($restrictions)
	{
		...
		$this->addCommand('ON DUPLICATE KEY UPDATE', $restrictions);
	...
	protected function addCommand($name, $added)
	{
		...
		// $added = $this->real_escape_string($added);
