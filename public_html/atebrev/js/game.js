/*dot dot animation	
var dots = 0;
$(document).ready(function(){setInterval (type, 600);});
function type()
{if(dots < 3){ $('.dots').append('.'); dots++;}
else{$('.dots').html(''); dots = 0;}}*/

/*============js for dialpad===============*/

  $('.digits').click(function(){
		 console.log();
                 if( $('#dialPadInput').val().length >= 18)
                 {
                     return false;
                 }
		 insertAtCaret('dialPadInput',$(this).text());
		  //$('#dialPadInput').focus();
		 //$('#dialPadInput').val($('#dialPadInput').val() + $(this).text());
                 sipSendDTMF($(this).text());
		
  });
 $('.back').click(function(){
	  $('#dialPadInput').val($('#dialPadInput').val().substring(0,$('#dialPadInput').val().length - 1));
  });


/*************************startof of cursor position  js***********************************/
