...
function _load(type){
	...
	****************************************/
    ...
    if(currentPage == url1 && url2 != undefined && url2.length > 2){
        ...
        ajaxload({url:url2, loadIn:'#rightsec',success:function(){if(hash.indexOf("setting")==0)setActive(url2);tour(1);}})
					...
					if(hash.indexOf('|') != -1)
					{
						ajaxload({
							...
							success:function(){
								...
								if(hash.indexOf("setting")==0)setActive(url2);
								tour(1);
							}
						})
					}
...
function show_message(message,type)
{
	...
	if(type != undefined && type != null && message != null && message != undefined)
		toastr[type](message)
}
...
//function for pagination
function batchPagination(count,strt,divs,batchId)
{
    if(strt == undefined || strt == 0 || strt== "")
        strt=1;
		...
		$(divs).paginate({
			...
			onChange                : function(page){
				...
				window.location.href= window.location.href.split('?')[0]+'?pageNo='+page;
                               ...
                               window.location.href= window.location.href.split('?')[0]+'?pageNo='+page+'&batchId='+batchId;
                        }
...
/*********js for tour*********/
var _playTour = false;
function tour(stepNo,ts){
	//_tour is available in tourjson.js
	
	if($(ts).hasClass('tour'))
		_playTour = true;
	if(_playTour){
		
		//cut the hash into url1 & url2
		var hash, url1, url2, currentId;
		hash = window.location.hash.substring(2);
		url1 = hash;
		...
		//url match
		var url;
		if(_tour.hasOwnProperty(url1)){
			url = url1
		}
		else if(_tour.hasOwnProperty(url2)){
			url = url2
		}
		else
			return false;
		
		//reset elements before printing another step
		$('.tip, .ic-ar').remove()
		$( "#rightsec" ).tabs({collapsible: true});
		$('.addZ').removeClass('addZ')
		
		//count number of steps in every url
		var steps = []
		for (step in _tour[url]) {
			if(step != 'prev' && step != 'next')
				steps.push(step);
		}
		
		//get the next url along with respective ids at the end
		function getNewHash2(hash2){
			var hash2;
			switch (hash2)
			{
			case 'reseller-client-setting.php':
				hash2 = 'reseller-client-setting.php?clientId='+window.storage['clientId'];
				$('#usRes').trigger('click');
			  break;
			case 'batchname.php' :
				hash2 = 'batchname.php?batchId='+window.storage['clientIdBatch'];
				$('#batcha').trigger('click');
			  break;
			case 'reseller-manage-plan-setting.php':
				hash2 = 'reseller-manage-plan-setting.php?tariffId='+window.storage['planId'];
			  break;
			}
			return hash2
		}
		
		//change url after completing all steps
		if(stepNo > steps.length){
			hash = '#!'+_tour[url]['next']['hash1']+'|'+getNewHash2(_tour[url]['next']['hash2']);
			if(_tour[url]['next']['hash2'] != '')
		...
		else//print the steps.
		{
			...
			if(stepNo < 1){
				hash = '#!'+_tour[url]['prev']['hash1']+'|'+getNewHash2(_tour[url]['prev']['hash2']);
				if(_tour[url]['prev']['hash2'] != ''){
					window.location.hash = hash;
					key1 = _tour[url]['step'+steps.length];
					}
				}
			else{
				key1 = _tour[url]['step'+stepNo];
				}
			//console.log(key1);
			//console.log(key1.length);
			for(i=1;i<=key1.length-1;i++){
				htm +='<p class="tip" style="'+key1[i].cPos+'">'+key1[i].content+'</p>\
				<i style="'+key1[i].aPos+'" class="ic-ar '+key1[i].arrow+'"></i>';	
				//console.log(url,key1.length,key1[i].content, key1[i].arrow, key1[i].cPos, key1[0].tabNo)
			}
			$( "#tabs" ).tabs({ active: key1[0].tabNo});
			$('.tourBg').show();
			$(key1[0].target).append(htm);
			$('#rightsec').append('<div class="tourNav"><button class="skip" onclick="skipTour()">Skip tour</button>\
								   <button class="back mrL" onclick="tour('+stepNo+'-1)">Back</button>\
								   <button class="next mrL" onclick="tour('+stepNo+'+1)">Next</button></div>')
			
			$(''+key1[0].elements+'').addClass('addZ')	
...
//skip tour separate function
function skipTour(){$('.tip, .ic-ar').remove();$('.addZ').removeClass('addZ');$('.tourBg, .tourNav').hide();_playTour = false;}

function SetValueAddCnt(ths){
    var countryName = $(ths).attr('countryName');
    var countryCode = $(ths).attr('countryCode');
    var countryFlags = $(ths).attr('countryFlags');
    
    $(ths).parent().prev().find('.setFlag').removeClass($(ths).parent().prev().find('.setFlag').attr('flagId'));
    
    $(ths).parent().prev().find('.setFlag').addClass(countryFlags);
    $(ths).parent().prev().find('.setFlag').attr('flagId' , countryFlags);
    $(ths).parent().parent().next().find('.code').val(countryCode.replace(/ /g, ''));
    $(ths).parent().hide();
