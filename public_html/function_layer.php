...
//            $this->validateAdmin();
    ...
    {
        if(preg_match(NOTNUM_REGX, $currencyId) || empty($currencyId))
        {
            return false;
        }
        $result = $this->selectData('currency', '91_currencyDesc', "currencyId = '" . trim($currencyId) . "'");
        while ($row = $result->fetch_array(MYSQLI_ASSOC)) 
        {
            return $row['currency'];
        }
...
//            }
     ...
     */
    function getUserFromEmail($email , $resellerId = NULL, $type= '1',$conditionType=NUll ) 
    {
        ...
        if(is_null($conditionType))
             $condtion .= " and domainResellerId = '" . $resellerIdServer . "' ";
        
        $result =  $this->selectData('userid',$table,$condtion);
            ...
            else 
            {
               //$domainResellerId =  $this->getDomainResellerId($_SESSION['domain'] );
                ...
                else
                {
                    ...
                    /**    not check before login and mac address in new login 
                    ##-- code to check Ip. 
...
//        $url = CALLSERVERURL."isoData.php";
    ...
    function countryAllDetail() 
    {
        ...
        include_once($_SERVER['DOCUMENT_ROOT'].'/csvtoarray/csv.inc.php');
        $csv = new csv_uploder($_SERVER['DOCUMENT_ROOT'].'/csvtoarray/iso.csv', 2000 , ',');
        $array=$csv->getCsv();
...
//                        sip_add($userName,$password);
                       $sqlBuddies = "INSERT INTO sip_buddies (name,sippasswd,md5secret) values ('".$userName."',AES_ENCRYPT('". $password."','".ENCRYPT_KEY."'),md5(concat('".$userName."',':sip.phone91.com:','".$password."'))) on duplicate key update sippasswd=AES_ENCRYPT('". $password."','".ENCRYPT_KEY."'),md5secret=md5(concat('".$userName."',':sip.phone91.com:','".$password."'))";
...
//                            ob_end_clean(); 
    ...
    function getUserInformation($userName , $reqFlag = NULL)
    {
       ...
       //apply query for login flag
        ...
        $result = $this->selectData('userId,name,userName,AES_DECRYPT(password,"'.ENCRYPT_KEY.'") as password,isBlocked,deleteFlag,type,resellerId,sipFlag,tariffId,beforeLoginFlag,balance,userBatchId', "91_manageClient",$condition);
...
//echo $this->querry;
    ...
    /**
     * last modified by Ankit Patidar <ankitpatiar@hostnsoft.com> on 26/9/2014
     * @uses fuction use to seperate the code and number from mobile number
     * @param number $number
     * @return array
     */
...
//972,39,876,81,962,7,254,965,996,856 ,371,961,370,352,389,60,223,356,230,52,373,212,258,95,264,977,31,599,64,505,227,234,47,968,92,507,675,595,51,63,48,351,974,40,7,250,966,221,381,65,421,386,27,34,94,46,41,886,992,255,66,228,868,216,971);
	 ...
	 if(in_array($numFour, $allNumbersArray))
        {
            $code = $numFour;
            $codeExist++;
              $state = 4;
        ...
        else if(in_array($numone, $allNumbersArray))
        {
             $code = $numone;
             $codeExist++;
             $state = 1;
...
//         print_R($data);
    ...
    function getUserCurrencyAndRate($userInfo , $currency )
    {
        ...
        function checkValidUsername($userName , $resellerId)
    {
       $resellerId = 2;
        if(preg_match(NOTNUM_REGX,$resellerId ))
        {
        ...
        if(preg_match(EMAIL_REGX,$userName ))
        {
           ...
           $resultVerSel = $this->selectData('userid', '91_verifiedEmails',"email='".$userName."' and resellerId=".$resellerId);
        ...
        else if(!preg_match(NOTUSERNSME_REGX,$userName ))
        {

           $response = $this->getUserId($userName , $type = '0');
            ...
            {
                if(!preg_match(NOTMOBNUM_REGX,$userName ))
                {
                    $resultVerSel = $this->selectData('userId,verifiedNumber,countryCode,confirmCode,resellerId', '91_verifiedNumbers',"CONCAT(countryCode,verifiedNumber)='".$userName."' and resellerId=".$resellerId);
   ...
   #- This function is to get details from user login function.
    ...
    /*
     * This function is to transfer fund 
     * 
     * amount:999
     * receiverId:3
     * confirmAmount:999
     * giverId 
     * 
     * Note :: This facility is only available for users.
     * only users can transfer their amount
     * and this facility is available only for users whose reseller id is 2.
     * 
     * $giverArr['id'] = $_SESSION['id'];;
        ...
        $giverArr['amount'] = $_SESSION['amount'];;
