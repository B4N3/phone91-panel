...
class userCallLog {
    ...
    function recentCall($request, $session) {
        ...
        while ($row = $res->fetch_array(MYSQL_ASSOC)) {
            if (!in_array($row['called_number'], $checkArr)) {
                ...
                if (isset($resArr[substr($row['uniqueId'], 0, -6)]["balance"]))
                    $resArr[substr($row['uniqueId'], 0, -6)]["balance"] += $row['deductBalance'];
                else
                    $resArr[substr($row['uniqueId'], 0, -6)]["balance"] = 0;
    ...
    function userCallLogs($request, $session) {
        ...
        foreach ($allcontact as $cntValue) {
            foreach ($cntValue as $value) {
                $contactNoArr[$value['code'] . $value['contactNo']] = $value['name'];
            }
        }
        while ($row = $res->fetch_array(MYSQL_ASSOC)) {
    ...
    function getCallLogsDetails($request, $session) {
        ...
        while ($row = $res->fetch_array(MYSQL_ASSOC)) {
            ...
            $row['date'] = date('d-m-Y', strtotime($row['call_start']));
            $row['time'] = date('H:i:s', strtotime($row['call_start']));
            $resArr[] = $row;
        }
        echo json_encode($resArr);
    }
...
//end of funtion getUserCallLogForTimeLine
    ...
    function getAllChart($request, $session) {
        ...
        if (preg_match(NOTNUM_REGX, $request['userId']) || strlen($request['userId']) > 30) {///[^a-zA-Z0-9\_\@\.]+/
            echo json_encode(array('status' => 'error', 'msg' => 'Invalid user Id!!!'));
            exit();
        }

        if (preg_match(NOTNUM_REGX, $request['type'])) {
            echo json_encode(array('status' => 'error', 'msg' => 'Invalid Type!!!'));
            exit();
        }
        ...
        for ($i = 0; $i < count($graph); $i++) {
            if (!preg_match(NOTALPHANUM_REGX, $graph[$i]))
    ...
    function getCallDetailsAdmin($request, $session) {
        ...
        //	mail('vikas@walkover.in','request',json_encode($result));
    ...
    function getAllCallDetails($request, $session) {
        include_once (CLASS_DIR . 'callLog_class.php');
        $logClsObj = new log_class();

        $chainId = $this->validateUserGetChainId($_SESSION['id'], 1, $session);
        ...
        echo $logClsObj->getAllCallDetails($sdate, $edate, $chainId);
    }

    function getResellerTimeline($request, $session) {
        include_once (CLASS_DIR . 'callLog_class.php');
        $logClsObj = new log_class();
        ...
        echo $logClsObj->resellerTimeLine($sdate, $edate, $searchType, $session);
    }
    ...
    function getRouteProfitDetail($request, $session) {
        ...
        echo $logClsObj->getRouteProfitDetail($request, $session);
    }
    ...
    function getRoutePrefixProfitDetail($request, $session) {
        ...
        echo $logClsObj->getRoutePrefixProfitDetail($request, $session);
    }
...
try {
    ...
    if (isset($_REQUEST['call']) && $_REQUEST['call'] != "" && method_exists($callLogObj, $_REQUEST['call']))
