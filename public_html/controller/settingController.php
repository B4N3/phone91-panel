...
class settingController
{
    ...
    {
        $settingObj = new setting_class();
        echo $settingObj->deleteRegisterId($request['id'],$request['type'],$session['id'],$request['temp']);
    }
    ...
    function changeListenStatus($request,$session)
    {
        $settingObj = new setting_class();
        echo $settingObj->changeListenStatus($request,$session['id']);
        unset($settingObj);

    }

    function getPaymentGatewayDetails($request,$session)
    {
        $settingObj = new setting_class();
        echo $settingObj->getPaymentGatewayDetails($session['id']);
        unset($settingObj);


    }

    function editPaymentGatewayDetails($request,$session)
    {
        $settingObj = new setting_class();
        echo $settingObj->editPaymentGatewayDetails($request,$session['id']);
        unset($settingObj);
...
if(isset($_REQUEST['call']) && $_REQUEST['call'] != "" && method_exists($settingCtrlObj,$_REQUEST['call'] ))
{
