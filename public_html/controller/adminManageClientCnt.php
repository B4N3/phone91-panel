...
function changeBulkClientListenTimeStatus($request,$session)
{
 ...
 function getCurrencyList(){
    ...
    function exportClientList($request,$session)
    {
	include_once(CLASS_DIR . "reseller_class.php");
	$resObj=new reseller_class();

	if(isset($request['value']) && $request['value'] == 1){
	$allClient=$resObj->getDetailsForExport($request,$session,1);
       }else{
	$allClient=$resObj->getDetailsForExport($request,$session);   
	}
 ...
 * @author sudhir pandey <sudhir@hostnsoft.com>
 * @since 25-02-2015
 * @desc function use to get limited plan list  
 */
function getlimitedPlanList($request,$session){
     include_once(CLASS_DIR . "routeClass.php");
     $routeObj=new routeClass();
     echo $msg = $routeObj->getlimitedPlanList($request,$session['id'],$session['client_type']);
}
 ...
 * @author sudhir pandey <sudhir@hostnsoft.com> 
 * @since 25-02-2015
 * @desc function use to set user limited plan.  
 */
function setUserlimitedPlan($request,$session){
    include_once(CLASS_DIR . "routeClass.php");
    $routeObj=new routeClass();
    echo $msg = $routeObj->setUserlimitedPlan($request,$session['id'],$session['client_type']);
}


function getUserLimitedPlanDetail($request,$session){
    
    include_once(CLASS_DIR . "routeClass.php");
    $routeObj=new routeClass();
    echo $msg = $routeObj->getUserLimitedPlanDetail($request,$session['id'],$session['client_type']);
}

function getAllLimitedPlan($request,$session){
    
    include_once(CLASS_DIR . "routeClass.php");
    $routeObj=new routeClass();
    echo $msg = $routeObj->getAllLimitedPlan($request,$session['id'],$session['client_type']);
    
}

function AddLimitedPlan($request,$session){
    
    include_once(CLASS_DIR . "routeClass.php");
    $routeObj=new routeClass();
    echo $msg = $routeObj->AddLimitedPlan($request,$session['id'],$session['client_type']);
    
}

function getUserDidDivertDetail($request,$session){
    include_once(CLASS_DIR . "didDivert_class.php");
    $routeObj=new didDivert_class();
    echo $msg = $routeObj->getUserDidDivertDetail($request,$session['id'],$session['client_type']);
}

function getDidDivtViaAssignId($request,$session){
    include_once(CLASS_DIR . "didDivert_class.php");
    $routeObj=new didDivert_class();
    echo $msg = $routeObj->getDidDivtViaAssignId($request,$session['id'],$session['client_type']);
}

function editDidDivertDetail($request,$session){
   include_once(CLASS_DIR . "didDivert_class.php");
    $routeObj=new didDivert_class();
    echo $msg = $routeObj->editDidDivertDetail($request,$session['id'],$session['client_type']);
}


function getLimitedPlanPrefix($request,$session){
    include_once(CLASS_DIR . "routeClass.php");
    $routeObj=new routeClass();
    echo $msg = $routeObj->getLimitedPlanPrefix($request,$session['id'],$session['client_type']);
}

function addPrefixAndCountry($request,$session){
    include_once(CLASS_DIR . "routeClass.php");
    $routeObj=new routeClass();
    echo $msg = $routeObj->addPrefixAndCountry($request,$session['id'],$session['client_type']);
}
