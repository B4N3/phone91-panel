...
class planController {
    ...
    function exportTariff($request, $session)
    {
        $planObj = new plan_class();
        
        $data['pid'] = $request['tariffId'];
        $type = $request['type'];
        
        if($type != "csv" && $type != "xlsx")
            return json_encode(array("msg"=>"Error invalid format","status"=>"error"));
        
        $result = $planObj->getTarrifDetails($data, $session['id_tariff'],$session['id'],$session['isAdmin'],"1");
...
//        var_dump($result);
        ...
        $fileName = $planObj->exportTariffRates($result['allvaluess'],$type);
        
        
        if($fileName != FALSE)
            $planObj->downloadFile($fileName);
        else
...
try{
    ...
    if (isset($_REQUEST['call']) && $_REQUEST['call'] != "" && method_exists($planCtrlObj,$_REQUEST['call'] ))
