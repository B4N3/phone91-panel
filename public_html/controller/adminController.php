...
class adminController {
 ...
 function getDeleteUserLog($request,$session)
 {
    ...
    function getPersonalTransaction($request,$session)
    {
         include_once(CLASS_DIR."transaction_class.php");
         $transObj  = new transaction_class();
         
        // echo $request["userId"]." -- ".$request["pageNo"]; 
         
        $transactionRec =  $transObj->getPersonalTransaction($request["userId"],3,NULL,$request["pageNo"],1,$request["filterBy"]);
        $transactionRec =json_decode($transactionRec,TRUE);
//        print_r($transactionRec);
        echo json_encode(array("status" => "success" , "data" => $transactionRec ));
