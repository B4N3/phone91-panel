...
function addcontactForCTC() {
 ...
 //p91Loader('start');
	    ...
	    * @returns {undefined}	     */
	    function getAccessNumberByState(counter)
	    {
                ...
                $.ajax({
                       ...
                       {
                          if(msg.status == 'error' && counter == "edit")
                              return false;
                          if(msg.status == 'error' && counter == "1" ) 
                          { 
                              //accessNoDisplay
                               $("#accessNoErr").show();
                               $("#accessNoDisplay").hide();
                          }
                          else
                          {
                               $("#accessNoErr").hide();
                               $("#accessNoDisplay").show();
                          }

                           if(jQuery.isEmptyObject(msg.data) && state!=0)
                           {
                             // if(typeof( msg.data.hashValue )=="undefined")
                              {
                                  ...
                                  if(counter == '')
                                  {
                                      findAccess = '<div class="clear pdB2 callErr">\
                                                 ...
                                                 <span class="ddLit">Sorry you have already used the available access numbers for your other contacts.</span>\
                                                 </div>\
                                                 </div>';
   ...
   //			    }
                               ...
                               if($.inArray(key,dedicatedAN) == -1){
                                   ...
                                   if(counter == '')
                                    findAccess+='<div class="accRes clear">\
                                          ...
                                          <a class="btn btn-blue btn-medium fl mrR1" onclick="addThisAccessNumber('+key+');" href="javascript:;">New Contact</a>\
                                          <a class="btn btn-blue btn-medium fl" onclick="getexestingcontact(this,'+key+',1);" href="javascript:void(0);">Existing Contacts</a>\
                           ...
                           //console.log(gAccessNumber+' get access no');
...
//			    var str = "<option value='0'>Select State</option>";
			    $.each(stateList,function(key,value){
				   ...
				   findAccess+='<div class="accRes clear">\
                                        ...
                                        <a class="assTo btn btn-medium btn-inverse fl mrR2" onclick="showAss(this)" href="javascript:void(0);">Assign To</a>\
                                        <span class="assClick dn">\
                                       <a class="btn btn-blue  btn-medium fl mrR1" onclick="addThisAccessNumber('+accNo+',\''+value.state+'\');" href="javascript:;">New Contact</a>\
                                       <a class="btn  btn-blue btn-medium fl" onclick="getexestingcontact(this,'+accNo+',1,\''+value.state+'\');" href="javascript:void(0);">Existing Contacts</a>\
                                       <!---Existing contacts dropdown-->\
...
//			    $('#accessState_'+counter).html(str);
        ...
        function getStatesByPrefix(counter)
        {
	       ...
	       console.log(prefix +' country not found ');
...
<input type="text" name="name" value="'+ data.name+'"/>\
    ...
    str +='<select id="accessCountry_edit" onChange="getStatesByPrefix(\'edit\');" name="accessCountry" class="edit_country">\
           ...
           </select></div><div style="display:none;" id="emptyAccessNo">Sorry you have already used the available access numbers for your other contacts.</div>\
           <div id="accessNoDetails" style="display:none;" ><div class="child"><select id="accessNumber_edit" class="accessNumber_dn_edit" onChange="renderExtension(\'edit\')" name="conAccessNumber">\
...
function addContactDD(counter)
{
	...
	str+='<div id="accessBox_'+counter+'" class="accessBox dn">\
		...
		<div style="display:none;" id="accessNoErr">Sorry you have already used the available access numbers for your other contacts.</div><div id="accessNoDisplay"><select id="accessNumber_'+counter+'" class="accessNumber_dn_'+counter+'" onChange="renderExtension('+counter+')" name="conAccessNumber[]"></select>\
 ...
 $.ajax({
        ...
        success:function(msg)
        {
            ...
            $.each(countryList,function(key,value){
                ...
                if(counter == 'edit' || gcountry != null || msg.usersCountry  != 0)
                {
                    if(code == key || gcountry == key || msg.usersCountry  != 0)
                    {
                        selected = 'selected=selected';
                        gcountry = null;
                    }
...
function callcostKeyup() 
{
    ...
    if (!Regx.test(s)) 
    {
        ...
        show_message("To get the call cost Please enter a valid source number,should be min 8 digit and max 18 digit and number only !", "error");
        return;
    }
