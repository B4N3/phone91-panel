...
var html ='<div class="wrapper">\
        ...
        var signupUrl = window.location.host+'/signUpWLabel.php';    
        $.each(data, function(key, item ){
			var start='<td><a class="tbbtn" href="http://'+signupUrl+'">Start</a></td>';
...
function getDefaultTariff()
{
    $.ajax({
        ...
        type:"post",
        dataType:"JSON",
        data:{"action":"getDefaultTariffList"},
        success:function(reponse){
            console.log(reponse);
            
            
            if(reponse != null && reponse.length > 0 && reponse != "")
            {
                var str = "";
                $.each(reponse, function(key,value){
                    
                    str += '<option value='+value.tariffId+'>'+value.currency+'</option>';
                })
                $('#tariff').html(str);
            }
            else
                show_message(reponse.msg,reponse.status);
