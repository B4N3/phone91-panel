...
//			    }
			  ...
			  var res = accStr.substr(0, accStr.length-1);
...
<input type="text" name="name" value="'+ data.name+'"/>\
    ...
    str += '<div class="child">';
    str +='<select id="accessCountry_edit" onChange="getStatesByPrefix(\'edit\');" name="accessCountry">\
		</select></div>\
           <div class="child"><select id="accessState_edit" onChange="getAccessNumberByState(\'edit\')" name="accessState"></select></div>\
           <div class="child"><select id="accessNumber_edit" class="accessNumber_dn_edit" onChange="renderExtension(\'edit\')" name="conAccessNumber"></select></div>\
...
function showCountryForAdd(ths){
    if($(ths).next().is(':visible')){
		$(ths).next().hide();
	}
	else{
		$(ths).next().show();
...
function validateNumber(ths,type)
{
    ...
    else
    {
        ...
        $("#response").show().addClass("error_red").html("Please enter number ( Minimum length 8, Maximum length 18)");
    }
    $("#source").focus();
    return false;
      }
  ...
  if (d.length < 8 || isNaN(d) || d.length > 18)
  {
    ...
    $("#response").show().addClass("error_red").html("please enter number (minimum length 8 , maximum length 16)");
      }
      $("#dest").focus();
      return false;
  }
...
function callcostKeyup() {
    ...
    var Regx = /^[0-9]{8,15}$/;
    if (!Regx.test(s)) {
        show_message("Source number are not valid,enter atleast 8 digits !", "error");
        return;
    }
if(d.length >=5){ 
    $.ajax({
