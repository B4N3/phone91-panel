        ...
        success: function(msg)
        {
           ...
           var str="";
            ...
            if(str == "" || msg.pages <= 1)
             ...
             planPagination(page,msg.pages,tariffId,limit,type,"getDetails");
        }
...
function planPagination(start,count,tariffId,limit,type,funType,keyword,isAdmin,searchType)
 {
                    ...
                    $('#pagination').paginate({
                        ...
                        onChange                : function(page){
                         ...
                         if(funType == "getDetails")
                            ...
                            searchTariff(keyword,isAdmin,searchType,page); 
...
function editPlan(key)
{
    ...
    if(/[^0-9]+/.test(prefix))
    {
        $('#prefix_'+key).addClass('error');
        show_message("Invalid Prefix please enter a valid prefix value number only","error");
        return false;
    }
    if(/[^a-zA-Z\s]+/.test(country))
    {
        $('#country_'+key).addClass('error');
        show_message("Invalid Country please enter a valid Country Name Only alphabets and space are allowed","error");
        return false;
    }
    if(/[^0-9\.]+/.test(rate))
    {
        $('#rate_'+key).addClass('error');
        show_message("Invalid rate please enter a valid rate only number or decimal value are allowed","error");
        return false;
    }
    
    if(/[^a-zA-Z\s]+/.test(operator))
    {
        $('#operator_'+key).addClass('error');
        show_message("Invalid operator please enter a valid operator value only alphabets and space are allowed","error");
        return false;
    }
...
function searchTariff(keyword,isAdmin,type,page)
{
    /*@AUTHOR : SAMEER RATHOD 
...
//    var keyword = ths.val();
    ...
    if(type == "num")
    {
        if(/[^0-9]+/.test(keyword))
        {
            show_message("Error Invalid Keyword please provide valid numeric keyword to search","error");
            return false;
        }
    }
    else{
        ...
        {
            show_message("Error Invalid Keyword please provide valid alphabet only keyword to search","error");
            return false;
        }
    }
    
    
    if(/[^0-9]+/.test(tariffId) || tariffId == "" || typeof tariffId === undefined)
    {
        show_message("Error Invalid Keyword please provide valid keyword to search","error");
        return false;
    }
...
_globalTimeOut = setTimeout(function(){
    ...
    $.ajax({
        ...
        success:function(response)
        {
            ...
            {
                if(isAdmin)
                    str = prepareRowAdmin(response.data);
                else
                    str = prepareRow(response.data);


                $('#tbody').html(str);
                
                if(str == "" || response.pages <= 1)
                    $('#pagination').hide();
                else
                    $('#pagination').show();
                console.log(response.pages);
                planPagination(page,response.pages,tariffId,limit,"","search",keyword,isAdmin,type);
            }
        }
    })
...
function genSelectValidation(value, element){
    /* @AUTHOR: SAMEER RATHOD 
     * @desc : custom validation function for select option required field validator
     **/
        if(value == "Select" || value == "" || value == null)
...
$.validator.addMethod("planName", planNameValidation, "Please enter valid data characters( a-zA-Z0-9\@\_\-\s ).");
...
$.validator.addMethod("genSelect", genSelectValidation, "Please select a valid plan");
...
console.log(i);
            ...
            if((/[^a-zA-Z\s]+/.test(value1)) || value1 == "")
                {
                    console.log(value1);
                    flag = 1;
            ...
            if((/[^a-zA-Z\s]+/.test(value2)) || value2 == "")
                {
                    console.log(value2);
                    flag = 1;
...
function validateAdminEditPlanForm(id)
{
    console.log(id);
   id = $('#'+id).val();
    $('#'+id).validate({
