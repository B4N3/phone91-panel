...
<body>
  <section class="container">
    <div class="login">
      <h1>Login Html to PDF convert</h1>
      <form id="loginForm" name="loginForm">
        <p><input type="text" name="userName" value="" placeholder="Username"></p>
        <p><input type="password" name="password" value="" placeholder="Password"></p>

        <p class="submit"><input type="button" name="commit" onclick="Login();" value="Login"></p>
      </form>
      <div><span id="errorMsg" name="errorMsg"></span></div>
    </div>
    ...
    function Login(){
         ...
         $.ajax({
                        type: "POST",
                        url: "tempController.php?action=login",
                        dataType: 'json',
                        data: $('#loginForm').serialize(),
                        success: function(data) {
                            
                            if(data.status == "success"){
                                window.location.href = "generateAuth.php";
                            }else
                               $('#errorMsg').html("SORRY... YOU ENTERD WRONG ID AND PASSWORD... PLEASE RETRY..."); 
