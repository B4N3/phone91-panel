...
<!--Inner Container-->
	...
	<div class="whiteWrapp mrT2 mrR2 fl" style="width:300px">
            <form id="fundtransfer">
                    ...
                    <div class="urBox mrT2 pd1" id="userRec" style="display:none;" > USER WILL RECEIVE <span class="blue"  id="receivingAmt"> 0 <?php echo $_SESSION['currencyName']; ?> </span></div>
...
function getUserCurrencyAndRate()
{
    ...
    $.ajax({
        ...
        dataType:"JSON",
            success:function(data)
            {
                if(typeof(data.content) != "undefined" && data.status == 'success' ) 
                {
                    ...
                    if(  amount.length > 0   )
                    {
                     ...
                     //toFixed(2)
                        ...
                        console.log('nidhi');
                        console.log(accAmount.test(amount));
                        
                        if(newAmt != 0 && accAmount.test(amount))
                        {
                           $('#userRec').show();
                        }
                        else
                     ...
                     $('#userInfoError').hide();
                     $('#transferTo').removeClass('error');
                    $('#userInfoError').text('Please Enter Valid User Information.');
                }
                else
                {
                    $('#userInfoError').show();
                    $('#transferTo').addClass('error');
                    $('#userInfoError').text(data.message);
                    $('#amount').val('');
                    $('#recRate').val('');
                    $('#receivingAmt').text(' 0 <?php echo $_SESSION['currencyName']; ?>');
                }
            } 
        })
}
...
$('#transfer').click(function() 
{
   ...
   // var accAmount = /^((\d+(\.\d *)?)|((\d*\.)?\d+))$/;
    ...
    if(userInfo.length <= 0  )
    {
       ...
       $('#transferTo').addClass('error');
       return;
    }else if ($('#transferTo').hasClass( "error" ))
    {
        return;
    }
    else
    ...
    console.log(recRate+' RATE');
    console.log($('#recRate').val());
    console.log($('#recRate'));
