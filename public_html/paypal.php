...
// Setup class
if($paypalMode)
  require_once('paypal.class_ankit.php');
else  
        ...
        if($paymentStatus == 'Reversed') //Reversed
	{
        ...
        if($dbIp == $ip and $recharge == $actUserCharge)//validate with ip and talktime and recharge amount
        {
            ...
            if($updatedFlag == 0)
            {
                ...
                //mail response
                if(!$mailErr->sendmail_mandrill($errorMails, 'Balance not updated while paypal recharge','Avialable details:userId:'.$idClient.' talktime:'.$dbTalktime,BUSINESSMAIL))
                {
                        trigger_error('problem in mail:\nmailto'.print_r($errorMails,1).'\nsub:'.$subject.'\nmsg:'.$message);
                }
