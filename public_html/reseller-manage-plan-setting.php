...
.bar { background-color: #aaa; width:0%; height:20px; border-radius: 3px; }
...
<div id="manage-plan-setting">
    ...
    <div id="mnpSettLeft">
        <h3 class="h3wico fl">
            <span class="fl" id="planNameSpan"><span class="hideInTablet">Manage</span> <?php echo $planArr['planName']; ?></span> 
		...
		<select name="" class="selectretail fr mrL" onchange="getTariffDetails($('#currentSelected').val(),$(this).val())">
			...
			<option>25</option>
                ...
                <a target="_blank" href="/controller/managePlanController.php?tariffId=<?php echo $_REQUEST['tariffId']; ?>&type=csv&call=exportTariff" ><input type="button" value="Export Csv"/></a>
                <a target="_blank" href="/controller/managePlanController.php?tariffId=<?php echo $_REQUEST['tariffId']; ?>&type=xlsx&call=exportTariff" ><input type="button" value="Export xls"/></a>
        ...
        <table border="0" cellspacing="0" cellpadding="0" id="" class="cmntbl boxsize mrT1">
            <thead>
                <tr>
                    <th><input type="checkbox" id="checkAll" onchange="toggleCheckbox(this)" /></th>
                    <th><input type="text" value="" id="cCodeSearch" placeholder="Country Code" onkeyup="searchTariff($(this).val(),0,'num')" style="width:100px" /></th>
                    <th><input type="text" value="" id="cNameSearch"placeholder="Country Name" onkeyup="searchTariff($(this).val(),0,'alpha')"/></th>
                    <th>Operator</th>
                    <th>Rate(<?php echo $planArr['currency']; ?>)</th>
                    <th>Action</th>
                </tr>
            </thead>
    ...
    <form name="appendForm"   id="appendForm" method="post"  enctype="multipart/form-data" onsubmit="return validateInsertRows();">
            ...
            <div class="box-imp">
                ...
                <div id="sel-wrap" class="sel-wrap">
                    ...
                    <div class="clear mrT2">
                        <span class="fl mrR">
                            <input type="checkbox" name="planWith" onchange="$('#planValue').toggleClass('required number');toggleAttribute($(this),'planValue');"/>
                            &nbsp;&nbsp;With&nbsp;&nbsp;
                ...
                <div id="imp-wrap" class="dn">
                    ...
                    <div class="clear mrT">
                        <span class="fl mrR">
                            <input type="checkbox" name="importWith" onchange="$('#importValue').toggleClass('required number');toggleAttribute($(this),'importValue');" />
                            &nbsp;&nbsp;With&nbsp;&nbsp;
                    ...
                    <div>
                        <select name="fileCurrency" style="width:190px;" class="currencyList">
                ...
                <div class="fields">
                    ...
                    <input type="text" name="billingSec" value=""  />
                                ...
                                <input class="btn btn-medium btn-primary" value="Append" type="submit" name="append" title="Append" onclick="$('.progress').show();" />
            </div>
        </div>
		</div>
    </form>
    <div id="dialog-confirm" title="Are You Sure you want to delete this entry?" style="display: none;">
        <p><span class="ui-icon ui-icon-alert" style="float:left; margin:0 7px 20px 0;"></span>This tariff will be permanently deleted and cannot be recovered. Are you sure?</p>
    </div>
	<div class="cl"></div>
</div>
...
function toggleAttribute(ths,id)
{
    if($(ths).is(":checked"))
    {
        $("#"+id).attr("min","1");
...
$(document).ready(function(){
        getTariffDetails('<?php echo $_REQUEST['tariffId']; ?>',$('.selectretail :selected').val(),'user',<?php echo $pageNo;?>);
         $('#pagination').show();
        selectPlan();
    ...
    $.ajax({
        ...
        success: function(response){
            ...
            if(response.status == "success")
            {
                ...
                $('#planLi_'+tariffId+' h3').html(planName);
