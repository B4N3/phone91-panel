...
//            error_reporting(-1);
        ...
        $usrIdTyp = trim($this->_request['userIdType']);
...
//                if(isset($loginType) && ($loginType == "" || preg_match(NOTNUM_REGX,$loginType )|| $loginType != 1))
        if (isset($loginType) && ($loginType == "" || $loginType != 1)) {
            if (empty($loginType)) {
                $error = array(RESPONSE=>"0",MESSAGE=>array("code" => "1001", "message" => "Invalid Login type please read the api documentation"));
                $this->response($this->json($error), 200);
            }
        }

        #check the username length if it is greate then 40 then log the failed attempt and return the user form here only
        #this validation is implemented later 
        $userNameLen = strlen($user);
        if($userNameLen < 3 || $userNameLen >40){
                $error = array(RESPONSE=>"0",MESSAGE=>array("code" => "1018", "message" => "Error Invalid user name should try with a valid user"));
                $this->response($this->json($error), 200); 
        }
//        var_dump($usrIdTyp);
        if(isset($usrIdTyp) && $usrIdTyp == 1){
             if(preg_match(NOTNUM_REGX,$user)){
                $error = array(RESPONSE=>"0",MESSAGE=>array("code" => "1019", "message" => "Error Invalid user Id please provide a valid user ID"));
                $this->response($this->json($error), 200); 
                return false;
            }
            $userIdType = 2;
                ...
                if (ctype_digit($user)) {
                    ...
                    if (empty($resellerId) || preg_match(NOTNUM_REGX, $resellerId) || strlen($resellerId) > 40) {
                        $error = array(RESPONSE=>"0",MESSAGE=>array("code" => "1000", "message" => "Invalid Reseller please try again"));
                        $this->response($this->json($error), 200);
                    }

                    #validate user
                    if (preg_match(NOTNUM_REGX, $user) || $userNameLen < 7 || $userNameLen > 18) {///[^a-zA-Z0-9\_\@\.]+/
                        $error = array(RESPONSE=>"0",MESSAGE=>array("code" => "1002", "message" => "Invalid Number please use a valid Number"));
                        $this->response($this->json($error), 200);
                    }
        ...
        //                            $user  = $cntClsObj->data['userId'];
                        ...
                        $userIdType = 1;
//                        $error = array(RESPONSE=>"0",MESSAGE=>array("code" => "1005", "message" => "Error no data found"));
//                        $this->response($this->json($error), 200);
                    }
                    ...
                    if ($result && !empty($cntClsObj->data['userId'])) {
                        $user = $cntClsObj->data['userId'];

                        $checkLoginFlag = 1;
                    } else {
                        if(empty($cntClsObj->msg))
                        {
                            $cntClsObj->code = "1021";
                            $cntClsObj->msg = "Error user not found";
                        }
                        $error = array(RESPONSE=>"0",MESSAGE=>array("code" => $cntClsObj->code, "message" => $cntClsObj->msg));
                        $this->response($this->json($error), 200);
                    }
                    ...
                    if (preg_match(NOTUSERNAME_REGX, $user) || strlen($user) < 5) {///[^a-zA-Z0-9\_\@\.]+/
                        $error = array(RESPONSE=>"0",MESSAGE=>array("code" => "1006", "message" => "Invalid user name please use a valid user name"));
                        $this->response($this->json($error), 200);
                    }
...
//var_dump($password);
            if (preg_match(NOTPASSWORD_REGX, $password) || strlen($password) < 5) {///[^a-zA-Z0-9\.\@\$\-\_]+/
                $error = array(RESPONSE=>"0",MESSAGE=>array("code" => "1018", "message" => "Invalid password please try again with a valid password"));
                $this->response($this->json($error), 200);
            }
...
//            $password = $pin; //$pin variable can be removed
            ...
            if (!preg_match('/^[a-f0-9]{32}$/', $auth) || empty($auth)) {
                ...
                $error = array(RESPONSE=>"0",MESSAGE=>array("code" => "1009", "message" => "Invalid authentication key please provide a valid authentication key"));
                $this->response($this->json($error), 200);
            }

            if ($auth != $password) {///[^a-zA-Z0-9\_\@\.]+/
                
                $error = array(RESPONSE=>"0",MESSAGE=>array("code" => "1010", "message" => "Invalid authentication key please contact provider"));
                $this->response($this->json($error), 200);
            }
            //auth key
            $passwordType = 3;
            ...
            $error = array(RESPONSE=>"0",MESSAGE=>array("code" => "1011", "message" => "Invalid authentication type please contact provider"));
            $this->response($this->json($error), 200);
        }
        ...
        if (!empty($user) && !empty($password)) {
            ...
            if (($loginAttampt > 10)) {
               ...
               $error = array(RESPONSE=>"0",MESSAGE=>array("code" => "1012", "message" => "Maximum Number of request exceed"));
...
//			include_once $_SERVER["DOCUMENT_ROOT"] . "/function_layer.php";
            ...
            if (!$result) {
                ...
                $error = array(RESPONSE=>"0",MESSAGE=>array("code" => "1013", "message" => "Unable to fetch user details please try again later"));
//                            $error = array(RESPONSE => "0", MESSAGE => "Unable to fetch user details please try again later");
            ...
            if ($res == '0') {
                ...
                if ($userIdType == 2 && ctype_digit($user) && isset($this->_request['accessToken'])) {
                    ...
                    if(empty($this->_request['accessType']) || ($this->_request['accessType'] != 1 && $this->_request['accessType'] !=2)){
...
//                          $respnse["msg"] = "Valid User";
                if ($return) {
                    ...
                    $respnse[CONTENT]['password'] = $getUserInfo["password"];
                    return $respnse;
                } else
                {
...
//            var_dump($response);
        if ($response['verified'] == 'true' || $response['verified_email'] != "") {
            ...
            if ($signupObj->check_email_avail($response['email'], $domainResult['domainName'],1)) {
                if ($accessType == 2) {
                ...
                if(empty($param['firstName']))
                    $param['firstName'] = "WeMissYourName";
...
//                    $response = json_decode($response , true);
                ...
                $userDetails = $this->funobj->getUserFromEmail($response['email'], $this->_request['resellerId'], '1',1);
                ...
                if ($userDetails) {
                    ...
                    foreach ($userDetails as $row) {
                        //print_r($row);
                        $resResellerId = $this->funobj->getResellerId($row['userid']);
                        if(!$resResellerId){
                            $error = array(RESPONSE => "0", MESSAGE => array("code" => "2005", "message" => "Error can not get reseller Id"));
                            $this->response($this->json($error), 200);
                        }
...
//                        $emailRecord[] = array('userName' => $userId, "password" => $password, "type" => $userInfo['type'],"resellerId"=>$resResellerId);
                        $emailRecord[] = $userInfo;
                    }

                    $error = array(RESPONSE => "1", MESSAGE => array("code" => "2002", "message" => "This user is already registered with us!!"), CONTENT => $emailRecord);
                } else {
                    $error = array(RESPONSE => "0", MESSAGE => array("code" => "2003", "message" => "Unable to  fetch user details please Try Again later!!"));
                }
            }
        } else {
...
//    }
     ...
     /**
     * @last updated by Ankit Patidar <ankitpatidar@hostnsoft.com> on 13/10/2014 curl added 
     */
    private function currencyList() {

//prepate url to get currency list
        $url = CALLSERVERURL."controller/pricingController.php?action=getDefaultTariffList";

        //curl to get currency list
        $ch = curl_init($url);
...
//		echo $error = '404 balance not found!!!';           
    ...
    private function twowaycalling() {
        ...
        $response = $this->login(true);
        if ($response[RESPONSE] == "1") {
            $nine["login"] = $response[CONTENT]['userName'];
            $nine["password"] = $response[CONTENT]['password'];
...
//            print_r($newArrMsgId);
//            die();
            if($newArrMsgId['status'] == "error"){
                $returnResult[RESPONSE] = "0";
                $returnResult[MESSAGE] = array("code" => "7005", "message" => $newArrMsgId['msg'] );
    ...
    private function changepassword() {
        ...
        if ($response[RESPONSE] == "1") {
            ...
            if($pinFlag){
                ...
                $result = $this->funobj->updateData( $data,'91_userLogin', "userId = '" . $response["id"] . "' ");
            }else{
...
//             if(!preg_match(NOTPHNNUM_REGX,$number) || preg_match(NOTNUM_REGX,$countryCode) || empty($number)){
        ...
        $cntClsObj = new contact_class();
//        $particular = $number;
        $condition = "CONCAT(countryCode,verifiedNumber) = '" . $number . "' and confirmCode='".$confirmCode."'";
...
//        $result = $cntClsObj->getVerifiedNumber($particular, 2, 'userId,confirmCode', $resellerId);
        $result = $cntClsObj->selectData('userId,confirmCode',"91_verifiedNumbers",$condition);
        
//             var_dump($cntClsObj->data);
        if (!$result) {
            $error = array(RESPONSE => "0", MESSAGE => array("code" => "7906", "message" => "Error unable to fetch details"));
            $this->response($this->json($error), 200);
        }
...
//    }
            ...
            if ($time && strtotime($contactTime) < strtotime($time) || $time == 0 || empty($contactTime)) {
                ...
                foreach ($allContactArr as $contactDetail) {
                    ...
                    $contactArray[] = array("contactNumber" => (isset($contactDetail['contactNo']) ? $contactDetail['contactNo'] : ""),
                        ...
                        "contactId" => (isset($contactDetail['contact_id']) ? (string) $contactDetail['contact_id'] : ""),
			"countryName" => (isset($contactDetail['countryName']) ? $contactDetail['countryName'] : ""),
			"stateName" => (isset($contactDetail['stateName']) ? $contactDetail['stateName'] : "")
			);
                }
                ...
                if (!empty($contactArray)) {
                    $respnse[RESPONSE] = "1";
                    $respnse[MESSAGE] = array("code" => "144", "message" => count($contactArray) . " contacts found successfully!!" , "lastUpdateTime" => strtotime($time) );
                    $respnse[CONTENT] = $contactArray;
                } else {
                    $respnse[RESPONSE] = "0";
                    $respnse[MESSAGE] = array("code" => "143", "message" => "Contacts not found!!");
   ...
   public function addContact() {
        ...
        if ($response[RESPONSE] == "1") {
            ...
            if ($response['status'] == "success") {
                $error[RESPONSE] = "1";
                $error[MESSAGE] = array("code" => "", "message" => $response['msg'],'lastContactId' => $response['lastContactId']);
            } else {
                $error[RESPONSE] = "0";
        ...
        if (isset($this->_request['deptId']) && isset($this->_request['customerNum']) and isset($this->_request['token'])) {
            ...
            while (count($numbers)) {
                ...
                while ($callResp == 'DIALING') {
                    ...
                    $callResJson = $call_obj->callResponse($param,$userId);
...
//      die();
    ...
    private function cutCall(){
        
        $response = $this->login(true);

        if ($response[RESPONSE] == "1") {
            $uniqueId = $this->_request['callId'];
            $userId = $response['id'];
            include_once(CLASS_DIR."call_class.php");
            $clsObjNew = new call_class();
            $result = $clsObjNew->cutCurrentCall($uniqueId,$userId);
            
            $error = array(RESPONSE => ($result?"1":"0"), MESSAGE => array("code" => $clsObjNew->code, "message" => $clsObjNew->msg));
            $this->response($this->json($error), 200);
        }
