        	...
        	<table width="100%" border="0" cellspacing="0" cellpadding="0" id="trstable" class="cmntbl boxsize cf">
                ...
                <tbody>
					 <?php
						 ...
						 foreach($transData['detail'] as $trans) { ?>
                                                     <tr class="">
                                                        ...
                                                        <td class="alR closeBalance"><?php 
                                                        $closeBal = round($trans['closingBalance'],2);
                                        if($closeBal > 0)
                                          echo $closeBal.' Dr';
                                        else if($closeBal < 0)
                                        { 
                  ...
                  $closeBalance = round($totalClosingBalance,2);
                      if($closeBalance > 0)
                      {
                        $totalClosingBalance =  'you need to take '.$closeBalance ." ". $userCurrency;
                        $closeClass = 'debit';
                      }
                      else if($closeBalance < 0)
                      { 
                        $closeBalText = substr($closeBalance, 1,strlen($closeBalance)-1);
                       $totalClosingBalance= 'you need to give services of '.$closeBalText." ".$userCurrency;
                       $closeClass = 'green';
                ...
                <tr >
                        <td class="alC" colspan="5">Net total</td>
