/**
 * 
 * @returns {undefined}validate Subsite add and add new subsite
 */
function addSite(){
    com_name = $.trim($("#com_name").val());
    dom_name = $.trim($("#dom_name").val());
    CopyWrite = $.trim($("#CopyWrite").val());
    language = $.trim($("#language").val());
    Theme = $.trim($("#Theme").val());
    $("input").removeClass("error_red");
    $("input").next().html('');
    if(com_name.length == 0 || dom_name.length == 0 || CopyWrite.length == 0 || language == 'Select' || Theme.length == 0){
	checkEmpty(com_name,'#com_name', 'company name');
	checkEmpty(dom_name,'#dom_name', 'domain name');
	checkEmpty(CopyWrite,'#CopyWrite', 'copywrite');
	checkEmpty(language,'#language', 'language');
	checkEmpty(Theme,'#Theme', 'theme');
	return false;
    }else{
	var options = { 
		success:       showResponse  // post-submit callback 
	};
	$("#add_subsite").ajaxForm(options)
	return true;
    }
}
...
function phone_confirmation(){
	...
	$.ajax({
		...
		.done(function(msg){
			...
			showMessage("Successfully verified", "#confirmation_code", "success");
		    }else if(msg == 2){
			showMessage("Number already in use", "#confirmation_code", "warning");
		    }else if(msg == 3){
			showMessage("Number already confirmed by you", "#confirmation_code", "warning");
		    }else if(msg == 4){
			showMessage("Code doenot match", "#confirmation_code", "warning");
		    }
		})
   }
}
...
function email_confirmation(l){
	...
	$.ajax({
		...
		.done(function(msg){
		    showMessage(msg, "#confirmationCode", "success");
		})
   }
...
function updateContact(){
	...
	$("#mobileNumber").focus();	
	showMessage("Only Digits", "#mobileNumber", "warning");
    }else{
	$.ajax({
	...
	.done(function(msg){
	    if(msg == 0){
		    showMessage("Check number", "#mobileNumber", "warning");
	    }else if(msg == 1){
		    showMessage("Number Change Successfully and verification code send to your number", "#mobileNumber", "success");
	    }else if(msg == 2){
		    showMessage("Number used by other user", "#mobileNumber", "warning");
	    }else if(msg == 3){
		    showMessage("Number already confirmed by you", "#mobileNumber", "warning");
	    }else if(msg == 4){
		    showMessage("Cannot send confirmation code", "#mobileNumber", "warning");
	    }else if(msg == 5){
		    showMessage("You have exceeded the maximum number of trial", "#mobileNumber", "warning");
	    }
...
function change_pass(){
	...
	if(c.length <1 || n1.length <1 || n2.length <1)	{
	    ...
	    showMessage("New Password are not matched", "#new_pwd", "warning");
	}
	else{
		$("#loading_img").show();
...
function change_email(){
	...
	if( n1.length <1 || n2.length <1){
	   ...
	   checkEmpty(n2,'#confirm_emailid','email');
	    ...
	    $("#new_emailid").focus();
	    showMessage("Enter correct email", "#new_emailid", "warning");
	}else if(!emailReg.test(n2)){
	    $("#confirm_emailid").focus();	
	    showMessage("Enter correct email", "#confirm_emailid", "warning");
	}else if(n1 != n2){
	    $("#confirm_emailid").addClass("error_red");
	    $("#new_emailid").focus();	
	    showMessage("Email doesnot match", "#new_emailid", "warning");
	}else{
		$("#loading_img").show();
		$.ajax({type: "GET",url: "action_layer.php?action=change_emailid",data: { new_emailid: n1,confirm_emailid: n2},success: function(msg)
		{
			...
			if($.trim(msg)==1){	
			    ...
			    $("#confirm_emailid").addClass("error_red");
			    showMessage("Emailid is not proper please choose another one.", "#new_emailid", "warning")
			}else if(msg==4){
			    showMessage("This email already registered.", "#new_emailid", "warning");
                         }else if(msg == 5){
			     showMessage("You have exceeded the maximum number of trial", "#mobileNumber", "warning");
			 }
		}
...
function edit_funds_transfer(form_id){
	...
	$("#amt_"+form_id).focus();	
	showMessage("Only Digits", "#amt_"+form_id, "warning");
    }else{
	$.ajax({
	...
	.done(function(msg){
	    if(msg == 1){
		$("#button_text").focus();
		$("#amt_"+form_id).attr('value','');
		$("#amt_"+form_id).removeClass('error_red');
		$("#amt_"+form_id).next().html("");
		showMessage("Fund Added", "#button_text", "success");
	    }
	})
    }
}
...
function validatePinForm(){
    ...
    if(bname.length <1 || totalPins.length <1 || amount.length <1)	{
	 ...
	 if(!$.isNumeric(amount)){
		    $("#amount").focus();
		    showMessage("Only numeric", "#amount", "warning");		    
	    }
	    if(!$.isNumeric(totalPins)){
		    $("#totalPins").focus();	
		    showMessage("Only numeric", "#totalPins", "warning");	   
	    }
     return false;
     
    }else if((document.getElementById('underMyResellerInpt2').checked) && ($("#underReseller").val().length <1)){
	$("#underReseller").focus();
	showMessage("Enter reseller name", "#guid", "warning");
	return false;
    }else{
	var options = { 
		success:       showResponse  // post-submit callback 
...
function feedback_submit(captcha){
	...
	$("#user_email").focus();	
	showMessage("Enter proper email id", "#user_email", "warning"); 
     }else{
	 $.ajax({
		url:'../action_layer.php?action=feedbackform'+'&mailid='+user_email+'&msg='+msg+'&number='+user_number+'&magic='+captcha,
		success: function(msg){
			...
			show_message('Your feedback not submitted..', 'error');
		}
	   });
    }
}
...
function checkEmpty(value,id,item){
     if( value.length <1){
	$(id).focus();	
	showMessage("Enter "+item, id, "warning")
    }
}
...
$.ajax({type:"GET",url:url1,dataType: "json",success:function(msg){
	...
	if(msg.status=="success"){
		...
		wooYayIntervalId = setInterval ( "checkStatus('"+msg.msg+"')", 5000 );
...
function GetAll()
{
        ...
        else if(document.getElementById('cashu').checked)
        {
