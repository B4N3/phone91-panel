            ...
            success:function(data){
                ...
                if(data.status == 1)
                {
                    ...
                    $.each(data.callFailedData,function(key,value)
                    {
                        ...
                        {
                            var type = '1';
                            var url = 'http://voice.phone91.com/wow/index.php#!manage-client.php?q='+$.base64.encode(data.chainIds.chainIds[value.chainId])+'&qs='+$.base64.encode(value.userId)+'&type='+type+'|batchDetail.php?batchId='+value.batchId+'&token='+$.base64.encode(data.chainIds.chainIds[value.chainId]);
                        }
                        else 
                        {
                             var type = '0';
                             var url = 'http://voice.phone91.com/wow/index.php#!manage-client.php?q='+$.base64.encode(data.chainIds.chainIds[value.chainId])+'&qs='+$.base64.encode(value.userId)+'&type='+type+'|transactional.php?clientId='+value.userId+'&tb=0';
                        }
                        
                        str +='<tr class=""><td><a href="'+url+'" >'+data.chainIds.chainIds[value.chainId]+'</a></td><td>'+value.telNum+'</td>\
