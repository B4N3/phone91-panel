...
$selRes = $funobj->selectData("*", "91_limitedPlan");
if($selRes)
{
...
<div class="quicKseachsec subPageSrch" style="overflow: visible;">
    ...
    <div class="quickSearch">
	 ...
	 <div>
		...
		<span><input name="lessThan" type="checkbox">balance < <input name="amtToCheck" type="float" style="width: 50px;" >USD  </span>
		...
		<span><input name="selectCountry" type="checkbox">Select country
			<div class="selectBoxWrapper">
			    ...
			    <div class="codeInput">
				<input name="code" style="margin:0;" type="text" id="code" class="codeSource" value="91" onkeyup="updateFlag(this)"/>
				   
    </div>
                                     </div></span>
                
               <span><input name="selectLimitedPlan" type="checkbox">Select Limited Plan
			<div class="selectBoxWrapper">
                            <select class="" name='selectPlan' id='selectPlan'>
                                          <?php 
                                               foreach($limitedPlanData as $key =>$val)
                                               {
		...
		<input class="mrT1 isInput100 btn btn-medium btn-primary" type="submit" value="filter">
             ...
             <input id="accessBtn" style="display:none;" onclick="sendPushNotificationOrSMS(2)" class="mrL btn btn-medium btn-primary fr mrT" type="button" value="ChangeAccess">
	     <input id="sendBtn" style="display:none;" onclick="openDialog()" class="mrL btn btn-medium btn-primary fr mrT" type="button" value="Send">
             <input id="changeIpBtn" style="display:none;" onclick="openIpDialog()" class="mrL btn btn-medium btn-primary fr mrT" type="button" value="Ip change">
	 </div>
	 
	 <a href='javascript:;' style="position: relative;top: 11px;left: 24px;" onclick="exportClientList()">Export Clients</a>
    </div>
...
<div id="Ipdialog" title="Send" style="display: none;">
    Perse: <input type="radio" name="perseUdp" value="perse">
    Udp:  <input type="radio" name="perseUdp" value="udp">
    <div>
    Port : <input type="text" name="port" id="port"/>
    </div>
    <div>
    Ip : <input type="text" name="ipdata" id="ipdata"/>
    </div>
    <input type="button" onclick="sendPushNotificationOrSMS(3)" class="mrL btn btn-medium btn-primary" value="Change Ip">
</div>
<!--//Right Section-->
...
function exportClientList()
{
    ...
    window.open(
	'/controller/adminManageClientCnt.php?action=exportClientList&q='+keyword+'&value='+value+'&'+$('#pushForm').serialize(),
	...
	appStatus = 1;
     ...
     var str = "";
			    ...
			    var contactStr = '';
                            ...
                            //chcek select all option is checked or not 
				...
				contactStr = '<input style="position: relative;left: -80px;top: 30px" onchange="showHide()" type="checkbox" class="check" contactToken ="'+value.contact_no+'" appStatus ="'+appStatus+'" '+$selectallcheck+'>';
			    else if(appStatus  == 1)
				contactStr = '<input style="position: relative;left: -80px;top: 30px" onchange="showHide()" type="checkbox" class="check" contactToken ="'+value.uname+'" appStatus ="'+appStatus+'" '+$selectallcheck+'>';
...
function sendPushNotificationOrSMS(contentType)
{
...
// send type =  send sms or call 
// contentType = 2 add long code number, 3 ip change  
...
var ipdata = 0;
if(contentType == 3){
   parseUdp = $('input:radio[name=perseUdp]:checked').val();
   port = $('#port').val();
   ipdata = $('#ipdata').val();
