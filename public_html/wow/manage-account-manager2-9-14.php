...
<div class="quickSearch">
         ...
         <input type="text" id="searchUser" onkeyup="advanceSearchAcm($(this).val())" placeholder="Search Client.." class="allTypeUser">
       
		 <div class="replaceBttn fl" style="position: relative;top: -32px;left: 261px;">
			<a href="#!manage-account-manager.php|add-account-manager.php" class="arBorder cmniner secondry fl cp primary" title="Add"><span class="ic-16 add " id="addtariffbtn"></span></a>
		</div>
    </div>
...
//  alert('Handler for .keyup() called.');
   ...
   var searchUrl='/controller/acmController.php';
   var xhr;
   ...
   if(globalTimeout != null) 
	...
	globalTimeout=setTimeout(function(){
                ...
                type: "POST",
                dataType:"JSON",
                data:{'action':'getAcmList','q':keyword},
                url: searchUrl,
                success: function(msg){
                    var str = acmListDesign(msg);
                     $("#mngClntList").html('');
                     $("#mngClntList").html(str);
                     loadMoreDetail(2,msg.pages,'/controller/acmController.php?action=loadMoreAcmByPage','createAcmList','mngClntList');
                }//function end
...
function deleteAcm(ths,acmId)
{
    ...
    {
            $.ajax({
                    url : "/controller/acmController.php?action=deleteAcm",
                    type: "POST", 
                    data:{acmId:acmId},
                    dataType: "json",
...
function acmListDesign(data)
{
 ...
 onclickFun = "window.location.href='#!manage-account-manager.php|update-account-manager.php?acmId="+item.acmId;
                 str+="<li onclick="+onclickFun+"\'>\
          <div class='linkCont'>\
            <span class='ic-16 link'>\
            </span>\
            <div class='showLinksCont dn'>\
                <span class='blackThmCrl'>item.fullName</span>\
                <span class='blueThmCrl'>\
                </span>\
            </div>\
           </div>\
          <div class='usrDescr'>\
            <div class='fixed'>\
                <p class='uname ellp'>"+item.fullName+"<h3 class='yelloThmCrl ellp'>"+item.userName+"</h3> \
                <span>"+item.number+"</span>\
                </p>\
            <div class='funder'>\
                <span onclick=\"deleteAcm(this,"+item.acmId+" );\" title='Delete' class='ic-24 delete cp'>\
                </span>\
            </div>\
        </div>\
    </div></li>";
