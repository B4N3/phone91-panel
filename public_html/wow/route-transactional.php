...
<!--            <li ><a href="#tabs-4" title="Edit Route Detail"><span class="ic-40 setting"></span></a></li>-->
    ...
    <div id="tabs-3" class="pd15">
     	<div class="leftSetform">
			    ...
			    <input type="hidden" id="sipBuddyId" name="sipBuddyId" value="" />
                  </div>
                    <div class="fields">
		     ...
		     <div class="fields">
                       ...
                       <input type="text" name="routeCallLimit" id="routeCallLimit" value="" />
                     <input type ="hidden" name="oldRouteCallLimit" id="oldRouteCallLimit" value=""/>     
                  </div>
		   <div class="fields">
                        <label>Port</label>
                       <input type="text" name="routePort" id="routePort" value="" />
                     <input type ="hidden" name="oldRoutePort" id="oldRoutePort" value=""/>     
                  </div>
		   <div class="fields" style="width: 522px;">
                        <label>Allow Port</label>
			...
			<div id="selCodex" style="width: 300px;height: 96px;position: relative;top: -31px;left: 239px;border-width: 4px;background-color: #F2F2F2;border-color: #E15F26;"></div>
                     <input type ="hidden" name="oldAllowCodecs" id="oldAllowCodecs" value=""/>     
                  </div>  
            ...
            <div class="rightSetForm" style="float: left;left: 375px;position: absolute;top: 42px;width: 300px;">
                <form class="formElemt settingform" id="editDivertedRoute">
                   ...
                   <input type="submit" id="saveRouteInfo" class="mrT btn btn-medium btn-primary" name="Done" value="Done" title="Done">
                  </form> 
...
function designTransactionLog(text){
 ...
 $.each( text.detail, function(key, item ) {
  str += '<tr class="hvrParent">\
    ...
    <td>'+parseFloat(text.totalClosingAmt).toFixed(2)+'</td>\
    </tr>';
...
function getRouteTransactionLog(routeId,pageNo){
 $.ajax({
                   ...
                   type: "POST", 
                   data:{routeId:routeId,pageNo:pageNo},
                   dataType: "json",
                   success:function (text)
                   {
...
$(document).ready(function() {
		...
		var options = { 
			...
			success:     
                                function(text)
                                {
				    $('#saveRouteInfo').removeAttr('disabled');
                                    show_message(text.msg,text.status);                                 
                                }
		};
 ...
 {
     $('#saveRouteInfo').attr('disabled','disabled');
    $.validator.setDefaults({
...
//                      
  ...
  function getRouteInfo(routeId){
    ...
    $.ajax({
			    ...
			    $('#routeCallLimit').val(text.routeCallLimit);
			    $('#routePort').val(text.port);
			    
                            $('#routePrefix').val(text.prefix);
                            console.log(text.tariffId);
                            $('#tariff').val(text.tariffId);
                            $('#sipBuddyId').val(text.sipBuddyId);
			    var tableli = '';
			    
			    
			    globalCodex = text.allowCodecs;
			    
			    console.log(text.allowCodecs);
			    if(text.allowCodecs != undefined)
				$.each(text.allowCodecs,function(index,value){
				
	tableli+='<table cellpadding="0" class="cf hX" style="display:inline-block;margin:2px ">\
						     <tbody>\
							     <tr class="hR">\
								     ...
								     <td class="hV hM" style="background-color: #ddd; color: #666;float:left;padding: 2px 2px 2px 4px;cursor:pointer">\
									     <span class="hO" name="^i" onclick="removeCodex(this,\''+value+'\')" title="" role="button" tabindex="0">x</span><input type="hidden" name="codexNames[]" value="'+value+'">\
								     </td>\
							     </tr>\
						     </tbody>\
					     </table>';
