...
if (!preg_match("/^[0-9]+$/", $_REQUEST['clientId'])) {
...
if(isset($vContactEmailArr[0]) && $vContactEmailArr[0] == 0){
...
<div id="tabs" class="mngClntFnction">
    <ul>
	    ...
	    <li onclick="getUserInfoAcm(<?php echo $managerId; ?>,<?php echo $clientId; ?>)"><a href="#tabs-4" title="Settings"><span class="ic-40 setting"></span></a></li>
            <li onclick="getUserSysDetail(<?php echo $clientId; ?>)"><a href="#tabs-5" title="Latest info"><span class="ic-40 latestinfo"></span></a></li>
            <li onclick="getUserLimitedPlanDetail(<?php echo $clientId; ?>)"><a href="#tabs-6" title="Liminted Plan"><span class="ic-40 addsip"></span></a></li>
            <li onclick="getUserDidDivertDetail(<?php echo $clientId; ?>)"><a href="#tabs-7" title="DidDivert info"><span class="ic-40 tranLog"></span></a></li>
   </ul>
...
//                        die();
        ...
        <div>
           ...
           <form class="formElemt " id="setDialPlanAndRoute">
            ...
            <div class="">
            <input type="radio" name="routeDialplan" value="route" id="routeradio" checked="checked" onchange="showDialorRouteDiv(this)">Route
            <input type="radio" name="routeDialplan" value="dialPlan" id="dialplanradio" onchange="showDialorRouteDiv(this)">Dial Plan
            </div>
        ...
        <div>
           ...
           <form class="formElemt " id="setlimitedPlan">
            <div class="add clear oh mrB1">
                    <span class="ic-12 lgrayArr db fl"></span>
                    <label class="db fl fwd">Assign Limited Plan : </label>
            </div>
...
<!--            <select name="routeList" id="routeList" style="">
             ...
             <select name="limitedPlanList" id="limitedPlanList" style="">
            </select>
            <input type="hidden" name="clientId" value="<?php echo $clientId;?>" />
           <input class="mrT2 btn btn-medium btn-primary"  value="Done" type="submit" title="Save Changes" />
          </form>  
    ...
    <div id="tabs-6">
    	 <div class="tablflip-scroll" id="userLimitedPlanTable">

             <div id="successDetail">
                 </br></br>
             <div>
                 <h3> Limited Plan Name : </h3><h3 class="yelloThmCrl ellp" id="limitedPlanName" ></h3>
            </div>
                 </br>
              <div>
                 <h3> Remaining Minute : </h3><h3 class="yelloThmCrl ellp" id="remainingPlanMinute" ></h3>
            </div>
                 </br>
              <div>
                 <h3> Remaining Real Credit : </h3><h3 class="yelloThmCrl ellp" id="remainingRealCredit" ></h3>
            </div>
             </div>
             
             <div id="errorDetail" class="dn">
             </div>
    ...
    <!--  // tab 6 end -->
    <div id="tabs-7">
        <div id="editOption"></div></br></br>
    	 <div class="tablflip-scroll" id="DidDivertTable">
      		   <table width="100%" cellspacing="0" cellpadding="0" border="0" class="cmntbl  boxsize" id="DidDivertInfo">
			...
			success:     
              ...
              $('#setDialPlanAndRoute').ajaxForm(options);    
               ...
               var options = { 
                        ...
                        url:"/controller/adminManageClientCnt.php?action=setUserlimitedPlan", 
			dataType: 'json',
			type:'POST', 
			success:     
                                function(text)
              ...
              $('#setlimitedPlan').ajaxForm(options);    
...
//                      
  ...
  function getUserLimitedPlanDetail(userId){
      $.ajax({
                   url : "/controller/adminManageClientCnt.php?action=getUserLimitedPlanDetail", 
                   type: "POST", 
                   data:{userId:userId},
                   dataType: "json",
                   success:function (text)
                   {
                       if(text.status == "success"){
                         $('#limitedPlanName').html(text.planName); 
                         $('#remainingPlanMinute').html(text.remainingMinute +" minute"); 
                         $('#remainingRealCredit').html(parseFloat(text.remainingRealCredit).toFixed(3) +" "+text.currency); 
                         $('#successDetail').show();
                         $('#errorDetail').hide();
                        console.log(text.planName + text.remainingMinute  + text.remainingRealCredit);
                       }else
                       {
                         $('#errorDetail').html("</br></br><h3> There are no any limited plan assign to user. </h3>")  
                         $('#successDetail').hide();
                         $('#errorDetail').show();
                       }
...
//                      
  ...
  function getUserDidDivertDetail(userId){
      $.ajax({
                   url : "/controller/adminManageClientCnt.php?action=getUserDidDivertDetail", 
                   type: "POST", 
                   data:{userId:userId},
                   dataType: "json",
                   success:function (text)
                   {
                      if(text.status == "success"){
                       var str = didDivertDesign(text.DidDivertDetail,userId);  
                       $('#DidDivertTable').html('');
                       $('#DidDivertTable').html(str);
                       $("#DidDivertInfo tbody tr:visible:even").addClass("even"); 
                       $("#DidDivertInfo tbody tr:visible:odd").addClass("odd");
                      }
//                      
                   }
  });
  ...
  function getUserInfoAcm(managerId,clientId){
    $.ajax({
...
//                        $('#systemInfo').html(str);
     ...
     $.ajax({
                    url : "/controller/adminManageClientCnt.php?action=getlimitedPlanList", 
                    type: "POST", 
                    data:{clientId:clientId},
                    dataType: "json",
                    success:function (text)
                    {
                        var str = '';
                         ...
                         $.each(text.limitedPlanData, function(key, item ) {
                             var selectroute = '';
                             if(text.userPlan == key){
                               selectroute = 'selected = "selected"';   
                             }
                             str += '<option value='+key+' '+selectroute+'>'+item+'</option>';
                         })
                        $('#limitedPlanList').html(str);
  ...
  function ipDetailDesign(text){
                                ...
                                <th width="20%">Latest Login Time  </th>\
                                <th width="20%">Ips</th>\
                                <th width="20%">Browser</th><th width="20%">Country</th>\
                                <th width="60%">&nbsp;</th>\
                                        </tr>\
                                        </thead>\
                                        <tbody>\
  ...
  str += '<tr class="even">\
                ...
                <td>'+item.browser+'</td><td>'+item.countryName+'</td>\
                <td>&nbsp;</td>\
          </tr>';
  ...
  function didDivertDesign(text,userId){
            var str ='<table width="100%" cellspacing="0" cellpadding="0" border="0" class="cmntbl  boxsize" id="DidDivertInfo">\
                <thead>\
                                <tr> \
                                <th width="20%">assignId</th>\
                                <th width="20%">Source No</th>\
                                <th width="20%">Destination No</th>\
                                <th width="20%">LongCodeNo</th>\
                                <th width="20%">LongCodeType</th>\
                                <th width="20%">Expiry Date</th>\
                                <th width="20%">Country</th>\
                                <th width="20%">State</th>\
                                <th width="20%">isEnable</th>\
                                <th width="20%">renew</th>\
                                </tr>\
                                </thead>\
                                <tbody>';
  ...
  $.each( text, function(key, item ) {
  str += '<tr class="even">\
                <td assigId = "'+item.assignId+'" userId="'+userId+'" onclick="editDidDivert(this);">'+item.assignId+'</td>\
                <td>'+item.sourceNo+'</td>\
                <td>'+item.destinationNo+'</td>\
                <td>'+item.longCodeNo+'</td>\
                <td>'+item.longCodeType+'</td>\
  ...
  function editDidDivert(ths){
     var assignId = $(ths).attr('assigId');
     var userId = $(ths).attr('userId');
     $.ajax({
                    url : "/controller/adminManageClientCnt.php?action=getDidDivtViaAssignId", 
                    type: "POST", 
                    ...
                    success:function (text)
                    {
                       ...
                       if(text.status == "success"){
                            ...
                            var str = '<div><form id="EditDidDivertDetail">';
                            var didDivertDetail = text.DidDivtData;

                            if(didDivertDetail.longEx == 1){
                               str +='<label>LongCode No</label>\
                                       <input type="text" name="longcodeNo" id="longCodeNo" value="'+didDivertDetail.longCodeNo+'"/>';
                            }else{
                               str +='<label>Expiry Date</label>\
                                       <input type="text" name="expiryDate" id="expiryDate" value="'+didDivertDetail.expiryDate+'"/>';
                            }

                            if(didDivertDetail.renew == 1){
                             var autorenew = "checked";
                             var offrenew = '';
                            }else
                            {
                             var autorenew = "";
                             var offrenew = "checked";
                            }
                            str += '<input type="radio" name="renew" value="1" '+autorenew+'>AutoRenew\
                                   <input type="radio" name="renew" value="0" '+offrenew+'>RenewOff';

                            if(didDivertDetail.isEnable == 1){
                             var enable = "checked";
                             var disable = '';
                            }else
                            {
                            ...
                            str += '<input type="radio" name="isEnable" value="1" '+enable+'>Enable\
                                   <input type="radio" name="isEnable" value="0" '+disable+'>Disable';


                            str +='<input type="hidden" name="assignId" value="'+assignId+'" />\
                                    <input type="hidden" name="userId" id="userIdData" value="'+userId+'" />\
                                    <input type="hidden" name="planId" id="planId" value="'+didDivertDetail.planId+'" />\
                                  <input class="mrT2 btn btn-medium btn-primary"  value="Done" type="button" onclick="editdidDivertDetail();" title="Save Changes" />\
                                   </form></div>'
  ...
  function editdidDivertDetail(){
     
                    $.ajax({
                        url:"/controller/adminManageClientCnt.php?action=editDidDivertDetail", 
                        type:'POST',        
			dataType: 'json',
                        data:$('#EditDidDivertDetail').serialize(),
			success:     
                                function(text)
                                {
                                     show_message(text.msg,text.status);
                                     if(text.status == "success"){
                                     getUserDidDivertDetail($('#userIdData').val());
                                     $('#editOption').html('');
                                        }
                                }
                                 });
