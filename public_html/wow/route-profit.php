...
<div class="commHeader">
    ...
    <div class="dateSearch"> From:<input type="text" id="fromDate" name="fromDate" value="" class=""> To:<input type="text" id="toDate" name="toDate" value="" class=""/> <button id="showChart" class="" onclick="getRouteProfitDetails();
        getRoutePrefixProfitDetails();">Go</button></div>
    ...
    <div class="logRight">
        <div id="resellerLog">
            ...
            <div class="rlFields">
                <label><h1>Total Profit : <span class="rlStrong" id="totalProfit">0</span> </h1></label>
...
<div class="logWrap">
    <div class="logLeft">
		...
		<div id="routeProfitCheck" class="dn">
            ...
            <div id="routePrefixProfit" class="dn"> 
...
//var detailsResponse;
    function getRouteProfitDetails()
    {
        ...
        $.ajax({
            ...
            success: function(response)
            {
                ...
                $('#totalProfit').html("  $" + profit.toFixed(4));

				$('#routeProfitCheck').show();
            }
        })
    }
    ...
    function getRoutePrefixProfitDetails()
    {
        ...
        $.ajax({
            url: "/controller/userCallLog.php",
            data: {"call": "getRoutePrefixProfitDetail", "fromDate": fromDate, "toDate": toDate},
            type: "post",
            dataType: "json",
            success: function(response)
            {
                ...
                if (response == null || response.length == 0) {
                    $('#routePrefixProfit').hide();
                    return false;
                }
                $('#routePrefixProfit').html('');
                $.each(response, function(key, element) {
                    console.log(key);

                    var profitDiv = '<h1 id="' + key + 'detail"></h1><div class="cols2 col" style="width: 600px; float: left;">\
                                        <div class="colInner">\
                                        <h3>  Profit </h3>\
                                        <div id="' + key + 'ProfitGraphTitle" class="gridTitle"></div>\
                                        <div id="' + key + 'ProfitGraphContainer"></div>\
                                        </div>\
                                    </div>\
                                    <div class="cols2 col" style="margin-left: 620px;">\
                                        <h3>  Loss </h3>\
                                        <div id="' + key + 'LossGraphTitle" class="gridTitle"></div>\
                                        <div id="' + key + 'LossGraphContainer"></div>\
                                        </div>\
                                    </div>';


                    $('#routePrefixProfit').append(profitDiv);
                    var profitDetail = new Array();
                    var lossDetail = new Array();
                    $.each(element, function(profitkey, profitelement) {

                        if (profitelement.routeProfit >= 0) {
                            var arrayProfit = new Array();
                            arrayProfit['prefix'] = profitelement.prefix;
                            arrayProfit['routeProfit'] = profitelement.routeProfit;
                            profitDetail.push(arrayProfit);
                        } else {
                            var arrayLoss = new Array();
                            arrayLoss['prefix'] = profitelement.prefix;
                            arrayLoss['routeProfit'] = profitelement.routeProfit * -1;
                            lossDetail.push(arrayLoss);
                        }

                    })
                    var profitDivName = key + 'Profit';
                    var lossDivName = key + 'Loss';
                    callBackDrawProfitGraph(profitDetail, profitDivName);
                    callBackDrawProfitGraph(lossDetail, lossDivName);

                    $('#' + key + 'detail').html(key + ' (USD)');

                })

                $('#routePrefixProfit').show();
...
//				$('#totalProfit').html(profit);
    ...
    function callBackDrawProfitGraph(data, call) {
        var categories = [];
        var dataArray = [];
        var mix = [];
        var i = 0;
        ...
        $.each(data, function(key, element) {
            ...
            categories[i] = key;
            dataArray[i] = element;
            mix[i] = new Array(element.prefix + '(prefix) ', parseFloat(element.routeProfit)); //get value and percentage
            i++; //increament counter                    
        }); //end of each
