...
if(!preg_match('/^[0-9]+/',$_REQUEST['tariffId']))
{
    echo "please select valid plan";
    die();
}    
...
<div class="planrowEdit dn" id="planrowEdit">
    <form name="editPlanAdmin" id="editPlanAdmin" method="post">
     ...
     <span class="fl text">Output Currency</span>
        <select class="currency currencyList" id="opCurr" name="outputCurr">
...
<div id="formWrap"> 
        ...
        <div class="tablflip-scroll mrT dn tariff_table_div">
           ...
           <thead>
                    <tr>
                        ...
                        <th width="22%" class="noBorder">Rate(<?php echo $planArr['currency']; ?>)</th>
    ...
    <div id="tabs-2" class="mrB1 pr">
           ...
           <div class="showDetls">
                ...
                <select class="small fl currencyList genSelect" name="fileCurrency" id="currency">
                     <!--  do not remove currencylist class -->
               </select>
 ...
 <div class="tablflip-scroll mrT clear">
   <table width="100%" cellspacing="0" cellpadding="0" border="0" id="clientTrstable" class="cmntbl  boxsize grayTabl">
            <thead>
                <tr>
                    <th width="10%"><input type="text" name="cntryCode" placeholder="Country Code" onkeyup="searchTariff($(this).val(),1,'num')" /></th>
                        <th width="10%"><input type="text" name="cntry" placeholder="Country" onkeyup="searchTariff($(this).val(),1,'alpha')" /></th>
                    <th width="10%">Operator</th>
                    <th width="22%" class="noBorder">Rate(<?php echo $planArr['currency']; ?>)</th>
                    <th width="50%" class="noBorder">&nbsp;</th>
                </tr>
...
//    clearForm:true,
    ...
    success: function(response)
    {
       ...
       $('#addPlanAdmin')[0].reset();
        if(response.status == "success")
            getTariffDetails(response.insertId,20,'admin');
    }
}
...
var optionsEditPlanAdmin = {
        ...
        return validateAdminEditPlanForm('idNameEdit')},		
        success: function(response)
        {
            show_message(response.msg,response.status);
            getPlanList();
