...
//        $pwd = $this->db->real_escape_string($pwd);
     ...
     * @return boolean
     */
  function login_user($userId, $pwd, $remember_me,$host = NULL,$signUp = 0,$tempVar= array(),$domainName = '',$fileName=NULL) 
  {
        session_start();
        session_unset();
        session_destroy();
...
//            $sendurl = $fileName."?error=".$_SESSION['error'].$this->param ;
        ...
        if ($result && $result->num_rows > 0 ) 
        {
            ...
            if($this->fbgl == true){
                ...
                $cntClsOnj = new contact_class();
                $resultVerifiedId = $cntClsOnj->addVerifiedEmailId($this->email,$getUserInfo['userId'],$getUserInfo['resellerId'],$host);
...
//    }
     ...
     */
    function verifyCode($code,$number,$type=NULL, $domainName = NULL ) 
    {
        ...
        if(!$resellerId)
            return json_encode(array("msg"=>"Error Invalid domain","status"=>"error"));
